...
mk_inode(uint8_t *raw, uint32_t inode_size, uint16_t extra_isize)
{
	...
	csum = ref_crc32c(csum, raw + offsetof(struct ext4fs_dinode,
	    i_nfs_generation), 4);
...
test_known_vectors(void)
{
	...
	for (size_t v = 0; v < sizeof(vectors) / sizeof(vectors[0]); v++) {
		...
		csum = ext4fs_inode_csum(&fs,
		    (struct ext4fs_dinode_large *)raw, 0x12345678);
		CHECK(csum == vectors[v].want, "inode csum known vector");
		if (csum != vectors[v].want)
			fprintf(stderr, "  size=%u extra=%u got=0x%08x "
			    "want=0x%08x\n", size, vectors[v].extra,
			    csum, vectors[v].want);
		free(raw);
	}
}
...
test_csum_roundtrip(void)
{
	...
	for (size_t v = 0; v < sizeof(sizes) / sizeof(sizes[0]); v++) {
		...
		if (size > 160) {
			raw[160] ^= 0x01;
			CHECK(ext4fs_inode_csum_verify(&fs, dp, ino) ==
			    EINVAL, "inode csum corrupted tail rejected");
		}

		/* A different inode number must fail. */
		CHECK(ext4fs_inode_csum_verify(&fs, dp, ino + 1) ==
		    EINVAL, "inode csum wrong ino rejected");
...
test_extra_isize_validation(void)
{
	...
	CHECK(ext4fs_inode_extra_isize(&fs, dp) == 0,
	    "128-byte inode extra 0");
...
main(void)
{
	...
	if (failures != 0) {
		fprintf(stderr, "ext4fs-inode-tests: %d failure(s)\n",
		    failures);
		return 1;
	}
