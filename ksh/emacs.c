...
static	int	question_skip;
static	int	question_redraw;
...
x_redraw(int limit)
{
	...
	x_flush();
	if (xbp == xbuf) {
		x_col = questionlen(question, NULL);
		if (x_col > xx_cols)
			truncate = (x_col / xx_cols) * xx_cols;
		if (question_redraw)
			pquestion(question + question_skip, truncate);
	}
	if (x_col > xx_cols)
		x_col = x_col - (x_col / xx_cols) * xx_cols;
	x_displen = xx_cols - 2 - x_col;
...
x_adjust(void)
{
	...
	/*
	 * we had a problem if the question length > xx_cols / 2
	 */
