...
struct lex_state {
	...
	union {
		...
		struct scsparen_info {
			...
			int case_depth;/* nesting depth of case...esac inside $(...) */
			int in_word;   /* simple keyword scan state */
			char wbuf[6];  /* small buffer for keyword: case/esac/in */
			int wlen;
...
static char	*special_question_expand(char *);
static int	dopquestion(const char *, int, const char **, int);
int		questionlen(const char *cp, const char **spp);
  ...
  Again:
	...
	while (!((c = getsc()) == 0 ||
		...
		case SCSPAREN: /* $( .. ) */
			...
			case 0: /* normal */
				/* simple keyword tracking for case/esac/in at top level (unquoted) */
				if (letnum(c) || c == '_') {
					if (statep->ls_scsparen.wlen < (int)sizeof(statep->ls_scsparen.wbuf)-1) {
						statep->ls_scsparen.wbuf[statep->ls_scsparen.wlen++] = (char)c;
						statep->ls_scsparen.wbuf[statep->ls_scsparen.wlen] = '\0';
					}
					statep->ls_scsparen.in_word = 1;
				} else {
					/* word boundary: check keyword if any */
					if (statep->ls_scsparen.in_word) {
						char *w = statep->ls_scsparen.wbuf;
						if (strcmp(w, "case") == 0)
							statep->ls_scsparen.case_depth++;
						else if (strcmp(w, "esac") == 0 && statep->ls_scsparen.case_depth > 0)
							statep->ls_scsparen.case_depth--;
						/* 'in' doesn't change depth, we just rely on depth>0 */
					}
					statep->ls_scsparen.in_word = 0;
					statep->ls_scsparen.wlen = 0;
					statep->ls_scsparen.wbuf[0] = '\0';
				}
				switch (c) {
				case '(':
					...
					break;
				case ')':
					if (statep->ls_scsparen.nparen == 1 && statep->ls_scsparen.case_depth > 0) {
						/* ignore case/esac pattern ')': do not close $(...) */
						/* nothing */;
					} else {
						statep->ls_scsparen.nparen--;
					}
...
static char *
special_question_expand(char *str)
{
	char *p = str;
...
set_question(int to)
{
	...
	case PS1: /* command */
		...
		if (sigsetjmp(genv->jbuf, 0)) {
			...
			char *tmp = special_question_expand(ps1);
			question = str_save(substitute(tmp, 0), saved_atemp);
		}
		quitenv(NULL);
		break;
	case PS2: /* command continuation */
		question = str_val(global("PS2"));
		break;
	}
}

static int
dopquestion(const char *sp, int ntruncate, const char **spp, int doprint)
{
	...
	while (*cp != 0) {
		...
		if (*cp == '\\') {
			...
			case 'n':	/* '\' 'n' newline */
				strbuf[0] = '\n';
				strbuf[1] = '\0';
				totlen = 0;	/* reset for question re-print */
				sp = cp + 1;
				break;
			case 'p':	/* '\' '$' $ or # */
				strbuf[0] = ksheuid ? '$' : '#';
...
void
pquestion(const char *cp, int ntruncate)
{
	dopquestion(cp, ntruncate, NULL, 1);
}

int
questionlen(const char *cp, const char **spp)
{
	return dopquestion(cp, 0, spp, 0);
}
