...
c_read(char **wp)
{
	...
	if ((cp = strchr(*wp, '?')) != NULL) {
		*cp = 0;
		if (isatty(fd)) {
			/* at&t ksh says it prints question on fd if it's open
...
c_eval(char **wp)
{
    ...
    /* POSIX: eval takes no options. In POSIX mode, treat all arguments as operands. */
    if (Flag(FPOSIX)) {
        s = pushs(SWORDS, ATEMP);
        s->u.strv = wp + 1; /* pass all args after "eval" */
    } else {
        if (ksh_getopt(wp, &builtin_opt, null) == '?')
            return 1;
        s = pushs(SWORDS, ATEMP);
...
c_brkcont(char **wp)
{
	...
	if (quit) {
		...
		/* POSIX: do not warn when n exceeds loop depth; use outermost loop */
	if (!Flag(FPOSIX)) {
