...
/* POSIX -h prehash control */
static int ksh_prehash_mode = 0;
static void prehash_function_tree(struct op *t);
static void prehash_walk(struct op *t);
#include "c_test.h"
...
define(const char *name, struct op *t)
{
	...
	/* POSIX: if -o posix and set -h, pre-hash utilities used by this function at definition time */
	if (Flag(FPOSIX) && Flag(FTRACKALL)) {
		ksh_prehash_mode = 1;
		prehash_function_tree(tp->val.t);
		ksh_prehash_mode = 0;
	}
	return 0;
}

/* Walk a function tree and pre-resolve external utilities for POSIX -h.
 * We traverse all TCOM nodes; for each with a command word, we call
 * findcom() which, in prehash mode, will create a tracked alias entry.
 * We avoid expanding parameters — we only use the literal command name.
 */
static void
prehash_walk(struct op *t)
{
	if (!t)
		return;
	/* Only prehash simple commands and descend into compound nodes */
	switch (t->type) {
	case TCOM:
		if (t->args && t->args[0]) {
			const char *raw = wdstrip(t->args[0]);
			if (raw && *raw && strchr(raw, '=') == NULL) {
				/* Trigger path resolution; builtins/functions are ignored */
				(void)findcom(raw, FC_BI|FC_FUNC|FC_PATH);
			}
		}
		break;
	case TPAREN: case TBRACE: case TLIST: case TPIPE: case TOR: case TAND:
	case TFOR: case TSELECT: case TWHILE: case TUNTIL: case TIF: case TELIF:
	case TELSE: case TCASE: case TASYNC: case TTIME: case TEXEC: case TCOPROC:
		/* Recurse into both sides/children if present */
		prehash_walk(t->left);
		prehash_walk(t->right);
		break;
	case TFUNCT:
		/* Prehash the function body as well */
		prehash_walk(t->left);
		break;
	default:
		break;
...
static void
prehash_function_tree(struct op *t)
{
	prehash_walk(t);
}
...
findcom(const char *name, int flags)
{
	...
	int insert = (Flag(FPOSIX) ? ksh_prehash_mode : Flag(FTRACKALL));	/* POSIX: in posix mode, only insert during prehash of functions */
