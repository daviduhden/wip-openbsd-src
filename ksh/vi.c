...
static int	pwidth;			/* display columns needed for question */
static int	question_trunc;		/* how much of question to truncate */
static int	question_skip;		/* how much of question to skip */
...
edit_reset(char *buf, size_t len)
{
	...
	cur_col = pwidth = questionlen(question, &p);
	question_skip = p - question;
	if (pwidth > x_cols - 3 - MIN_EDIT_SPACE) {
		cur_col = x_cols - 3 - MIN_EDIT_SPACE;
		question_trunc = pwidth - cur_col;
		pwidth -= question_trunc;
	} else
		question_trunc = 0;
...
#endif
	/* Only print the full question if we cleared the screen. */
...
display(char *wb1, char *wb2, int leftside)
{
	 ...
	 * In this first loop, col does not include the question.
	 */
	...
	*twb1 = ' ';
	 ...
	 * In this final loop, col includes the question.
	 */
...
static void
vi_pquestion(int full)
{
	pquestion(question + (full ? 0 : question_skip), question_trunc);
}
