...
static void DrawTrianglePattern(Window, GC, GC, GC, GC, int, int, int,
                                int, char);
static void DrawSeparator(Window, GC, GC, int, int, int, int, int);
static void DrawUnderline(MenuRoot *mr, GC gc, int x, int y, char *txt,
                          int off);
static MenuStatus MenuInteraction(MenuRoot *menu, MenuRoot *menuPrior,
                                  MenuItem **pmiExecuteAction,
...
static void PopDownAndRepaintParent(MenuRoot *mr,
                                    Bool *fSubmenuOverlaps);
static int DoMenusOverlap(MenuRoot *mr, int x, int y, int width,
                          int height, Bool fTolerant);
static Bool FPopupMenu(MenuRoot *menu, MenuRoot *menuPrior, int x,
                       int y, Bool fWarpItem, MenuOptions *pops,
                       Bool *ret_overlap);
...
#define IS_TITLE_MENU_ITEM(mi)                                         \
  (((mi) ? ((mi)->func_type == F_TITLE) : FALSE))
#define IS_POPUP_MENU_ITEM(mi)                                         \
  (((mi) ? ((mi)->func_type == F_POPUP) : FALSE))
...
MenuStatus do_menu(MenuRoot *menu, MenuRoot *menuPrior,
                   ...
                   Bool fStick, XEvent *eventp, MenuOptions *pops)
{
  ...
  if (!fFailedPopup)
    retval = MenuInteraction(menu, menuPrior, pmiExecuteAction,
                             cmenuDeep, fStick);
  else
    retval = MENU_ABORTED;
  cmenuDeep--;
  ...
  if (cmenuDeep == 0)
  {
    ...
    if (retval == MENU_DONE || retval == MENU_DONE_BUTTON)
    {
      ...
      {
        cindirectDeep++;
        ExecuteFunction((*pmiExecuteAction)->action, ButtonWindow,
                        &Event, Context, -1);
        cindirectDeep--;
      }
...
static void GetPreferredPopupPosition(MenuRoot *mr, int *px, int *py)
{
  int menu_x, menu_y;
...
static int IndexFromMi(MenuItem *miTarget)
{
  int i = 0;
...
static MenuItem *MiFromMenuIndex(MenuRoot *mr, int index)
{
  int i = -1;
  MenuItem *mi = mr->first;
...
static int CmiFromMenu(MenuRoot *mr) { return IndexFromMi(mr->last); }
                                  ...
                                  int cmenuDeep, Bool fSticks)
{
  ...
  Bool fPopupAndWarp =
    FALSE; /* used for keystrokes, to popup and move to
			       * that menu */
  ...
  while (TRUE)
  {
    ...
    case MotionNotify:
      ...
      {
        int x_root, y_root;
        XQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild, &x_root,
                      &y_root, &JunkX, &JunkY, &JunkMask);
    ...
    if (mi)
    {
      ...
      if (fPopup)
      {
        ...
        else
        {
		 ...
		 want to be able to popdown w/o actually removing the menu */
            ...
            {
              x = mops.pos_hints.x +
                  mops.pos_hints.x_factor * mrPopup->width;
              y = mops.pos_hints.y +
                  mops.pos_hints.y_factor * mrPopup->height;
            }
            else
            {
      ...
      if (fDoMenu)
      {
        ...
        retval =
          do_menu(mrPopup, menu, pmiExecuteAction, cmenuDeep, FALSE,
      ...
      if (menu->selected)
      {
        ...
        if (mrPopup && fOffMenuAllowed == FALSE)
        {
          ...
          XQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild, &x, &y,
                        &JunkX, &JunkY, &JunkMask);
          XGetGeometry(dpy, menu->w, &JunkRoot, &mx, &my, &mw, &mh,
                       &JunkBW, &JunkDepth);
      ...
      (*pmiExecuteAction)->action)
  {
    ...
    case F_WINDOWLIST:
      ...
      else
      {
        ...
        {
          GetPreferredPopupPosition(menu, &lastMenuPosHints.x,
                                    &lastMenuPosHints.y);
  ...
  if (!(pops->flags.f.has_poshints))
  {
    ...
    if ((Tmp_win) && (menuPrior == NULL) && (Context & C_RALL))
    {
      ...
      x = Tmp_win->frame_x + Tmp_win->frame_width -
          ...
          ButtonPosition(Context, Tmp_win) * Tmp_win->title_height -
          menu->width + 1;
    }
  ...
  if (menuPrior != NULL)
  {
         ...
         pops->pos_hints.fRelative == FALSE || x_overlap == 0))
    {
	 ...
	 relative positioning hints */
      ...
      x_offset =
        prev_width * menuPrior->ms->feel.PopupOffsetPercent / 100 +
      ...
      if (x + menu->width < prev_x + right_x)
        ...
        fDefaultLeft = FALSE;
      fEmergencyLeft =
        (prev_x > Scr.MyDisplayWidth - right_x) ? TRUE : FALSE;
      ...
      if (menu->ms->feel.f.Animated)
      {
        ...
        AnimatedMoveOfWindow(menuPrior->w, prev_x, prev_y, end_x,
                             prev_y, TRUE, -1, NULL);
      } /* if (menu->ms->feel.f.Animated) */
      else if (prev_x + x_offset > x + 3 &&
               x_offset + 3 > -menu->width && !(pops->flags.f.fixed))
      {
...
static void SetMenuItemSelected(MenuItem *mi, Bool f)
{
  ...
  case BGradMenu:
    if (f == True)
    {
      ...
      XGetGeometry(dpy, mi->mr->w, &JunkRoot, &JunkX, &JunkY, &mw, &mh,
                   &JunkBW, &JunkDepth);
      if (iy + ih > mh)
        ih = mh - iy;
      /* grab image */
    ...
    else if (f == False && mi->mr->stored_item.width != 0)
    {
      ...
      XCopyArea(dpy, mi->mr->stored_item.stored, mi->mr->w,
                ...
                mi->mr->stored_item.width, mi->mr->stored_item.height,
 ...
 *
 ***********************************************************************/
static void RelieveRectangle(Window win, int x, int y, int w, int h,
                             GC Hilite, GC Shadow)
{
...
static void PaintEntry(MenuItem *mi)
{
  ...
  if (mr->ms->look.f.Hilight)
  {
        ...
        (((*mi->item) != 0) || mi->picture || mi->lpicture))
    {
      ...
      XFillRectangle(dpy, mr->w, mr->ms->look.MenuActiveBackGC,
                     mr->xoffset + 3, y_offset + d,
                     mr->width - mr->xoffset - 6, y_height - d);
    }
    else if (th == 0)
    {
#ifdef GRADIENT_BUTTONS
      if (!fGradient)
#endif
        XClearArea(dpy, mr->w, mr->xoffset + 3, y_offset,
                   mr->width - mr->xoffset - 6, y_height, 0);
    }
    else
    {
      fClear = True;
      ...
      (fClear || !mr->ms->look.f.Hilight))
  {
...
#endif
    {
      ...
      if (th == 2 && mi->prev && mi->prev->state)
        XClearArea(dpy, mr->w, mr->xoffset, y_offset + 1, mr->width,
                   y_height - 1, 0);
      else
        XClearArea(dpy, mr->w, mr->xoffset, y_offset - th + 1,
                   mr->width, y_height + 2 * (th - 1), 0);
    }
  }
        ...
        (((*mi->item) != 0) || mi->picture || mi->lpicture))
    {
      ...
      if (th == 2)
      {
        RelieveRectangle(mr->w, mr->xoffset + 2, y_offset - 1,
                         mr->width - 4 - sw, mi->y_height + 2, ReliefGC,
                         ShadowGC);
      }
    }
  ...
  /* Top of the menu */
  if (mi == mr->first)
    DrawSeparator(mr->w, ReliefGC, ReliefGC, 0, 0, mr->width - 1, 0,
                  -1);
  ...
  if (IS_TITLE_MENU_ITEM(mi))
  {
    ...
    else if (mr->ms->look.TitleUnderlines == 1)
    {
      ...
      if (mi != mr->first)
      {
        ...
        DrawSeparator(mr->w, ShadowGC, ReliefGC, mr->xoffset + 5,
                      y_offset + 1, mr->width - 6, y_offset + 1, 1);
      }
    }
  }
  else
  ...
  if (mi->func_type == F_POPUP)
  {
    if (mi->state)
      DrawTrianglePattern(mr->w, ShadowGC, ReliefGC, ShadowGC, ReliefGC,
                          mr->width - 13, y_offset + d - 1,
                          mr->width - 7, y_offset + d + 7,
                          mr->ms->look.f.TriangleRelief);
    else
      DrawTrianglePattern(
        mr->w, ReliefGC, ShadowGC, ReliefGC, mr->ms->look.MenuGC,
  ...
  if (mi->picture)
  {
    ...
    if (mi->picture->depth > 0) /* pixmap? */
    {
      ...
      XCopyArea(dpy, mi->picture->picture, mr->w, ReliefGC, 0, 0,
                mi->picture->width, mi->picture->height, x,
                y_offset + 1);
    ...
    {
      XCopyPlane(dpy, mi->picture->picture, mr->w, currentGC, 0, 0,
                 mi->picture->width, mi->picture->height, x,
                 y_offset + 1, 1);
    }
  }
  ...
  if (mi->lpicture)
  {
    ...
    {
      XCopyPlane(dpy, mi->lpicture->picture, mr->w, currentGC, 0, 0,
                 mi->lpicture->width, mi->lpicture->height,
 ...
 *
 ****************************************************************************/
static void DrawSeparator(Window w, GC TopGC, GC BottomGC, int x1,
                          int y1, int x2, int y2, int extra_off)
{
 ...
 *
 ****************************************************************************/
static void DrawTrianglePattern(Window w, GC GC1, GC GC2, GC GC3, GC gc,
                                int l, int u, int r, int b, char relief)
{
  int m;
...
void PaintMenu(MenuRoot *mr, XEvent *pevent)
{
  ...
  if (ms)
  {
    ...
    case BGradMenu:
      ...
      if (type == HGradMenu)
      {
        if (mr->backgroundset == False)
        {
          ...
          pmap =
            XCreatePixmap(dpy, Scr.Root, mr->width, 5, Scr.d_depth);
          ...
          {
            unsigned short x =
              i * bounds.width / ms->look.face.u.grad.npixels;
      ...
      else if (type == VGradMenu)
      {
        if (mr->backgroundset == False)
        {
          register int i = 0;
          register int dh =
            bounds.height / ms->look.face.u.grad.npixels + 1;

          pmap =
            XCreatePixmap(dpy, Scr.Root, 5, mr->height, Scr.d_depth);
          pmapgc = XCreateGC(dpy, pmap, gcm, &gcv);

          while (i < ms->look.face.u.grad.npixels)
          {
            unsigned short y =
              i * bounds.height / ms->look.face.u.grad.npixels;
            XSetForeground(dpy, pmapgc,
                           ms->look.face.u.grad.pixels[i++]);
      ...
      else /* D or BGradient */
      {
        ...
        {
          if ((int)(i * ms->look.face.u.grad.npixels / numLines) >
              cindex)
          {
          ...
          if (type == DGradMenu)
            XDrawLine(dpy, mr->w, Scr.TransMaskGC, 0, i, i, 0);
          else /* BGradient */
            XDrawLine(dpy, mr->w, Scr.TransMaskGC, 0,
                      mr->height - 1 - i, i, mr->height - 1);
        }
      }
      break;
    ...
    case PixmapMenu:
      ...
      XCopyArea(dpy, p->picture, mr->w, Scr.TransMaskGC, 0, 0, width,
                height, x, y);
      break;

    case TiledPixmapMenu:
      XSetWindowBackgroundPixmap(dpy, mr->w,
...
void MakeMenu(MenuRoot *mr)
{
  ...
  {
    width =
      XTextWidth(mr->ms->look.pStdFont->font, cur->item, cur->strlen);
    ...
    width =
      XTextWidth(mr->ms->look.pStdFont->font, cur->item2, cur->strlen2);
    if (width < 0)
      width = 0;
    if (width > mr->width2)
      mr->width2 = width;
       ...
       cur_prev = cur, cur = cur->next, cItems++)
  {
      ...
      if ((cur->strlen > 0) || (cur->strlen2 > 0))
      {
        ...
        else
        {
          if ((cur == mr->first) || (cur->next == NULL))
            cur->y_height = mr->ms->look.EntryHeight - HEIGHT_EXTRA +
                            1 + (HEIGHT_EXTRA_TITLE / 2);
          else
            cur->y_height = mr->ms->look.EntryHeight - HEIGHT_EXTRA +
                            1 + HEIGHT_EXTRA_TITLE;
        }
      }
      else
      {
        ...
        cur->next != NULL)
    {
      ...
      menuContinuation =
        NewMenuRoot(szMenuContinuationActionAndName + 6, False);
  ...
  mr->w =
    XCreateWindow(dpy, Scr.Root, 0, 0, (unsigned int)(mr->width),
...
#else
        fvwm_msg(WARN, "scanForPixmap", "Couldn't find pixmap %s",
                 name);
#endif
      break;
    }
               ...
               Bool fPixmapsOk, Bool fNoPlus)
{
  ...
  if (item != (char *)0)
  {
    ...
    if (ch != '\0')
    {
      if (FHotKeyUsedBefore(menu, ch))
      {
        fvwm_msg(
          WARN, "AddToMenu",
          "Hotkey %c is reused in menu %s; second binding ignored.", ch,
          menu->name);
        tmp->hotkey = 0;
      }
      else
  ...
  tmp->state = 0;
  find_func_type(tmp->action, &(tmp->func_type),
                 &(tmp->func_needs_window));
