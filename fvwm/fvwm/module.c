...
void initModules(void)
{
  ...
  PipeMask =
    (unsigned long *)safemalloc(sizeof(unsigned long) * npipes);
  ...
  *Module = 0;
  ...
  if (arg1 == NULL)
  {
    fvwm_msg(ERR, "executeModule",
             "No such module '%s' in ModulePath '%s'", cptr,
             ModulePath);
    free(cptr);
    return;
  }
...
int HandleModuleInput(Window w, int channel)
{
  ...
  if (size > 255)
  {
    fvwm_msg(ERR, "HandleModuleInput", "Module command is too big (%d)",
             size);
    size = 255;
  }
  ...
  if (strlen(text) > 0)
  {
    ...
    if (strncasecmp(text, "UNLOCK", 6) == 0)
...
static unsigned long *make_vpacket(unsigned long *body,
...
void SendPacket(int module, unsigned long event_type,
                unsigned long num_datum, ...)
{
  ...
  PositiveWrite(module, body,
                (num_datum + HEADER_SIZE) * sizeof(body[0]));
}
...
void SendConfig(int module, unsigned long event_type,
                const FvwmWindow *t)
{
...
static unsigned long *make_named_packet(int *len,
                                        unsigned long event_type,
...
void SendName(int module, unsigned long event_type, unsigned long data1,
...
int PositiveWrite(int module, unsigned long *ptr, int size)
{
  ...
  if (PipeMask[module] & M_LOCKONSEND)
  {
    ...
    while (
      (e = read(readPipes[module], &targetWindow, sizeof(Window))) > 0)
    {
                    ...
                    unsigned long context, char *action, int *Module)
{
  ...
  if (*Module >= 0)
  {
    ...
    for (t = Scr.FvwmRoot.next; t != NULL; t = t->next)
    {
      ...
      SendName(*Module, M_WINDOW_NAME, t->w, t->frame, (unsigned long)t,
               t->name);
      SendName(*Module, M_ICON_NAME, t->w, t->frame, (unsigned long)t,
               t->icon_name);

      if (t->icon_bitmap_file != NULL &&
          t->icon_bitmap_file != Scr.DefaultIcon)
        SendName(*Module, M_ICON_FILE, t->w, t->frame, (unsigned long)t,
                 t->icon_bitmap_file);
...
#ifdef MINI_ICONS
      if (t->mini_icon != NULL)
        SendMiniIcon(*Module, M_MINI_ICON, t->w, t->frame,
                     (unsigned long)t, t->mini_icon->width,
                     t->mini_icon->height, t->mini_icon->depth,
                     t->mini_icon->picture, t->mini_icon->mask,
                     t->mini_pixmap_file);
#endif
    }
    ...
    if (Scr.Hilite == NULL)
      ...
      BroadcastPacket(M_FOCUS_CHANGE, 5, Scr.Hilite->w,
                      Scr.Hilite->frame, (unsigned long)Scr.Hilite,
                      Scr.DefaultDecor.HiColors.fore,
                      Scr.DefaultDecor.HiColors.back);
