...
typedef struct ScreenInfo
{
  ...
  */
  ...
  int NoRaiseUnfocused;       /* Don't raise on focus unless clicked */
  int BlockFocusStealing;     /* Ignore auto-focus on new map */
  int FilterSelections;       /* Reserved: filter selections through proxy */
  int ForwardClientMessages;  /* Forward unknown ClientMessage events to client */
  unsigned long SaveYourselfDelayMs; /* Optional WM_SAVE_YOURSELF grace period */
  enum { ExecDirect = 0, ExecViaXpra = 1, ExecViaXephyr = 2 } ExecProxy;
  struct
  {
