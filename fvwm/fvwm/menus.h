...
#define IS_MENU_RETURN(x)                                              \
  ((x) >= MENU_DONE && (x) <= MENU_ABORTED_BUTTON)
#define IS_MENU_BUTTON(x)                                              \
  ((x) == MENU_DONE_BUTTON || (x) == MENU_ABORTED_BUTTON)
#define MENU_ADD_BUTTON(x)                                             \
  ((x) == MENU_DONE || (x) == MENU_ABORTED ? (x) + 1 : (x))
...
void AnimatedMoveOfWindow(Window w, int startX, int startY, int endX,
                          int endY, Bool fWarpPointerToo, int cusDelay,
                          float *ppctMovement);
