    ...
    unsigned long context, char *action, int *Module)
{
	...
	while (next_desk != INT_MAX) {
		...
		if ((flags & SHOW_ALLDESKS) && !(flags & NO_DESK_SORT)) {
			/* run through the windowlist finding the first desk not
			 * already processed */
			next_desk = INT_MAX;
			for (ii = 0; ii < numWindows; ii++) {
				t = windowList[ii];
		...
		for (ii = 0; ii < numWindows; ii++) {
			...
			if (((t->Desk == next_desk) ||
				...
				if (flags & SHOW_GEOMETRY) {
					...
					snprintf(
					    loc, sizeof(loc), "%d", dwidth);
					strlcat(tname, loc, sizeof(tname));
					snprintf(
					    loc, sizeof(loc), "x%d", dheight);
					strlcat(tname, loc, sizeof(tname));
					if (t->frame_x >= 0)
					...
					if (t->flags & STICKY)
						strlcat(
						    tname, " S", sizeof(tname));
					if (t->flags & ONTOP)
						strlcat(
						    tname, " T", sizeof(tname));
					if (t->flags & ICONIFIED)
						strlcat(
						    tname, ")", sizeof(tname));
