...
int main(int argc, char **argv)
{
  ...
  for (i = 1; i < argc; i++)
  {
	...
	buflen = 6+strlen(argv[i]);
        config_commands[num_config_commands] = (char *)safemalloc(buflen);
...
/* unused */
  ...
  InternUsefulAtoms ();
  /* Initialize and advertise EWMH support before selecting on root. */
  EWMH_Init();
  ...
  /* On OpenBSD, tighten process promises and filesystem view. */
  (void)fvwm_unveil(FVWM_MODULEDIR, "r");
  /* Common system binaries to allow Exec; users can extend via PATH. */
  (void)fvwm_unveil("/bin", "rx");
  (void)fvwm_unveil("/usr/bin", "rx");
  /* Allow reading user configuration by default. */
  (void)fvwm_unveil(getenv("HOME"), "r");
  (void)fvwm_unveil_lock();
  if (fvwm_pledge("stdio rpath proc exec", NULL) == -1)
    err(1, "pledge");
...
void StartupStuff(void)
{
  ...
  CaptureAllWindows();
  /* After initial capture, refresh client lists for EWMH. */
...
void InitVariables(void)
{
  ...
  /* Default policy: reduce focus stealing and avoid unintended raising */
  Scr.NoRaiseUnfocused = 1;
  Scr.BlockFocusStealing = 1;
  Scr.FilterSelections = 0;
  Scr.ForwardClientMessages = 1; /* Historical default behavior */
  Scr.SaveYourselfDelayMs = 0;
  Scr.ExecProxy = ExecDirect;
  {
    const char *fcm_env = getenv("FVWM_FORWARD_CLIENT_MESSAGES");
    if (fcm_env && *fcm_env)
    {
      if (fcm_env[0] == '0' || fcm_env[0] == 'n' || fcm_env[0] == 'N')
        Scr.ForwardClientMessages = 0;
      else if (fcm_env[0] == '1' || fcm_env[0] == 'y' || fcm_env[0] == 'Y')
        Scr.ForwardClientMessages = 1;
    }
    {
      const char *sy_env = getenv("FVWM_SAVEYOURSELF_DELAY_MS");
      if (sy_env && *sy_env)
      {
        char *endp = NULL;
        unsigned long v = strtoul(sy_env, &endp, 10);
        if (endp && endp != sy_env)
...
void Done(int restart, char *command)
{
  ...
  /* Ask clients to save state before shutdown/restart (ICCCCM WM_SAVE_YOURSELF). */
  {
    FvwmWindow *t;
    for (t = Scr.FvwmRoot.next; t != NULL; t = t->next)
    {
      if (t->w && _XA_WM_SAVE_YOURSELF)
      {
        send_clientmessage(dpy, t->w, _XA_WM_SAVE_YOURSELF, CurrentTime);
      }
    }
    XSync(dpy, 0);
  }

  /* Optional small delay to allow clients to process SaveYourself.
   * Configure via environment variable FVWM_SAVEYOURSELF_DELAY_MS (default: 0).
   */
  if (Scr.SaveYourselfDelayMs > 0)
  {
    /* sleep in 10ms slices to remain interruptible */
    unsigned long left = Scr.SaveYourselfDelayMs;
    while (left > 0)
    {
      unsigned long chunk = (left > 10) ? 10 : left;
      usleep(chunk * 1000);
      left -= chunk;
    }
  }
