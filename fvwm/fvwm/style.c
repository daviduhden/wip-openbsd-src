	...
	while ((*restofline != 0) && (*restofline != '\n')) {
		...
		case 'i':
			if (ITIS("IconTitle")) {
				...
				IconBoxes =
				    (icon_boxes *)xmalloc(sizeof(icon_boxes));
				memset(IconBoxes, 0,
				...
				if (num == 4) { /* if 4 numbers */
					...
					restofline =
					    tmp; /* got word, move past it */
				} /* end not 4 args */
				/* If we created an IconBox, put it in the
				 * chain. */
				if (IconBoxes != 0) { /* If no error */
					if (tname.IconBoxes ==
					    0) { /* If first one */
						tname.IconBoxes =
						    IconBoxes; /* chain to root
								*/
					} else { /* else not first one */
						which->next =
						    IconBoxes; /* add to end of
								  chain */
					} /* end not first one */
					which = IconBoxes; /* new current box.
							      save for grid */
				} /* end no error */
			} /* end iconbox parameter */ else if (ITIS("ICONGRI"
								    "D")) {
				SKIP("ICONGRID");
				...
				if (which == 0) { /* If no current box */
					fvwm_msg(ERR, "ProcessNewStyle",
					    "IconGrid must follow an IconBox "
					...
					if (num != 2 ||
					    ...
					    which->IconGrid[1] < 1) {
						fvwm_msg(ERR, "ProcessNewStyle",
						    "IconGrid needs 2 numbers "
						    "> 0. Got %d numbers. "
						    "x=%d y=%d!",
						    num,
						    (int)which->IconGrid[0],
			...
			} else if (ITIS("ICONFILL")) { /* direction to fill
							  iconbox */
				SKIP("ICONFILL");
				SKIPSPACE; /* skip whitespace after keyword */
				...
				if (which == 0) { /* If no current box */
					...
					unsigned char IconFill_1; /* first  type
							   direction parsed */
					unsigned char IconFill_2; /* second type
							   direction parsed */
					GETWORD; /* read in word for length */
					if (Get_TBLR(restofline, &IconFill_1) ==
					    0) { /* top/bot/lft/rgt */
						fvwm_msg(ERR, "ProcessNewStyle",
						    "IconFill must be followed "
						    "by T|B|R|L, found %.*s.",
						    len,
		...
		case 's':
			 ...
			 */
				...
				SKIP("STARTSONPAGE");
				tname.off_flags |= STARTSONDESK_FLAG;
				spargs = sscanf(restofline, "%d %d %d", &tmpno1,
				    &tmpno2, &tmpno3);
		...
		case 'u':
			...
			if (ITIS("UseDecor")) {
				...
				if (len > 0) {
					int hit = 0;
					/* changed to accum multiple Style
					 * definitions (veliaa@rpi.edu) */
...
static int
Get_TBLR(char *restofline, unsigned char *IconFill)
{
	*IconFill = 0; /* init */
