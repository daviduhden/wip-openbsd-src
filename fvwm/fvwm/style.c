...
#define ITIS(THIS) strncasecmp(restofline, THIS, sizeof(THIS) - 1) == 0
  ...
  while ((*restofline != 0) && (*restofline != '\n'))
  {
    ...
    case 'i':
      ...
      else if (ITIS("IconBox"))
      {
        ...
        num = sscanf(restofline, "%d%d%d%d", &IconBoxes->IconBox[0],
                     &IconBoxes->IconBox[1], &IconBoxes->IconBox[2],
                     &IconBoxes->IconBox[3]);
        if (num == 4)
        { /* if 4 numbers */
          for (i = 0; i < num; i++)
        ...
        { /* Not 4 numeric args dje */
          char
            geom_string[25]; /* bigger than =32767x32767+32767+32767 */
          int geom_flags;
          GETWORD; /* read in 1 word w/o advancing */
          if (len > 0 && len < 24)
          { /* if word found, not too long */
            strncpy(geom_string, restofline,
                    len);         /* copy and null term */
            geom_string[len] = 0; /* null terminate it */
            geom_flags =
              XParseGeometry(geom_string, &IconBoxes->IconBox[0],
            ...
            { /* zero width ind invalid */
              fvwm_msg(ERR, "ProcessNewStyle",
                       "IconBox requires 4 numbers or geometry! "
                       "Invalid string <%s>.",
                       geom_string);
      ...
      else if (ITIS("ICONGRID"))
      {
        ...
        {                                    /* have a place to grid */
          num = sscanf(restofline, "%hd%hd", /* 2 shorts */
                       &which->IconGrid[0], &which->IconGrid[1]);
          if (num != 2 || which->IconGrid[0] < 1 ||
              which->IconGrid[1] < 1)
          {
            fvwm_msg(ERR, "ProcessNewStyle",
                     "IconGrid needs 2 numbers > 0. Got %d numbers. "
                     "x=%d y=%d!",
                     num, (int)which->IconGrid[0],
                     (int)which->IconGrid[1]);
      ...
      { /* direction to fill iconbox */
        ...
        {                           /* have a place to fill */
          ...
          if (Get_TBLR(restofline, &IconFill_1) == 0)
          { /* top/bot/lft/rgt */
            fvwm_msg(
              ERR, "ProcessNewStyle",
              "IconFill must be followed by T|B|R|L, found %.*s.", len,
              restofline); /* its wrong */
          }
          else
          {                   /* first word valid */
            ...
            if (Get_TBLR(restofline, &IconFill_2) == 0)
            { /* top/bot/lft/rgt */
              fvwm_msg(
                ERR, "ProcessNewStyle",
            ...
            else if ((IconFill_1 & ICONFILLHRZ) ==
                     (IconFill_2 & ICONFILLHRZ))
            {
              fvwm_msg(ERR, "ProcessNewStyle",
                       "IconFill must specify a horizontal and "
                       "vertical direction.");
            }
            else
            {                                 /* Its valid! */
    ...
    case 's':
      ...
      else if (ITIS("STARTSONDESK"))
      {
        ...
        {
          tname.off_flags &= ~STARTSONDESK_FLAG;
          fvwm_msg(ERR, "ProcessNewStyle", "bad StartsOnDesk arg: %s",
                   restofline);
        }
        /**/
        GETWORD;
      ...
      else if (ITIS("STARTSONPAGE"))
      {
        ...
        tname.off_flags |= STARTSONDESK_FLAG;
        spargs =
          sscanf(restofline, "%d %d %d", &tmpno1, &tmpno2, &tmpno3);
        ...
        {
          tname.off_flags &= ~STARTSONDESK_FLAG;
          fvwm_msg(ERR, "ProcessNewStyle", "bad StartsOnPage args: %s",
                   restofline);
        }
      }
      /**/
    ...
    case 'u':
      ...
      else if (ITIS("UseStyle"))
      {
        ...
        if (len > 0)
        {
          ...
          {
            if (!strncasecmp(restofline, nptr->name, len))
            { /* match style */
              if (!hit)
              { /* first match */
                char *save_name;
                save_name = tname.name;
                memcpy((void *)&tname, (const void *)nptr,
                       sizeof(name_list)); /* copy everything */
              ...
              { /* subsequent match */
                ...
                if (nptr->value)
                  tname.value = nptr->value;
#ifdef MINI_ICONS
                if (nptr->mini_value)
                  tname.mini_value = nptr->mini_value;
#endif
#ifdef USEDECOR
                if (nptr->Decor)
                  tname.Decor = nptr->Decor;
#endif
    ...
    {
      fvwm_msg(ERR, "ProcessNewStyle", "bad style command: %s",
               restofline);
...
static int Get_TBLR(char *restofline, unsigned char *IconFill)
{
  *IconFill = 0; /* init */
  ...
  else if (!(ITIS("L") || ITIS("LFT") || ITIS("LEFT")))
  {           /* "left" */
