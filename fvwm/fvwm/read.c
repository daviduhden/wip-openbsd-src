...
static int start_pipe_process(const char *command, PipeChild *child)
{
  int pipe_fd[2];
  pid_t pid;
...
static void stop_pipe_process(PipeChild *child, int timed_out,
  ...
  for (attempt = 0; attempt < PIPE_REAP_ATTEMPTS; ++attempt)
  {
    ...
    if (waited == -1)
    {
      if (errno == EINTR)
        continue;
      if (errno != ECHILD)
        fvwm_msg(ERR, cmdname, "waitpid failed for '%s': %s", command,
                 strerror(errno));
      break;
    }
    if (waited == 0)
 ...
 */
static void ReadSubFunc(XEvent *eventp, Window junk,
                        FvwmWindow *tmp_win, unsigned long context,
                        char *action, int *Module, int piperead)
{
  ...
  { /* if there is a second arg */
    if (strncasecmp(option, "Quiet", 5) == 0)
    {                      /* is the arg "quiet"? */
    ...
    if (missing_quiet == 'n')
    {
      if (piperead)
        ...
        fvwm_msg(ERR, cmdname,
                 "file '%s' not found in $HOME or " FVWM_CONFIGDIR,
                 ofilename);
    }
              ...
              unsigned long context, char *action, int *Module)
{
  ...
  if (last_read_failed && this_read == 0)
  {
    ...
    ExecuteFunction((char *)read_system_rc_cmd, NULL, &Event, C_ROOT,
                    -1);
  }
