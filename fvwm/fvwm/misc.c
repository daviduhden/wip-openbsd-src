...
void Destroy(FvwmWindow *Tmp_win)
{
  ...
  */
  ...
  /* EWMH: refresh lists and active window after destruction */
  EWMH_UpdateClientList();
  EWMH_UpdateClientListStacking();
  EWMH_UpdateActiveWindow();
  /* EWMH: re-evaluate desktop count/names/workarea */
  EWMH_RecalcDesktops();
...
Bool StashEventTime (XEvent *ev)
{
  ...
  switch (ev->type)
    ...
    case PropertyNotify:
      ...
      break;
    case SelectionClear:
      if (Scr.FilterSelections && Scr.ExecProxy == ExecViaXephyr)
        return False; /* ignore selection clears from host display */
      NewTimestamp = ev->xselectionclear.time;
      break;
    case SelectionRequest:
      if (Scr.FilterSelections && Scr.ExecProxy == ExecViaXephyr)
        return False; /* ignore selection requests from host display */
...
{
  if (Scr.NoRaiseUnfocused && Scr.Focus != t)
    return;
  FvwmWindow *t2;
  int count, i;
  Window *wins;
