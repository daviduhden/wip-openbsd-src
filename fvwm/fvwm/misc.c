 ...
 **************************************************************************/
void
free_window_names(FvwmWindow *tmp, Bool nukename, Bool nukeicon)
{
	if (!tmp)
		return;
	...
	if (Tmp_win->flags & TITLE) {
		...
		for (i = 0; i < Scr.nr_right_buttons; i++)
			if (Tmp_win->right_w[i] != None)
				XDeleteContext(
				    dpy, Tmp_win->right_w[i], FvwmContext);
	}
...
RestoreWithdrawnLocation(FvwmWindow *tmp, Bool restart)
{
	...
	if (XGetGeometry(dpy, tmp->w, &JunkRoot, &xwc.x, &xwc.y, &JunkWidth,
		 ...
		 * Although this is normally the case, it is not always true.
		 * The most common example is when the user does something in an
		 * application which will, after some amount of computational
		 * delay, cause the window to be unmapped, but then switches
		 * screens before this happens.  The XTranslateCoordinates call
		 * above will set the window coordinates to either be larger
		 * than the screen, or negative. This will result in the window
		 * being placed in odd, or even unviewable locations when the
		 * window is remapped.  The followin code forces the "relative"
		 * location to be within the bounds of the display.
		 *
		 * gpw -- 11/11/93
		 *
		 ...
		 *
		 * Also, fixed so that it only does this stuff if a window is
		 * more than half off the screen. (RN)
		 */
		...
		if (!restart) {
			...
			if ((tmp->frame_x < 0) || (tmp->frame_y < 0) ||
				...
				if ((xwc.x < -w2) ||
				    (xwc.x > (Scr.MyDisplayWidth - w2))) {
					xwc.x = xwc.x % Scr.MyDisplayWidth;
					if (xwc.x < -w2)
						xwc.x += Scr.MyDisplayWidth;
				}
				if ((xwc.y < -h2) ||
				    (xwc.y > (Scr.MyDisplayHeight - h2))) {
 ...
 */
int
GetMoveArguments(char *action, int x, int y, int w, int h, int *pFinalX,
	...
	while (action != NULL) {
		...
		if (StrEquals(tok, "context")) {
			...
			if (sscanf(&(tok[6]), "%d", &button) != 1 ||
				...
				if (button & 0x01)
					context_window =
					    tmp_win->left_w[button / 2];
				else
					context_window =
					    tmp_win->right_w[button / 2 - 1];
			}
		...
		taction = GetOneMenuPositionArgument(naction, x, width,
		    &(pops->pos_hints.x), &(pops->pos_hints.x_factor));
		naction = GetOneMenuPositionArgument(taction, y, height,
		    &(pops->pos_hints.y), &(pops->pos_hints.y_factor));
		if (naction == taction) {
...
RaiseWindow(FvwmWindow *t)
{
	...
	if (i > 0) {
		...
		if (wins[0] != Scr.FvwmRoot.stack_next->frame &&
			...
			XConfigureWindow(
			    dpy, wins[0], (CWSibling | CWStackMode), &changes);
		}

		/*
		      RBW  - 01/05/1998 - move all raised windows to front of
		   stacking order chain.
		  */
		j = 0;
		t2 = &Scr.FvwmRoot;
		while (j < count2) {
			...
			if (t1 != t2->stack_next) {
				...
				t1->stack_next =
				    t2->stack_next; /* Set new pointers. */
