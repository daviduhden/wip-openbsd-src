    ...
    case XA_WM_TRANSIENT_FOR:
      {
        if(XGetTransientForHint(dpy, Tmp_win->w, &Tmp_win->transientfor))
        {
           ...
           RaiseWindow(Tmp_win);
           /* Re-evaluate window type for EWMH */
           EWMH_SetWindowType(Tmp_win);
        }
        else
        {
           Tmp_win->flags &= ~TRANSIENT;
      ...
      break;
    default:
      if (Event.xproperty.atom == WM_WINDOW_ROLE)
      {
        /* Role changed; refresh type classification */
        EWMH_SetWindowType(Tmp_win);
      }
      break;
...
void HandleClientMessage()
{
  ...
  */
  if (Tmp_win)
  {
    /* Let EWMH consume known messages first */
    if (EWMH_HandleClientMessage(Tmp_win, &Event.xclient))
      return;
    if (Scr.ForwardClientMessages)
    {
...
void HandleButtonPress()
{
       ...
       (ControlMask|Mod1Mask|Mod2Mask|Mod3Mask|Mod4Mask|Mod5Mask)) == 0))
  {
    SetFocus(Tmp_win->w,Tmp_win,1);
/* #ifdef CLICKY_MODE_1 */
    if ((Scr.ClickToFocusRaises && !Scr.NoRaiseUnfocused) ||
...
/* #endif */
    {
      if (!Scr.NoRaiseUnfocused || (Tmp_win && Tmp_win == Scr.Focus))
        RaiseWindow(Tmp_win);
    }
