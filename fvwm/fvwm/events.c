...
#ifdef SHAPE
	if (ShapesSupported)
		EventHandlerJumpTable[ShapeEventBase + ShapeNotify] =
		    HandleShapeNotify;
#endif /* SHAPE */
}
 ...
 ************************************************************************/
int
GetContext(FvwmWindow *t, XEvent *e, Window *w)
{
	int Context, i;
	...
	if (e->xkey.subwindow != None) {
		if (e->type == KeyPress)
			*w = e->xkey.subwindow;
			...
			/* domivogt (6-Jan-198): I don't understand what's
			 * happening here. If the mouse is over the client
			 * window. The subwindow has an unique id that no
			 * visible part of the FvwmWindow has. */
		...
		if (*w == t->title_w)
			...
			if (i < 4)
				...
				if (i < Scr.nr_left_buttons)
					...
					for (i = 0; i < Scr.nr_right_buttons;
					    i++) {
						if (*w == t->right_w[i]) {
							Context =
							    (1 << i) * C_R1;
...
HandleFocusIn()
{
	...
	if (!Tmp_win) {
		...
		BroadcastPacket(M_FOCUS_CHANGE, 5, Tmp_win->w, Tmp_win->frame,
		    (unsigned long)Tmp_win, GetDecor(Tmp_win, HiColors.fore),
...
HandleKeyPress()
{
	...
	if (Tmp_win) {
		if (Event.xkey.window != Tmp_win->w) {
			...
			XSendEvent(
			    dpy, Tmp_win->w, False, KeyPressMask, &Event);
		}
	}
	...
	case XA_WM_HINTS:
		...
		if ((Tmp_win->wmhints->flags & IconPixmapHint) ||
			...
			if (!(Tmp_win->flags & SUPPRESSICON)) {
				...
				XDeleteContext(
				    dpy, Tmp_win->icon_w, FvwmContext);
			...
			if (Tmp_win->flags & ICONIFIED) {
				...
				CreateIconWindow(Tmp_win, Tmp_win->icon_x_loc,
				    Tmp_win->icon_y_loc);
				BroadcastPacket(M_ICONIFY, 7, Tmp_win->w,
...
HandleClientMessage()
{
	...
	if ((Event.xclient.message_type == _XA_WM_CHANGE_STATE) && (Tmp_win) &&
	    (Event.xclient.data.l[0] == IconicState) &&
	    !(Tmp_win->flags & ICONIFIED)) {
		XQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild,
		    &(button.xmotion.x_root), &(button.xmotion.y_root), &JunkX,
		    &JunkY, &JunkMask);
		button.type = 0;
...
HandleMapNotify()
{
	...
	*/
	...
	if (Tmp_win->flags & ICONIFIED)
		BroadcastPacket(M_DEICONIFY, 3, Tmp_win->w, Tmp_win->frame,
		    (unsigned long)Tmp_win);
	else
		BroadcastPacket(M_MAP, 3, Tmp_win->w, Tmp_win->frame,
		    (unsigned long)Tmp_win);
...
HandleButtonPress()
{
	...
	if (Context == C_TITLE)
		...
		SetBorder(
		    Tmp_win, (Scr.Hilite == Tmp_win), True, True, PressedW);
	...
	for (MouseEntry = Scr.AllBindings; MouseEntry != NULL;
		...
		if (((MouseEntry->Button_Key == Event.xbutton.button) ||
			...
			ExecuteFunction(
			    MouseEntry->Action, Tmp_win, &Event, Context, -1);
			break;
		}
	}
	PressedW = None;
...
HandleEnterNotify()
{
	...
	if (XCheckTypedWindowEvent(dpy, ewp->window, LeaveNotify, &d)) {
		/*
		     RBW - if we're in startup, this is a coerced focus, so we
		   don't want to save the event time, or exit prematurely.
		  */
		if (!fFvwmInStartup) {
			StashEventTime(&d);
...
/* an EnterEvent in one of the PanFrameWindows activates the Paging */
	...
	if (ewp->window == Scr.PanFrameTop.win ||
		...
		HandlePaging(Scr.EdgeScrollX, Scr.EdgeScrollY,
		    &Event.xcrossing.x_root, &Event.xcrossing.y_root, &delta_x,
		    &delta_y, True);
		return;
	}
#endif /* NON_VIRTUAL */
	...
	if (!Tmp_win || cre->window == Tmp_win->icon_w ||
		...
		if ((Tmp_win) && ((Tmp_win->icon_pixmap_w == cre->window))) {
			...
			if (!(Tmp_win->flags & ICON_UNMAPPED))
				BroadcastPacket(M_ICON_LOCATION, 7, Tmp_win->w,
				    Tmp_win->frame, (unsigned long)Tmp_win,
				    Tmp_win->icon_x_loc, Tmp_win->icon_y_loc,
				    Tmp_win->icon_w_width,
				    Tmp_win->icon_w_height +
		...
		if (Tmp_win) {
			if (cre->window != Tmp_win->icon_pixmap_w &&
				...
				XConfigureWindow(
				    dpy, Tmp_win->icon_pixmap_w, xwcm, &xwc);
			}
		...
		if (xwc.stack_mode == Above || xwc.stack_mode == Below) {
			...
			if (xwc.stack_mode == Below) {
				/*
				    ...
				    */
				FvwmSib = (FvwmSib == otherwin) ?
				    FvwmSib->stack_next :
				    FvwmSib->stack_prev;
			}
			if (Tmp_win !=
			    FvwmSib) { /* Don't chain it to itself!  */
				Tmp_win->stack_prev->stack_next =
				    Tmp_win->stack_next; /* Pluck from chain. */
				Tmp_win->stack_next->stack_prev =
				    Tmp_win->stack_prev;
				Tmp_win->stack_next =
				    FvwmSib; /* Set new pointers.   */
				Tmp_win->stack_prev = FvwmSib->stack_prev;
				FvwmSib->stack_prev->stack_next =
				    Tmp_win; /* Re-insert above sibling. */
				FvwmSib->stack_prev = Tmp_win;
			}
		} else {
			/*
			      Oh, bother! We have to rebuild the stacking order
			   ring to figure out where this one went (TopIf,
...
HandleVisibilityNotify()
{
	...
	if (Tmp_win && Tmp_win->frame == last_event_window) {
		...
		/* For the most part, we'll raised partially obscured ONTOP
		 * windows here. The exception is ONTOP windows that are
		 * obscured by other ONTOP windows, which are raised in
...
My_XNextEvent(Display *dpy, XEvent *event)
{
	...
	if (XPending(dpy)) {
		DBUG("My_XNextEvent",
		    "taking care of queued up events & returning");
...
ResyncFvwmStackRing(void)
{
	...
	for (i = 0; i < nchildren; i++) {
		...
		if (t1 != NULL && t1 != t2) {
			...
			t2->stack_prev->stack_next =
			    t1; /* Insert in new position. */
			t2->stack_prev = t1;
			t2 = t1;
		}
	}
