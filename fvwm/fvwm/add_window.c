...
AddWindow(Window w)
{
	...
	SelectDecor(
	    tmp_win, styles.on_flags, styles.border_width, styles.resize_width);
	...
	if (XGetCommand(dpy, tmp_win->w, &client_argv, &client_argc)) {
		...
		status = XrmGetResource(
		    db, "fvwm.page", "Fvwm.Page", &str_type, &rm_value);
		if ((status == True) && (rm_value.size != 0)) {
			spargs = sscanf(rm_value.addr, "%d %d %d", &tmpno1,
			    &tmpno2, &tmpno3);
			switch (spargs) {
			case 1: {
				styles.on_flags |= STARTSONDESK_FLAG;
				styles.Desk =
				    (tmpno1 > -1) ? tmpno1 + 1 : tmpno1;
				break;
			}
			case 2: {
				styles.on_flags |= STARTSONDESK_FLAG;
	...
	*/
	...
	tmp_win->frame = XCreateWindow(dpy, Scr.Root, tmp_win->frame_x,
	    ...
	    tmp_win->bw, CopyFromParent, InputOutput, CopyFromParent, valuemask,
	    &attributes);
	...
	if (tmp_win->flags & BORDER) {
		...
		for (i = 0; i < 4; i++) {
			...
			tmp_win->corners[i] = XCreateWindow(dpy, tmp_win->frame,
			    0, 0, tmp_win->corner_width, tmp_win->corner_width,
	...
	if (tmp_win->flags & TITLE) {
		...
		tmp_win->title_w = XCreateWindow(dpy, tmp_win->frame,
		    ...
		    tmp_win->title_height, 0, CopyFromParent, InputOutput,
		...
		for (i = 4; i >= 0; i--) {
			if ((i < Scr.nr_left_buttons) &&
				...
				tmp_win->left_w[i] = XCreateWindow(dpy,
				    tmp_win->frame, tmp_win->title_height * i,
	...
	if (tmp_win->flags & BORDER) {
		...
		for (i = 0; i < 4; i++) {
			...
			tmp_win->sides[i] = XCreateWindow(dpy, tmp_win->frame,
			    0, 0, tmp_win->boundary_width,
	...
	SetupFrame(
	    tmp_win, tmp_win->frame_x, tmp_win->frame_y, width, height, True);
	...
	if (tmp_win->flags & TITLE) {
		XSaveContext(
		    dpy, tmp_win->title_w, FvwmContext, (caddr_t)tmp_win);
		for (i = 0; i < Scr.nr_left_buttons; i++)
			XSaveContext(dpy, tmp_win->left_w[i], FvwmContext,
			    (caddr_t)tmp_win);
		for (i = 0; i < Scr.nr_right_buttons; i++)
			if (tmp_win->right_w[i] != None)
				XSaveContext(dpy, tmp_win->right_w[i],
	...
	XTranslateCoordinates(
	    dpy, tmp_win->frame, Scr.Root, JunkX, JunkY, &a, &b, &JunkChild);
	tmp_win->xdiff -= a;
	tmp_win->ydiff -= b;
	if ((tmp_win->flags & ClickToFocus) || Scr.MouseFocusClickRaises) {
		...
		for (i = 1; i <= 3; i++)
			if (Scr.buttons2grab & (1 << i)) {
				 ...
				 */
				/* domivogt (2-Jan-1999): No. Or at least not
				 * like this. In the present form no button
				 * presses go through to the title bar anymore.
				 * They are all swallowed by the frame window.
				 */
				XGrabButton(dpy, (i), AnyModifier,
				    tmp_win->frame, True, ButtonPressMask,
	...
	if (tmp_win->mini_icon != NULL)
		BroadcastMiniIcon(M_MINI_ICON, tmp_win->w, tmp_win->frame,
		    ...
		    tmp_win->mini_icon->picture, tmp_win->mini_icon->mask,
 ...
 ***********************************************************************/
void
GrabButtons(FvwmWindow *tmp_win)
{
	Binding *MouseEntry;
		...
		if ((MouseEntry->Action != NULL) &&
			...
			if (MouseEntry->Button_Key > 0) {
				XGrabButton(dpy, MouseEntry->Button_Key,
				    MouseEntry->Modifier, tmp_win->w, True,
		...
		if ((tmp->Context &
			...
			if (tmp->Modifier != AnyModifier) {
				XGrabKey(dpy, tmp->Button_Key,
				    tmp->Modifier | LockMask, tmp_win->frame,
 ...
 ***********************************************************************/
void
GetWindowSizeHints(FvwmWindow *tmp)
{
	long supplied = 0;
		...
		*/
		if ((minAspectX * maxAspectY) > (maxAspectX * minAspectY)) {
			...
			fvwm_msg(WARN, "GetWindowSizeHints",
			    ...
			    "ratio -> ignoring, but program displaying this "
			    "window "
			    "should be fixed!!!!",
			    tmp->w);
		}
 ...
 ***********************************************************************/
void
LookInList(FvwmWindow *tmp_win, name_list *styles)
{
	name_list *nptr;
