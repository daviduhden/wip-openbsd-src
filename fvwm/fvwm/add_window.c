...
FvwmWindow *AddWindow(Window w)
{
  ...
  tmp_win->IconBoxes =
    styles.IconBoxes; /* copy iconboxes ptr (if any) */
  tmp_win->buttons =
    styles.on_buttons; /* on and off buttons combined. */
#ifdef USEDECOR
  ...
  SelectDecor(tmp_win, styles.on_flags, styles.border_width,
              styles.resize_width);
  ...
  else if ((tmp_win->wmhints) && (tmp_win->wmhints->flags &
  ...
  if (XGetCommand(dpy, tmp_win->w, &client_argv, &client_argc))
  {
    ...
    status = XrmGetResource(db, "fvwm.desk", "Fvwm.Desk", &str_type,
                            &rm_value);
    ...
    {
      spargs =
        sscanf(rm_value.addr, "%d %d %d", &tmpno1, &tmpno2, &tmpno3);
      switch (spargs)
      {
      case 1:
      {
  ...
  if (styles.ForeColor != NULL)
  {
    ...
    if ((XParseColor(dpy, Scr.FvwmRoot.attr.colormap, styles.ForeColor,
                     &color)) &&
  ...
  if (styles.BackColor != NULL)
  {
    ...
    if ((XParseColor(dpy, Scr.FvwmRoot.attr.colormap, styles.BackColor,
                     &color)) &&
        (XAllocColor(dpy, Scr.FvwmRoot.attr.colormap, &color)))
  ...
  */
  ...
  tmp_win->title_w = 0;
  tmp_win->title_width =
    tmp_win->frame_width - 2 * tmp_win->corner_width - 3 + tmp_win->bw;
  ...
  if (tmp_win->flags & BORDER)
  {
    ...
    for (i = 0; i < 4; i++)
    {
      ...
      tmp_win->corners[i] = XCreateWindow(
        ...
        tmp_win->corner_width, 0, CopyFromParent, InputOutput,
        CopyFromParent, valuemask, &attributes);
    }
#if defined(PIXMAP_BUTTONS) && defined(BORDERSTYLE)
    if (TexturePixmap)
  ...
  {
    tmp_win->title_x =
      tmp_win->boundary_width + tmp_win->title_height + 1;
    tmp_win->title_y = tmp_win->boundary_width;
    attributes.cursor = Scr.FvwmCursors[TITLE_CURSOR];
    tmp_win->title_w = XCreateWindow(
      dpy, tmp_win->frame, tmp_win->title_x, tmp_win->title_y,
      tmp_win->title_width, tmp_win->title_height, 0, CopyFromParent,
...
#endif
        tmp_win->left_w[i] =
          XCreateWindow(dpy, tmp_win->frame, tmp_win->title_height * i,
                        0, tmp_win->title_height, tmp_win->title_height,
...
#endif
        tmp_win->right_w[i] = XCreateWindow(
          dpy, tmp_win->frame,
          tmp_win->title_width - tmp_win->title_height * (i + 1), 0,
  ...
  if (tmp_win->flags & BORDER)
  {
    ...
    for (i = 0; i < 4; i++)
    {
      ...
      tmp_win->sides[i] = XCreateWindow(
        dpy, tmp_win->frame, 0, 0, tmp_win->boundary_width,
        tmp_win->boundary_width, 0, CopyFromParent, InputOutput,
        CopyFromParent, valuemask, &attributes);
    }
#if defined(PIXMAP_BUTTONS) && defined(BORDERSTYLE)
    if (TexturePixmap)
  ...
  if (tmp_win->flags & TITLE)
  {
    ...
    for (i = 0; i < Scr.nr_left_buttons; i++)
      XSaveContext(dpy, tmp_win->left_w[i], FvwmContext,
                   (caddr_t)tmp_win);
  ...
  if (tmp_win->flags & BORDER)
  {
    for (i = 0; i < 4; i++)
    {
      XSaveContext(dpy, tmp_win->sides[i], FvwmContext,
                   (caddr_t)tmp_win);
      XSaveContext(dpy, tmp_win->corners[i], FvwmContext,
                   (caddr_t)tmp_win);
    }
  }
  RaiseWindow(tmp_win);
  ...
  XGetGeometry(dpy, tmp_win->w, &JunkRoot, &JunkX, &JunkY, &JunkWidth,
               &JunkHeight, &JunkBW, &JunkDepth);
  XTranslateCoordinates(dpy, tmp_win->frame, Scr.Root, JunkX, JunkY, &a,
                        &b, &JunkChild);
  tmp_win->xdiff -= a;
  tmp_win->ydiff -= b;
  ...
  if (tmp_win->mini_icon != NULL)
    BroadcastMiniIcon(
      M_MINI_ICON, tmp_win->w, tmp_win->frame, (unsigned long)tmp_win,
      tmp_win->mini_icon->width, tmp_win->mini_icon->height,
      tmp_win->mini_icon->depth, tmp_win->mini_icon->picture,
      tmp_win->mini_icon->mask, tmp_win->mini_pixmap_file);
#endif
...
void GrabButtons(FvwmWindow *tmp_win)
{
  ...
  while (MouseEntry != (Binding *)0)
  {
    if ((MouseEntry->Action != NULL) &&
        (MouseEntry->Context & C_WINDOW) && (MouseEntry->IsMouse == 1))
    {
      if (MouseEntry->Button_Key > 0)
      {
        XGrabButton(dpy, MouseEntry->Button_Key, MouseEntry->Modifier,
                    ...
                    GrabModeAsync, None, Scr.FvwmCursors[DEFAULT]);
        if (MouseEntry->Modifier != AnyModifier)
        {
          XGrabButton(dpy, MouseEntry->Button_Key,
                      (MouseEntry->Modifier | LockMask), tmp_win->w,
      ...
      {
        XGrabButton(dpy, 1, MouseEntry->Modifier, tmp_win->w, True,
                    ButtonPressMask | ButtonReleaseMask, GrabModeAsync,
                    GrabModeAsync, None, Scr.FvwmCursors[DEFAULT]);
        XGrabButton(dpy, 2, MouseEntry->Modifier, tmp_win->w, True,
                    ButtonPressMask | ButtonReleaseMask, GrabModeAsync,
                    GrabModeAsync, None, Scr.FvwmCursors[DEFAULT]);
        XGrabButton(dpy, 3, MouseEntry->Modifier, tmp_win->w, True,
                    ButtonPressMask | ButtonReleaseMask, GrabModeAsync,
                    GrabModeAsync, None, Scr.FvwmCursors[DEFAULT]);
        if (MouseEntry->Modifier != AnyModifier)
        {
          XGrabButton(
            dpy, 1, (MouseEntry->Modifier | LockMask), tmp_win->w, True,
            ButtonPressMask | ButtonReleaseMask, GrabModeAsync,
            GrabModeAsync, None, Scr.FvwmCursors[DEFAULT]);
          XGrabButton(
            dpy, 2, (MouseEntry->Modifier | LockMask), tmp_win->w, True,
            ButtonPressMask | ButtonReleaseMask, GrabModeAsync,
            GrabModeAsync, None, Scr.FvwmCursors[DEFAULT]);
          XGrabButton(
            dpy, 3, (MouseEntry->Modifier | LockMask), tmp_win->w, True,
            ButtonPressMask | ButtonReleaseMask, GrabModeAsync,
            GrabModeAsync, None, Scr.FvwmCursors[DEFAULT]);
        }
      }
...
void GrabKeys(FvwmWindow *tmp_win)
{
  ...
  for (tmp = Scr.AllBindings; tmp != NULL; tmp = tmp->NextBinding)
  {
    if ((tmp->Context &
        ...
        (tmp->IsMouse == 0))
    {
      XGrabKey(dpy, tmp->Button_Key, tmp->Modifier, tmp_win->frame,
...
void FetchWmProtocols(FvwmWindow *tmp)
{
  ...
  else
  {
    ...
    if ((XGetWindowProperty(dpy, tmp->w, _XA_WM_PROTOCOLS, 0L, 10L,
                            False, _XA_WM_PROTOCOLS, &atype, &aformat,
                            &nitems, &bytes_remain,
    ...
    */
    if ((minAspectX * maxAspectY) > (maxAspectX * minAspectY))
    {
      ...
      fvwm_msg(WARN, "GetWindowSizeHints",
               "window id 0x%08x max_aspect ratio is < min_aspect "
               "ratio -> ignoring, but program displaying this window "
               "should be fixed!!!!",
               tmp->w);
    }
  }
...
void LookInList(FvwmWindow *tmp_win, name_list *styles)
{
  ...
  for (nptr = Scr.TheList; nptr != NULL; nptr = nptr->next)
  {
    ...
    else if (matchWildcards(nptr->name, tmp_win->class.res_name) ==
             TRUE)
    {
      merge_styles(styles, nptr);
    }
