...
void CreateIconWindow(FvwmWindow *tmp_win, int def_x, int def_y)
{
  ...
  {
    XSaveContext(dpy, tmp_win->icon_pixmap_w, FvwmContext,
                 (caddr_t)tmp_win);
    XDefineCursor(dpy, tmp_win->icon_pixmap_w,
    ...
    { /* all boxes */
      ...
      {                                      /* nothing found yet */
        ...
        while ((dim[2].step < 0            /* filling reversed */
                  ? (dim[2].start_at + dim[2].icon_dimension -
                       dim[2].nom_dimension >
        ...
        {                                      /* nothing found yet */
          ...
          dim[2].start_at +=
            dim[2].step; /* Grid inner value & direction */
...
void GrabIconButtons(FvwmWindow *tmp_win, Window w)
{
  ...
  while (MouseEntry != (Binding *)0)
  {
    if ((MouseEntry->Action != NULL) &&
        (MouseEntry->Context & C_ICON) && (MouseEntry->IsMouse == 1))
    {
      if (MouseEntry->Button_Key > 0)
        XGrabButton(dpy, MouseEntry->Button_Key, MouseEntry->Modifier,
                    w, True, ButtonPressMask | ButtonReleaseMask,
                    GrabModeAsync, GrabModeAsync, None,
                    Scr.FvwmCursors[DEFAULT]);
      else
      {
        XGrabButton(dpy, 1, MouseEntry->Modifier, w, True,
                    ButtonPressMask | ButtonReleaseMask, GrabModeAsync,
                    GrabModeAsync, None, Scr.FvwmCursors[DEFAULT]);
        XGrabButton(dpy, 2, MouseEntry->Modifier, w, True,
                    ButtonPressMask | ButtonReleaseMask, GrabModeAsync,
                    GrabModeAsync, None, Scr.FvwmCursors[DEFAULT]);
        XGrabButton(dpy, 3, MouseEntry->Modifier, w, True,
                    ButtonPressMask | ButtonReleaseMask, GrabModeAsync,
                    GrabModeAsync, None, Scr.FvwmCursors[DEFAULT]);
      }
    }
...
void GetXPMFile(FvwmWindow *tmp_win)
{
  ...
  if (rc != XpmSuccess)
  {
    fvwm_msg(ERR, "GetXPMFile",
             "XpmReadFileToXpmImage failed, pixmap %s, rc %d", path,
             rc);
    free(path);
    return;
  }
  ...
  rc = XpmCreatePixmapFromXpmImage(
    dpy, Scr.Root, &my_image, &tmp_win->iconPixmap,
    &tmp_win->icon_maskPixmap, &xpm_attributes);
  if (rc != XpmSuccess)
  {
    fvwm_msg(ERR, "GetXPMFile",
        ...
        ((t->flags & TRANSIENT) && (t->transientfor == tmp_win->w)))
    {
      ...
      if (t == tmp_win)
        BroadcastPacket(M_DEICONIFY, 11, t->w, t->frame,
                        (unsigned long)t, t->icon_x_loc, t->icon_y_loc,
                        t->icon_p_width,
                        t->icon_p_height + t->icon_w_height, t->frame_x,
...
void Iconify(FvwmWindow *tmp_win, int def_x, int def_y)
{
  ...
  for (t = Scr.FvwmRoot.next; t != NULL; t = t->next)
  {
        ...
        ((t->flags & TRANSIENT) && (t->transientfor == tmp_win->w)))
    {
      ...
      if (t != tmp_win)
      {
        ...
        BroadcastPacket(M_ICONIFY, 7, t->w, t->frame, (unsigned long)t,
                        -10000, -10000, t->icon_w_width,
                        t->icon_w_height + t->icon_p_height);
  ...
  BroadcastPacket(
    M_ICONIFY, 11, tmp_win->w, tmp_win->frame, (unsigned long)tmp_win,
    tmp_win->icon_x_loc, tmp_win->icon_y_loc, tmp_win->icon_w_width,
