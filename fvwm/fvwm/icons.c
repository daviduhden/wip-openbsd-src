 ...
 ****************************************************************************/
void
CreateIconWindow(FvwmWindow *tmp_win, int def_x, int def_y)
{
	int		     final_x, final_y;
	...
	if (!(tmp_win->flags & NOICON_TITLE) || (tmp_win->icon_p_height == 0))
		tmp_win->icon_w = XCreateWindow(dpy, Scr.Root, final_x,
		    ...
		    tmp_win->icon_w_height, 0, CopyFromParent, CopyFromParent,
		    CopyFromParent, valuemask, &attributes);

	if ((tmp_win->flags & ICON_OURS) && (tmp_win->icon_p_width > 0) &&
	    (tmp_win->icon_p_height > 0)) {
		tmp_win->icon_pixmap_w = XCreateWindow(dpy, Scr.Root, final_x,
		    final_y, tmp_win->icon_p_width, tmp_win->icon_p_height, 0,
		...
		XChangeWindowAttributes(
		    dpy, tmp_win->icon_pixmap_w, valuemask, &attributes);
	}

#ifdef XPM
#ifdef SHAPE
	if (ShapesSupported && (tmp_win->flags & SHAPED_ICON)) {
		XShapeCombineMask(dpy, tmp_win->icon_pixmap_w, ShapeBounding, 2,
		    2, tmp_win->icon_maskPixmap, ShapeSet);
	}
#endif
#endif
	...
	if (tmp_win->icon_w != None) {
		XSaveContext(
		    dpy, tmp_win->icon_w, FvwmContext, (caddr_t)tmp_win);
	...
	if (tmp_win->icon_pixmap_w != None) {
		XSaveContext(
		    dpy, tmp_win->icon_pixmap_w, FvwmContext, (caddr_t)tmp_win);
		XDefineCursor(
		    dpy, tmp_win->icon_pixmap_w, Scr.FvwmCursors[DEFAULT]);
 ...
 ****************************************************************************/
void
DrawIconWindow(FvwmWindow *Tmp_win)
{
	GC    Shadow, Relief;
	...
	if (Scr.Hilite == Tmp_win) {
		if (Scr.d_depth < 2) {
			Relief = Shadow = Scr.DefaultDecor.HiShadowGC;
	...
	if ((Tmp_win->iconPixmap != None) && (!(Tmp_win->flags & SHAPED_ICON)))
		RelieveWindow(Tmp_win, Tmp_win->icon_pixmap_w, 0, 0,
		    Tmp_win->icon_p_width, Tmp_win->icon_p_height, Relief,
		    Shadow, FULL_HILITE);

	/* need to locate the icon pixmap */
	if (Tmp_win->iconPixmap != None) {
		if (Tmp_win->iconDepth == Scr.d_depth) {
			XCopyArea(dpy, Tmp_win->iconPixmap,
			    Tmp_win->icon_pixmap_w, Scr.ScratchGC3, 0, 0,
			    Tmp_win->icon_p_width - 4,
			    Tmp_win->icon_p_height - 4, 2, 2);
		} else
			XCopyPlane(dpy, Tmp_win->iconPixmap,
			    Tmp_win->icon_pixmap_w, Scr.ScratchGC3, 0, 0,
			    Tmp_win->icon_p_width - 4,
 ...
 ************************************************************************/
void
RedoIconName(FvwmWindow *Tmp_win)
{
	...
	Tmp_win->icon_t_width = XTextWidth(
	    Scr.IconFont.font, Tmp_win->icon_name, strlen(Tmp_win->icon_name));
...
AutoPlace(FvwmWindow *t)
{
	...
	if ((t->flags & StickyIcon) || (t->flags & STICKY)) {
		...
		SetupFrame(
		    t, new_x, new_y, t->frame_width, t->frame_height, False);
...
#define HRZ_FILL icon_boxes_ptr->IconFlags &ICONFILLHRZ
		...
		for (icon_boxes_ptr = t->IconBoxes; /* init */
			...
			dim[1].step =
			    icon_boxes_ptr->IconGrid[1]; /* y amount */
			...
			if (BOT_FILL) { /* fill from bottom */
				dim[1].step =
				    0 - dim[1].step; /* reverse step */
			} /* end fill from bottom */

			dim[2].step =
			    icon_boxes_ptr->IconGrid[0]; /* x amount */
			...
			if (RGT_FILL) { /* fill from right */
				dim[2].step =
				    0 - dim[2].step; /* reverse step */
			} /* end fill from right */
			for (i = 1; i <= 2; i++) { /* for dimensions 1 and 2 */
				/* If the window is taller than the icon box,
				 * ignore the icon height when figuring where to
				 * put it. Same goes for the width This should
				 * permit reasonably graceful handling of big
				 * icons. */
				dim[i].nom_dimension = dim[i].icon_dimension;
				if (dim[i].icon_dimension >=
				    dim[i].end_at - dim[i].start_at) {
					dim[i].nom_dimension =
					    dim[i].end_at - dim[i].start_at - 1;
				}
			...
			while (
				...
				if (dim[1].start_at + dim[1].icon_dimension >
					...
					dim[1].real_start =
					    ...
					    dim[1].base; /* move on screen */
				} /* end off screen */
				if (dim[1].start_at <
				    dim[1].base) { /* if off other edge */
					dim[1].real_start =
					    dim[1].base; /* move on screen */
				} /* end off other edge */
				dim[2].start_at =
				...
				while ((dim[2].step < 0 ? /* filling reversed */
					...
					dim[2].start_at +=
					    dim[2].step; /* Grid inner value &
							    direction */
				} /* end while room inner dimension */
				dim[1].start_at +=
				    dim[1].step; /* Grid outer value & direction
						  */
		...
		if (t->icon_w != None)
			XMoveResizeWindow(dpy, t->icon_w, t->icon_xl_loc,
			    t->icon_y_loc + t->icon_p_height, t->icon_w_width,
			    ICON_HEIGHT);
		BroadcastPacket(M_ICON_LOCATION, 7, t->w, t->frame,
		    (unsigned long)t, t->icon_x_loc, t->icon_y_loc,
...
GetXPMFile(FvwmWindow *tmp_win)
{
	...
	rc = XpmCreatePixmapFromXpmImage(dpy, Scr.Root, &my_image,
	    &tmp_win->iconPixmap, &tmp_win->icon_maskPixmap, &xpm_attributes);
	if (rc != XpmSuccess) {
		fvwm_msg(ERR, "GetXPMFile",
 ...
 ****************************************************************************/
void
GetIconWindow(FvwmWindow *tmp_win)
{
 ...
 ****************************************************************************/
void
GetIconBitmap(FvwmWindow *tmp_win)
{
	/* We are guaranteed that wmhints is non-null when calling this
	 * routine */
	XGetGeometry(dpy, tmp_win->wmhints->icon_pixmap, &JunkRoot, &JunkX,
	    &JunkY, (unsigned int *)&tmp_win->icon_p_width,
...
DeIconify(FvwmWindow *tmp_win)
{
	...
	for (t = Scr.FvwmRoot.next; t != NULL; t = t->next) {
		if ((t == tmp_win) || IsTransientDescendantOf(t, tmp_win)) {
			...
			XFlush(dpy);
			if (t == tmp_win)
				BroadcastPacket(M_DEICONIFY, 11, t->w, t->frame,
				    (unsigned long)t, t->icon_x_loc,
				    t->icon_y_loc, t->icon_p_width,
				    t->icon_p_height + t->icon_w_height,
				    t->frame_x, t->frame_y, t->frame_width,
				    t->frame_height);
			else
				BroadcastPacket(M_DEICONIFY, 7, t->w, t->frame,
				    (unsigned long)t, t->icon_x_loc,
				    t->icon_y_loc, t->icon_p_width,
				    t->icon_p_height + t->icon_w_height);
			/* End AS */
 ...
 ****************************************************************************/
void
Iconify(FvwmWindow *tmp_win, int def_x, int def_y)
{
	FvwmWindow	 *t;
	...
	if ((tmp_win == Scr.Hilite) && (tmp_win->flags & ClickToFocus) &&
	    (tmp_win->next)) {
	...
	for (t = Scr.FvwmRoot.next; t != NULL; t = t->next) {
		if ((t == tmp_win) || IsTransientDescendantOf(t, tmp_win)) {
			...
			t->flags &= ~MAPPED;
			XSelectInput(
			    dpy, t->w, eventMask & ~StructureNotifyMask);
			...
			if (t != tmp_win) {
				...
				BroadcastPacket(M_ICONIFY, 7, t->w, t->frame,
				    (unsigned long)t, -10000, -10000,
				    t->icon_w_width,
	...
	if (tmp_win->icon_w == None) {
		if (tmp_win->flags & ICON_MOVED)
			CreateIconWindow(
			    tmp_win, tmp_win->icon_x_loc, tmp_win->icon_y_loc);
		else
	...
	BroadcastPacket(M_ICONIFY, 11, tmp_win->w, tmp_win->frame,
	    (unsigned long)tmp_win, tmp_win->icon_x_loc, tmp_win->icon_y_loc,
	    tmp_win->icon_w_width,
	    tmp_win->icon_w_height + tmp_win->icon_p_height,
	    tmp_win->frame_x, /* next 4 added for Animate module */
	    tmp_win->frame_y, tmp_win->frame_width, tmp_win->frame_height);
 ...
 ****************************************************************************/
void
SetMapStateProp(FvwmWindow *tmp_win, int state)
{
