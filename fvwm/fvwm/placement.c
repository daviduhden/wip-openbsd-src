...
/*  RBW - 11/02/1998  */
int
SmartPlacement(FvwmWindow *t, int width, int height, int *x, int *y,
...
/*  RBW - 11/02/1998  */
void
CleverPlacement(FvwmWindow *t, int *x, int *y, int pdeltax, int pdeltay)
{
	/**/
	int test_x = 0, test_y = 0;
	int xbest, ybest;
...
/*  RBW - 11/02/1998  */
int
get_next_x(FvwmWindow *t, int x, int y, int pdeltax, int pdeltay)
{
	/**/
	int	    xnew;
	int	    xtest;
...
/*  RBW - 11/02/1998  */
int
test_fit(FvwmWindow *t, int x11, int y11, int aoimin, int pdeltax, int pdeltay)
{
	/**/
	FvwmWindow *testw;
	int	    x12, x21, x22;
	...
	for (testw = Scr.FvwmRoot.next; testw != NULL; testw = testw->next) {
		...
		if (testw->flags & ICONIFIED) {
			if (testw->icon_w == None ||
			    testw->flags & ICON_UNMAPPED)
				continue;
...
Bool
PlaceWindow(
    FvwmWindow *tmp_win, unsigned long tflag, int Desk, int PageX, int PageY)
{
	/**/
	FvwmWindow *t;
	...
	if (tflag & STARTSONDESK_FLAG) {
		...
		/*
		      ...
		      it's a restart or recapture, and that option's
		   disallowed...
		  */
		if (PPosOverride &&
		    (Restarting || (Scr.flags & WindowsCaptured)) &&
		...
		/*
		      it's ActivePlacement and SkipMapping, and that's
		   disallowed.
		  */
		if (!PPosOverride &&
	...
	if (tflag & STICKY_FLAG)
		...
		tmp_win->Desk =
		    (Desk > -1) ? Desk - 1 : Desk; /*  RBW - 11/20/1998  */
	else {
		Atom	       atype;
		int	       aformat;
	...
	if (!(tmp_win->flags & TRANSIENT) &&
		...
		if (tflag & RANDOM_PLACE_FLAG) {
			if (tflag & SMART_PLACE_FLAG)
				smartlyplaced = SmartPlacement(tmp_win,
			...
			if (!smartlyplaced) {
				...
				if ((Scr.randomy +=
					...
					Scr.randomy =
					    2 * GetDecor(tmp_win, TitleHeight);
				tmp_win->attr.x =
				    (Scr.randomx - pdeltax) - tmp_win->old_bw;
				tmp_win->attr.y =
				    (Scr.randomy - pdeltay) - tmp_win->old_bw;
			} else {
				tmp_win->attr.x =
				    xl - tmp_win->old_bw + tmp_win->bw;
				tmp_win->attr.y =
				    yt - tmp_win->old_bw + tmp_win->bw;
			}
			...
			if (tmp_win->frame_x + tmp_win->frame_width +
			    ...
			    PageRight) {
				tmp_win->attr.x = PageRight -
				    tmp_win->attr.width - tmp_win->old_bw +
				    tmp_win->bw - 2 * tmp_win->boundary_width;
				Scr.randomx = 0;
			}
			if (tmp_win->frame_y + 2 * tmp_win->boundary_width +
				tmp_win->title_height + tmp_win->frame_height >
			    PageBottom) {
				tmp_win->attr.y = PageBottom -
				    tmp_win->attr.height - tmp_win->old_bw +
				    tmp_win->bw - tmp_win->title_height -
				    2 * tmp_win->boundary_width;
				;
				Scr.randomy = 0;
			}
			...
			/* if the gravity is towards the top, move it by the
			 * title height */
			tmp_win->ydiff +=
			    gravy * (tmp_win->bw - tmp_win->old_bw);
			tmp_win->xdiff +=
			...
			if (!smartlyplaced) {
				if (GrabEm(POSITION)) {
					...
					moveLoop(tmp_win, 0, 0, DragWidth,
					    DragHeight, &xl, &yt, False, True);
	    ...
	    */
		...
		if (((tmp_win->flags & SHOW_ON_MAP) && HonorStartsOnPage) &&
			...
			/*
			   ...
			   (i.e., as though there were only one page, no virtual
			   desktop), then 2) readjust relative to the current
			   page.
			  */
			...
			/*
		      ...
		      happening? Probably old_bw, try xclock -borderwidth 100
		  */
			if (tmp_win->attr.y < 0) {
				tmp_win->attr.y =
