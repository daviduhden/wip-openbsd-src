...
void CleverPlacement(FvwmWindow *t, int *x, int *y, int pdeltax,
                     int pdeltay);
/**/
      ...
      while ((test_window != (FvwmWindow *)0) && (loc_ok == True))
      {
            ...
            (test_window->flags & STICKY))
        {
              ...
              (test_window->icon_w) && (test_window != t))
          {
            ...
            th =
              test_window->icon_p_height + test_window->icon_w_height;
...
int get_next_x(FvwmWindow *t, int x, int y, int pdeltax, int pdeltay)
{
  ...
  for (testw = Scr.FvwmRoot.next; testw != NULL; testw = testw->next)
  {
    ...
    if (testw->flags & ICONIFIED)
    {
           ...
           (t->frame_height + 2 * t->bw + y)))
      {
        ...
        if (xtest > x)
          xnew = MIN(xnew, xtest);
        xtest =
          testw->icon_x_loc - stickyx - (t->frame_width + 2 * t->bw);
        if (xtest > x)
          xnew = MIN(xnew, xtest);
      }
    }
    ...
    {
      xtest =
        testw->frame_width + 2 * testw->bw + testw->frame_x - stickyx;
      if (xtest > x)
        xnew = MIN(xnew, xtest);
  ...
  for (testw = Scr.FvwmRoot.next; testw != NULL; testw = testw->next)
  {
    ...
    if (testw->flags & ICONIFIED)
    {
      ...
      if (ytest > y)
        ynew = MIN(ynew, ytest);
      ytest =
        testw->icon_y_loc - stickyy - (t->frame_height + 2 * t->bw);
      if (ytest > y)
        ynew = MIN(ynew, ytest);
    }
    else
    {
      ytest =
        testw->frame_height + 2 * testw->bw + testw->frame_y - stickyy;
      if (ytest > y)
        ynew = MIN(ynew, ytest);
...
*/
  if (!(tflag & STICKY_FLAG) && (tflag & STARTSONDESK_FLAG))
  {
    ...
    {
      int current_px =
        (Scr.MyDisplayWidth != 0) ? Scr.Vx / Scr.MyDisplayWidth : 0;
      int current_py =
        (Scr.MyDisplayHeight != 0) ? Scr.Vy / Scr.MyDisplayHeight : 0;

      px =
        (PageX != 0) ? ((PageX > 0) ? PageX - 1 : PageX) : current_px;
      py =
        (PageY != 0) ? ((PageY > 0) ? PageY - 1 : PageY) : current_py;
        ...
        (HonorStartsOnPage))))
  {
    ...
    if (tflag & RANDOM_PLACE_FLAG)
    {
      if (tflag & SMART_PLACE_FLAG)
        smartlyplaced = SmartPlacement(
          tmp_win, tmp_win->frame_width + 2 * tmp_win->bw,
          tmp_win->frame_height + 2 * tmp_win->bw, &xl, &yt, pdeltax,
          pdeltay);
      if (!smartlyplaced)
      {
