...
#endif /* WINDOWSHADE */
    {"XORValue", SetXOR, F_XOR, FUNC_NO_WINDOW}, {"", 0, 0, 0}};
...
*/
static int
func_comp(const void *a, const void *b)
{
	char *key = (char *)a;
 ...
 ***********************************************************************/
void
ExecuteFunction(char *Action, FvwmWindow *tmp_win, XEvent *eventp,
    unsigned long context, int Module)
{
	...
	if (!matched) {
		...
		if (desperate)
			executeModule(
			    eventp, w, tmp_win, context, taction, &Module);
		desperate = 0;
	}
...
void
find_func_type(char *action, short *func_type, Bool *func_needs_window)
{
	int j, len = 0;
	char *endtok = action;
	Bool matched;
	...
	if (action) {
		...
		matched = FALSE;
		while ((!matched) &&
		       ((mlen = strlen(func_config[j].keyword)) > 0)) {
			if ((mlen == len) &&
			    (strncasecmp(
			         action, func_config[j].keyword, mlen) == 0)) {
				matched = TRUE;
				/* found key word */
				*func_type = func_config[j].func_type;
				*func_needs_window =
				    func_config[j].func_needs_window;
				return;
			} else
				j++;
		}
