...
void MoveViewport(int newx, int newy, Bool grab)
{
  ...
  */
  ...
  /* EWMH viewport and geometry reflect only the current page and screen size */
...
void changeDesks(int desk)
{
  ...
  /* EWMH: reflect current desktop change */
  EWMH_UpdateCurrentDesktop(Scr.CurrentDesk);
  /* EWMH: recompute desktops (count/names/workarea) if needed */
  EWMH_RecalcDesktops();
...
/*  else if (StickyWin && (StickyWin->flags & STICKY))
  ...
  /* EWMH: after re-mapping/unmapping, refresh client lists */
		       ...
		       unsigned long context,char *action, int *Module)
{
  ...
  */
  ...
  /* EWMH: moving window across desks may expand the max desk index */
  EWMH_RecalcDesktops();
