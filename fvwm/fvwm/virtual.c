 ...
 ***************************************************************************/
void
HandlePaging(int HorWarpSize, int VertWarpSize, int *xl, int *yt, int *delta_x,
    int *delta_y, Bool Grab)
{
	int x, y, total;
	...
	*delta_y = 0;
	...
	while (total < Scr.ScrollResistance) {
		...
		/* check actual pointer location since PanFrames can get buried
		   under a window being moved or resized - mab */
		if ((x >= edge_thickness) &&
...
checkPanFrames(void)
{
	...
	if (edge_thickness != last_edge_thickness) {
		...
		XMoveResizeWindow(dpy, Scr.PanFrameRight.win,
		    Scr.MyDisplayWidth - edge_thickness, 0, edge_thickness,
		    Scr.MyDisplayHeight);
		XMoveResizeWindow(dpy, Scr.PanFrameBottom.win, 0,
		    Scr.MyDisplayHeight - edge_thickness, Scr.MyDisplayWidth,
		    edge_thickness);
...
initPanFrames()
{
	 ...
	 * unmapped */
	Scr.PanFrameTop.win = XCreateWindow(dpy, Scr.Root, 0, 0,
	    Scr.MyDisplayWidth, edge_thickness, 0, /* no border */
	    CopyFromParent, InputOnly, CopyFromParent, valuemask, &attributes);
	attributes.cursor = Scr.FvwmCursors[LEFT];
	Scr.PanFrameLeft.win = XCreateWindow(dpy, Scr.Root, 0, 0,
	    edge_thickness, Scr.MyDisplayHeight, 0, /* no border */
	    CopyFromParent, InputOnly, CopyFromParent, valuemask, &attributes);
	attributes.cursor = Scr.FvwmCursors[RIGHT];
	Scr.PanFrameRight.win = XCreateWindow(dpy, Scr.Root,
	    Scr.MyDisplayWidth - edge_thickness, 0, edge_thickness,
	    Scr.MyDisplayHeight, 0, /* no border */
	    CopyFromParent, InputOnly, CopyFromParent, valuemask, &attributes);
	attributes.cursor = Scr.FvwmCursors[BOTTOM];
	Scr.PanFrameBottom.win = XCreateWindow(dpy, Scr.Root, 0,
	    Scr.MyDisplayHeight - edge_thickness, Scr.MyDisplayWidth,
	    edge_thickness, 0, /* no border */
	    CopyFromParent, InputOnly, CopyFromParent, valuemask, &attributes);
...
MoveViewport(int newx, int newy, Bool grab)
{
	...
	*/
	...
	if ((deltax != 0) || (deltay != 0)) {
		...
		the viewport from the bottom of the stacking order up, to
		minimize the expose-redraw overhead. Windows that will be moving
		into view will be moved top down, for the same reason. Use the
		new  stacking-order chain, rather than the old last-focussed
		chain.
	    */
		...
		while (t != &Scr.FvwmRoot || t1 != &Scr.FvwmRoot) {
			if (t != &Scr.FvwmRoot) {
				...
				if ((txr >= PageLeft && txl <= PageRight &&
					...
					/* If the window is iconified, and
...
changeDesks(int desk)
{
	...
	/* OK, someone beat me up, but I don't like this. If you are a
	 * predominantly focus-follows-mouse person, but put in one sticky
	 * click-to-focus window (typically because you don't really want to
	 * give focus to this window), then the following lines are screwed up.
	 */
