                      ...
                      unsigned long context, char *action, int *Module)
{
  ...
  { /* check range */
    fvwm_msg(ERR, "setEdgeThickness",
             "EdgeThickness arg must be between 0 and 2, found %d",
             val);
    return;
  }
  edge_thickness = val;
                  ...
                  int *delta_x, int *delta_y, Bool Grab)
{
  ...
  *delta_y = 0;
  ...
  if ((Scr.VxMax == 0 || (*xl >= edge_thickness &&
  ...
  while (total < Scr.ScrollResistance)
  {
    ...
    if (XCheckWindowEvent(dpy, Scr.PanFrameTop.win, LeaveWindowMask,
                          &Event))
    {
      StashEventTime(&Event);
      return;
    }
    if (XCheckWindowEvent(dpy, Scr.PanFrameBottom.win, LeaveWindowMask,
                          &Event))
    {
      StashEventTime(&Event);
      return;
    }
    if (XCheckWindowEvent(dpy, Scr.PanFrameLeft.win, LeaveWindowMask,
                          &Event))
    {
      StashEventTime(&Event);
      return;
    }
    if (XCheckWindowEvent(dpy, Scr.PanFrameRight.win, LeaveWindowMask,
                          &Event))
    {
      StashEventTime(&Event);
      return;
    ...
    while (t != &Scr.FvwmRoot || t1 != &Scr.FvwmRoot)
    {
      if (t != &Scr.FvwmRoot)
      {
	    ...
	    */
        ...
        if ((txr >= PageLeft && txl <= PageRight && tyb >= PageTop &&
             tyt <= PageBottom) &&
            ...
            !t1->tmpflags.ViewportMoved)
        {
              ...
              (!(t1->flags & STICKY)))
          {
            if (!(t1->flags & StickyIcon))
            {
              ...
              {
                BroadcastPacket(M_ICON_LOCATION, 7, t1->w, t1->frame,
                                (unsigned long)t1, t1->icon_x_loc,
                                t1->icon_y_loc, t1->icon_w_width,
