...
void DrawButton(FvwmWindow *t, Window win, int W, int H, ButtonFace *bf,
                GC ReliefGC, GC ShadowGC, Boolean inverted,
                int stateflags);

#ifdef VECTOR_BUTTONS
void DrawLinePattern(Window win, GC ReliefGC, GC ShadowGC,
               ...
               Window expose_win)
{
  ...
  if (onoroff)
  {
    ...
    if ((GetDecor(t, BorderStyle.active.style) & ButtonFaceTypeMask) ==
        TiledPixmapButton)
...
#if defined(PIXMAP_BUTTONS) && defined(BORDERSTYLE)
  valuemask = notex_valuemask = CWBorderPixel;
  attributes.border_pixel = notex_attributes.border_pixel = BorderColor;
#else
    ...
    for (i = 0; i < Scr.nr_left_buttons; ++i)
    {
      if (t->left_w[i] != None)
      {
        ...
        if (flush_expose(t->left_w[i]) ||
            (expose_win == t->left_w[i]) || (expose_win == None)
#if defined(PIXMAP_BUTTONS) && defined(BORDERSTYLE)
            || NewColor
#endif
        )
        {
          ...
          if (bf->style & UseBorderStyle)
            XChangeWindowAttributes(dpy, t->left_w[i], valuemask,
                                    &attributes);
          else
...
#endif
              DrawButton(t, t->left_w[i], t->title_height,
                         t->title_height, tsbf, ReliefGC, ShadowGC,
...
#endif
            DrawButton(t, t->left_w[i], t->title_height,
                       t->title_height, bf, ReliefGC, ShadowGC,
                       inverted, GetDecor(t, left_buttons[i].flags));
                ...
                FlatButton))
          {
            if (GetDecor(t, left_buttons[i].state[bs].style) &
                SunkButton)
              RelieveWindow(
                t, t->left_w[i], 0, 0, t->title_height, t->title_height,
                (inverted ? ReliefGC : ShadowGC),
                (inverted ? ShadowGC : ReliefGC), BOTTOM_HILITE);
            else
              RelieveWindow(
                t, t->left_w[i], 0, 0, t->title_height, t->title_height,
                (inverted ? ShadowGC : ReliefGC),
                (inverted ? ReliefGC : ShadowGC), BOTTOM_HILITE);
          }
        }
      }
    }
      ...
      if (t->right_w[i] != None)
      {
        ...
        )
        {
...
#if defined(PIXMAP_BUTTONS) && defined(BORDERSTYLE)
          if (bf->style & UseBorderStyle)
            XChangeWindowAttributes(dpy, t->right_w[i], valuemask,
                                    &attributes);
          else
            XChangeWindowAttributes(dpy, t->right_w[i], notex_valuemask,
                                    &notex_attributes);
          ...
          if (bf->style & UseTitleStyle)
          {
            ...
            for (; tsbf; tsbf = tsbf->next)
#endif
              DrawButton(t, t->right_w[i], t->title_height,
                         t->title_height, tsbf, ReliefGC, ShadowGC,
                ...
                FlatButton))
          {
            if (GetDecor(t, right_buttons[i].state[bs].style) &
                SunkButton)
              RelieveWindow(
                t, t->right_w[i], 0, 0, t->title_height,
                t->title_height, (inverted ? ReliefGC : ShadowGC),
                (inverted ? ShadowGC : ReliefGC), BOTTOM_HILITE);
            else
              RelieveWindow(
                t, t->right_w[i], 0, 0, t->title_height,
                t->title_height, (inverted ? ShadowGC : ReliefGC),
                (inverted ? ReliefGC : ShadowGC), BOTTOM_HILITE);
          }
        }
      }
  ...
  if (t->flags & BORDER)
  {
    ...
    for (i = 0; i < 4; i++)
    {
      int vertical = i % 2;
#ifdef BORDERSTYLE
      int flags = onoroff ? GetDecor(t, BorderStyle.active.style)
        ...
        if (flags & HiddenHandles)
        {
          if (flags & NoInset)
            RelieveWindowHH(
              ...
              ((vertical) ? y : t->boundary_width), rgc, sgc,
              vertical ? (i == 3 ? LEFT_HILITE : RIGHT_HILITE)
                       : (i ? BOTTOM_HILITE : TOP_HILITE),
              (0x0001 << i));
          else
            RelieveWindowHH(t, t->sides[i], 0, 0,
                            ((vertical) ? t->boundary_width : x),
                            ((vertical) ? y : t->boundary_width), rgc,
                            sgc,
                            vertical ? (LEFT_HILITE | RIGHT_HILITE)
...
#endif /* BORDERSTYLE */
          RelieveWindow(
            t, t->sides[i], 0, 0, ((i % 2) ? t->boundary_width : x),
            ((i % 2) ? y : t->boundary_width), rgc, sgc, (0x0001 << i));
      }
      ChangeWindowColor(t->corners[i], valuemask);
      if ((flush_expose(t->corners[i])) ||
          (expose_win == t->corners[i]) || (expose_win == None))
      {
        ...
        {
          RelieveWindowHH(
            t, t->corners[i], 0, 0, t->corner_width,
            ((i / 2) ? t->corner_width + t->bw : t->corner_width), rgc,
            sgc, corners[i], corners[i]);

          if (!(flags & NoInset))
          {
            if (t->boundary_width > 1)
              RelieveParts(t, i | HH_HILITE, ((i / 2) ? rgc : sgc),
                           (vertical ? rgc : sgc));
            else
              RelieveParts(t, i | HH_HILITE, ((i / 2) ? sgc : sgc),
                           (vertical ? sgc : sgc));
          }
        }
        else
        {
          ...
          if (t->boundary_width > 1)
            RelieveParts(t, i, ((i / 2) ? rgc : sgc),
                         (vertical ? rgc : sgc));
          else
            RelieveParts(t, i, ((i / 2) ? sgc : sgc),
                         (vertical ? sgc : sgc));
#ifdef BORDERSTYLE
        }
#endif
  ...
  else /* no decorative border */
  {
    ...
    else
    {
          ...
          (expose_win == None))
      {
        ...
        {
          RelieveWindow(
            t, t->frame, t->boundary_width - 1 - t->bw,
                ...
                int stateflags)
{
...
#ifdef VECTOR_BUTTONS
  case VectorButton:
    if ((t->flags & MWMButtons) && (stateflags & MWMDecorMaximize) &&
        (t->flags & MAXIMIZED))
  ...
  case PixmapButton:
    ...
    XCopyArea(dpy, p->picture, win, Scr.TransMaskGC, 0, 0, width,
              height, x, y);
    break;
  ...
  case VGradButton:
  {
    ...
    if (type == HGradButton)
    {
      ...
      while (i < bf->u.grad.npixels)
      {
        ...
        XFillRectangle(dpy, win, Scr.TransMaskGC, bounds.x + x,
                       bounds.y, dw, bounds.height);
      }
    }
    else
    {
      register int i = 0, dh = bounds.height / bf->u.grad.npixels + 1;
      while (i < bf->u.grad.npixels)
      {
        unsigned short y = i * bounds.height / bf->u.grad.npixels;
        XSetForeground(dpy, Scr.TransMaskGC, bf->u.grad.pixels[i++]);
        XFillRectangle(dpy, win, Scr.TransMaskGC, bounds.x,
                       bounds.y + y, bounds.width, dh);
      }
    }
  }
  break;
...
void SetTitleBar(FvwmWindow *t, Bool onoroff, Bool NewTitle)
{
  ...
  else
  {
    ...
    if (PressedW == t->title_w)
    {
      ...
      for (; bf; bf = bf->next)
#endif
        DrawButton(t, t->title_w, t->title_width, t->title_height, bf,
                   ShadowGC, ReliefGC, True, 0);
    }
    else
    {
#ifdef MULTISTYLE
    ...
    if (!(tb_style & FlatButton))
    {
      if (tb_style & SunkButton)
        RelieveWindow(t, t->title_w, 0, 0, t->title_width,
                      t->title_height, ShadowGC, ReliefGC,
                      BOTTOM_HILITE);
      else
        RelieveWindow(t, t->title_w, 0, 0, t->title_width,
  ...
  if (t->flags & STICKY || Scr.StipledTitles)
  {
    ...
    {
      XDrawLine(dpy, t->title_w, ShadowGC, 4,
                t->title_height / 2 - i - 1, hor_off - 6,
                t->title_height / 2 - i - 1);
      XDrawLine(dpy, t->title_w, ShadowGC, 6 + hor_off + w,
                t->title_height / 2 - i - 1, t->title_width - 5,
                t->title_height / 2 - i - 1);
      XDrawLine(dpy, t->title_w, ReliefGC, 4, t->title_height / 2 - i,
                hor_off - 6, t->title_height / 2 - i);
      XDrawLine(dpy, t->title_w, ReliefGC, 6 + hor_off + w,
                t->title_height / 2 - i, t->title_width - 5,
                t->title_height / 2 - i);
 ...
 *
 ****************************************************************************/
void RelieveWindow(FvwmWindow *t, Window win, int x, int y, int w,
                   int h, GC ReliefGC, GC ShadowGC, int hilite)
{
  XSegment seg[4];
  int i;
  int edge;
 ...
 *
 ****************************************************************************/
void RelieveWindowHH(FvwmWindow *t, Window win, int x, int y, int w,
                     int h, GC ReliefGC, GC ShadowGC, int draw,
                     int hilite)
{
  XSegment seg[4];
  int i = 0;
                     ...
                     struct vector_coords *coords, int w, int h)
{
  ...
  for (; i < coords->num; ++i)
  {
    XDrawLine(dpy, win, coords->line_style[i] ? ReliefGC : ShadowGC,
                ...
                Bool sendEvent)
{
  ...
  fvwm_msg(DBG, "SetupFrame",
           "Routine Entered (x == %d, y == %d, w == %d, h == %d)", x, y,
           w, h);
#endif
  ...
  if (Resized)
  {
    ...
    if (tmp_win->flags & TITLE)
      tmp_win->title_height =
        GetDecor(tmp_win, TitleHeight) + tmp_win->bw;
    ...
    if (tmp_win->flags & TITLE)
    {
      ...
      tmp_win->title_x =
        tmp_win->boundary_width + (left)*tmp_win->title_height;
    ...
    if (tmp_win->flags & BORDER)
    {
      tmp_win->corner_width = GetDecor(tmp_win, TitleHeight) +
                              tmp_win->bw + tmp_win->boundary_width;
  ...
  )
  {
    ...
    client_event.xconfigure.y =
      y + tmp_win->title_height + tmp_win->boundary_width;
    client_event.xconfigure.width = w - 2 * tmp_win->boundary_width;
    client_event.xconfigure.height =
      h - 2 * tmp_win->boundary_width - tmp_win->title_height;
...
#ifdef FVWM_DEBUG_MSGS
    fvwm_msg(
      DBG, "SetupFrame", "Sent ConfigureNotify (w == %d, h == %d)",
...
void SetShape(FvwmWindow *tmp_win, int w)
{
  ...
  if (ShapesSupported)
  {
    ...
    if (tmp_win->title_w)
    {
      ...
      XShapeCombineRectangles(dpy, tmp_win->frame, ShapeBounding, 0, 0,
