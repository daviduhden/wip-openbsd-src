...
#define GetButtonState(window)                                                 \
	(onoroff ? ((PressedW == (window)) ? ActiveDown : ActiveUp) : Inactive)
#else
#ifdef ACTIVEDOWN_BTNS
#define GetButtonState(window) ((PressedW == (window)) ? ActiveDown : ActiveUp)
#endif
#ifdef INACTIVE_BTNS
    ...
    FvwmWindow *t, Bool onoroff, Bool force, Bool Mapped, Window expose_win)
{
	...
	if (t->flags & TITLE) {
		...
		for (i = 0; i < Scr.nr_left_buttons; ++i) {
			if (t->left_w[i] != None) {
				enum ButtonState bs =
				    GetButtonState(t->left_w[i]);
				ButtonFace *bf =
				    &GetDecor(t, left_buttons[i].state[bs]);
#if !(defined(PIXMAP_BUTTONS) && defined(BORDERSTYLE))
		...
		for (i = 0; i < Scr.nr_right_buttons; ++i) {
			if (t->right_w[i] != None) {
				enum ButtonState bs =
				    GetButtonState(t->right_w[i]);
				ButtonFace *bf =
				    &GetDecor(t, right_buttons[i].state[bs]);
#if !(defined(PIXMAP_BUTTONS) && defined(BORDERSTYLE))
	...
	if (t->flags & BORDER) {
		...
		for (i = 0; i < 4; i++) {
			...
			if ((flush_expose(t->sides[i])) ||
			    (expose_win == t->sides[i]) ||
			    (expose_win == None)) {
				GC sgc, rgc;
		...
		if (t->boundary_width < 2) {
			...
			if (Scr.d_depth < 2) {
				...
				XSetWindowBackgroundPixmap(
				    dpy, t->frame, BackPixmap);
				XClearWindow(dpy, t->frame);
				XSetWindowBackgroundPixmap(
				    dpy, t->Parent, BackPixmap);
				XClearWindow(dpy, t->Parent);
			} else {
#if defined(PIXMAP_BUTTONS) && defined(BORDERSTYLE)
				XSetWindowBackgroundPixmap(
				    dpy, t->frame, TexturePixmap);
#endif
				XSetWindowBorder(dpy, t->frame, BorderColor);
				XClearWindow(dpy, t->frame);
				XSetWindowBackground(
				    dpy, t->Parent, BorderColor);
				XSetWindowBorder(dpy, t->Parent, BorderColor);
				XClearWindow(dpy, t->Parent);
			...
			if ((flush_expose(t->frame)) ||
			    (expose_win == t->frame) || (expose_win == None)) {
				if (t->boundary_width > 2) {
					RelieveWindow(t, t->frame,
    ...
    GC ShadowGC, Boolean inverted, int stateflags)
{
	...
	case VectorButton:
		if ((t->flags & MWMButtons) &&
			...
			DrawLinePattern(
			    win, ShadowGC, ReliefGC, &bf->vector, w, h);
		else
			DrawLinePattern(
			    win, ReliefGC, ShadowGC, &bf->vector, w, h);
		break;
...
SetTitleBar(FvwmWindow *t, Bool onoroff, Bool NewTitle)
{
	...
	NewFontAndColor(
	    GetDecor(t, WindowFont.font->fid), Forecolor, BackColor);
	...
	if (Scr.d_depth < 2) {
		...
		RelieveWindow(t, t->title_w, hor_off + w + 2, 0,
		    t->title_width - w - hor_off - 2, t->title_height, ReliefGC,
		    ShadowGC, BOTTOM_HILITE);
		XFillRectangle(dpy, t->title_w,
		    (PressedW == t->title_w ? ShadowGC : ReliefGC), hor_off - 2,
		    0, w + 4, t->title_height);
		...
		if (PressedW == t->title_w) {
			...
			for (; bf; bf = bf->next)
#endif
				DrawButton(t, t->title_w, t->title_width,
				    t->title_height, bf, ShadowGC, ReliefGC,
				    True, 0);
		} else {
#ifdef MULTISTYLE
			for (; bf; bf = bf->next)
		...
		if (!(tb_style & FlatButton)) {
			if (tb_style & SunkButton)
				RelieveWindow(t, t->title_w, 0, 0,
				    t->title_width, t->title_height, ShadowGC,
				    ReliefGC, BOTTOM_HILITE);
			else
				RelieveWindow(t, t->title_w, 0, 0,
				    t->title_width, t->title_height, ReliefGC,
				    ShadowGC, BOTTOM_HILITE);
		}
	...
	if (t->flags & STICKY || Scr.StipledTitles) {
		for (i = 0; i < t->title_height / 2 - 3; i += 4) {
			...
			XDrawLine(dpy, t->title_w, ShadowGC, 6 + hor_off + w,
			    ...
			    t->title_height / 2 - i - 1);
			XDrawLine(dpy, t->title_w, ReliefGC, 4,
			    t->title_height / 2 - i, hor_off - 6,
			    t->title_height / 2 - i);
 ...
 ****************************************************************************/
void
RelieveWindow(FvwmWindow *t, Window win, int x, int y, int w, int h,
 ...
 ****************************************************************************/
void
RelieveWindowHH(FvwmWindow *t, Window win, int x, int y, int w, int h,
...
void
RelieveParts(FvwmWindow *t, int i, GC hor, GC vert)
{
	XSegment seg[2];
	...
	if ((t->flags & MWMBorders) || (t->boundary_width < 3)) {
		...
		case 3:
			...
			if (t->boundary_width > 3) {
				seg[0].x2 =
				    t->corner_width - t->boundary_width + 1;
 ...
 ****************************************************************************/
void
DrawLinePattern(Window win, GC ReliefGC, GC ShadowGC,
    struct vector_coords *coords, int w, int h)
{
	int i = 1;
	for (; i < coords->num; ++i) {
...
void
SetupFrame(FvwmWindow *tmp_win, int x, int y, int w, int h, Bool sendEvent)
{
	XEvent client_event;
	...
	fvwm_msg(DBG, "SetupFrame",
	    "Routine Entered (x == %d, y == %d, w == %d, h == %d)", x, y, w, h);
#endif
	...
	if (Resized) {
		...
		tmp_win->title_width =
		    w - (left + right) * tmp_win->title_height -
	...
	if (sendEvent
		...
		XSendEvent(
		    dpy, tmp_win->w, False, StructureNotifyMask, &client_event);
#ifdef FVWM_DEBUG_MSGS
		fvwm_msg(DBG, "SetupFrame",
 ...
 ****************************************************************************/
void
SetShape(FvwmWindow *tmp_win, int w)
{
#ifdef SHAPE
	if (ShapesSupported) {
		XRectangle rect;
		...
		XShapeCombineShape(dpy, tmp_win->frame, ShapeBounding,
		    tmp_win->boundary_width,
		    tmp_win->title_height + tmp_win->boundary_width, tmp_win->w,
		...
		if (tmp_win->title_w) {
			...
			rect.width =
			    w - 2 * tmp_win->boundary_width + tmp_win->bw;
			rect.height = tmp_win->title_height;

			XShapeCombineRectangles(dpy, tmp_win->frame,
			    ShapeBounding, 0, 0, &rect, 1, ShapeUnion,
			    Unsorted);
		}
	}
#endif
