...
static void DoResize(int x_root, int y_root, FvwmWindow *tmp_win,
                     geom *drag, geom *orig, int *xmotionp,
                     int *ymotionp);
                   ...
                   unsigned long context, char *action, int *Module)
{
  ...
  XGetGeometry(dpy, (Drawable)ResizeWindow, &JunkRoot, &drag->x,
               &drag->y, (unsigned int *)&drag->width,
  ...
  while (!finished)
  {
    ...
    if (Event.type == MotionNotify)
      ...
      while (XCheckMaskEvent(
        dpy, ButtonMotionMask | ButtonReleaseMask | PointerMotionMask,
        &Event))
      {
        StashEventTime(&Event);
    ...
    case MotionNotify:
      ...
      HandlePaging(Scr.EdgeScrollX, Scr.EdgeScrollY, &x, &y, &delta_x,
                   &delta_y, False);
  ...
  if (!abort)
  {
    ...
    ConstrainSize(tmp_win, &drag->width, &drag->height, True, xmotion,
                  ymotion);
    SetupFrame(tmp_win, drag->x - tmp_win->bw, drag->y - tmp_win->bw,
...
#define _min(a, b) (((a) < (b)) ? (a) : (b))
  ...
  /*
     ...
     */
  dwidth =
    ((dwidth - baseWidth + constrainx) / xinc * xinc) + baseWidth;
  dheight =
...
#define minAspectY tmp_win->hints.min_aspect.y
  ...
  if (tmp_win->hints.flags & PAspect)
  {
    if ((minAspectX * dheight > minAspectY * dwidth) && (xmotion == 0))
    {
      ...
      delta =
        makemult(minAspectX * dheight / minAspectY - dwidth, xinc);
    ...
    {
      delta =
        makemult(dwidth * maxAspectY / maxAspectX - dheight, yinc);
