...
static void DoResize(int x_root, int y_root, FvwmWindow *tmp_win, geom *drag,
    geom *orig, int *xmotionp, int *ymotionp);
    ...
    unsigned long context, char *action, int *Module)
{
	...
	if (n == 2) {
		...
		ConstrainSize(tmp_win, &drag->width, &drag->height, False,
		    xmotion, ymotion);
	...
	XGetGeometry(dpy, (Drawable)ResizeWindow, &JunkRoot, &drag->x, &drag->y,
	...
	if (eventp->type == KeyPress) {
		XQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild, &stashed_x,
		    &stashed_y, &JunkX, &JunkY, &JunkMask);
		DoResize(stashed_x, stashed_y, tmp_win, drag, orig, &xmotion,
		    &ymotion);
	} else
	...
	while (!finished) {
		...
		case ButtonPress:
			...
			/* Abort the resize if
			 *  - the move started with a pressed button and another
		...
		case KeyPress:
			...
			if (XLookupKeysym(&(Event.xkey), 0) == XK_Escape ||
				 ...
				 * with key */
				if (stashed_x >= 0)
					XWarpPointer(dpy, None, Scr.Root, 0, 0,
					    0, 0, stashed_x, stashed_y);
			}
			done = TRUE;
			break;
		...
		case MotionNotify:
			...
			/* resize before paging request to prevent resize from
			 * lagging mouse - mab */
			...
			if ((delta_x != 0) || (delta_y != 0)) {
				...
				DoResize(x, y, tmp_win, drag, orig, &xmotion,
				    &ymotion);
			}
			done = TRUE;
		default:
		...
		if (!done) {
			...
			DispatchEvent();
			MoveOutline(Scr.Root, drag->x - tmp_win->bw,
			    drag->y - tmp_win->bw,
			    drag->width - 1 + 2 * tmp_win->bw,
			    drag->height - 1 + 2 * tmp_win->bw);
		}
	...
	if (!abort) {
		...
		ConstrainSize(tmp_win, &drag->width, &drag->height, True,
		    xmotion, ymotion);
 ...
 ************************************************************************/
static void
DoResize(int x_root, int y_root, FvwmWindow *tmp_win, geom *drag, geom *orig,
    int *xmotionp, int *ymotionp)
{
	int action = 0;
	...
	if (action) {
		...
		MoveOutline(Scr.Root, drag->x - tmp_win->bw,
		    drag->y - tmp_win->bw, drag->width - 1 + 2 * tmp_win->bw,
		    drag->height - 1 + 2 * tmp_win->bw);
	}
    ...
    FvwmWindow *tmp_win, int width, int height, Bool Init, Bool resetLast)
{
	...
	XDrawString(dpy, Scr.SizeWindow, Scr.StdGC, offset,
	    Scr.StdFont.font->ascent + SIZE_VINDENT, str, 13);
}
...
void
ConstrainSize(FvwmWindow *tmp_win, int *widthp, int *heightp, Bool roundUp,
...
#define _min(a, b) (((a) < (b)) ? (a) : (b))
	...
	dheight =
	    ((dheight - baseHeight + constrainy) / yinc * yinc) + baseHeight;
...
#define minAspectY tmp_win->hints.min_aspect.y
	...
	if (tmp_win->hints.flags & PAspect) {
		...
		if ((maxAspectX * dheight < maxAspectY * dwidth)) {
			delta = makemult(
			    dwidth - maxAspectX * dheight / maxAspectY, xinc);
