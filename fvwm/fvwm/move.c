...
float rgpctMovementDefault[32] = {
  -.01, 0,   .01, .03, .08, .18, .3, .45, .60,
  .75,  .85, .90, .94, .97, .99, 1.0
  /* must end in 1.0 */
};
...
void AnimatedMoveOfWindow(Window w, int startX, int startY, int endX,
                          int endY, Bool fWarpPointerToo, int cmsDelay,
                          float *ppctMovement)
{
  ...
  {
    XGetGeometry(dpy, w, &JunkRoot, &currentX, &currentY, &JunkWidth,
  ...
  do
  {
    ...
    if (fWarpPointerToo == TRUE)
    {
      XQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild, &JunkX,
                    &JunkY, &pointerX, &pointerY, &JunkMask);
                      ...
                      Bool fAnimated, Bool fMoveToPage)
{
  ...
  {
    n =
      GetMoveArguments(action, x, y, width + tmp_win->bw,
  ...
  else /* icon window */
  {
    ...
    tmp_win->icon_xl_loc =
      FinalX - (tmp_win->icon_w_width - tmp_win->icon_p_width) / 2;
    ...
    if (fAnimated)
    {
      AnimatedMoveOfWindow(
        tmp_win->icon_w, -1, -1, tmp_win->icon_xl_loc,
    ...
    if (tmp_win->icon_pixmap_w != None)
    {
      ...
      if (fAnimated)
      {
        AnimatedMoveOfWindow(tmp_win->icon_pixmap_w, -1, -1,
                             tmp_win->icon_x_loc, FinalY, fWarp, -1,
                             NULL);
      }
      else
      {
                          ...
                          int *px, int *py)
{
  ...
  while (Scr.SnapAttraction >= 0 && tmp)
  {
    ...
    if (tmp_win != tmp && (tmp_win->Desk == tmp->Desk))
    {
      ...
      if (!((other.y + other.h) < (*py) || (other.y) > (*py + self.h)))
      {
              ...
              Bool AddWindow)
{
  ...
  if (((!opaque_move) && (!Scr.gs.EmulateMWM)) || (AddWindow))
    MoveOutline(Scr.Root, xl, yt, Width - 1 + 2 * bw,
                Height - 1 + 2 * bw);
      ...
      while (paged <= 1)
      {
        ...
        else
        {
          if (tmp_win->flags & ICONIFIED)
          {
            ...
            tmp_win->icon_xl_loc =
              xl - (tmp_win->icon_w_width - tmp_win->icon_p_width) / 2;
...
void DisplayPosition(FvwmWindow *tmp_win, int x, int y, int Init)
{
  ...
  XDrawString(dpy, Scr.SizeWindow, Scr.StdGC, offset,
              Scr.StdFont.font->ascent + SIZE_VINDENT, str,
              strlen(str));
}
