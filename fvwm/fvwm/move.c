 ...
 * entry */
void
AnimatedMoveOfWindow(Window w, int startX, int startY, int endX, int endY,
    Bool fWarpPointerToo, int cmsDelay, float *ppctMovement)
{
	...
	do {
		...
		if (fWarpPointerToo == TRUE) {
			...
			XWarpPointer(dpy, None, Scr.Root, 0, 0, 0, 0, pointerX,
			    pointerY);
		}
		XFlush(dpy);
 ...
 ****************************************************************************/
void
move_window_doit(XEvent *eventp, Window w, FvwmWindow *tmp_win,
    ...
    Bool fMoveToPage)
{
	...
	XGetGeometry(dpy, w, &JunkRoot, &x, &y, (unsigned int *)&width,
	...
	if (fMoveToPage) {
		...
		n = GetMoveArguments(action, x, y, width + tmp_win->bw,
		    height + tmp_win->bw, &FinalX, &FinalY, &fWarp);
		if (n != 2)
	...
	if (w == tmp_win->frame) {
		...
		BroadcastPacket(M_ICON_LOCATION, 7, tmp_win->w, tmp_win->frame,
		    (unsigned long)tmp_win, tmp_win->icon_x_loc,
		    tmp_win->icon_y_loc, tmp_win->icon_w_width,
		...
		if (tmp_win->icon_pixmap_w != None) {
			...
			if (fAnimated) {
				...
				if (fWarp)
					XWarpPointer(dpy, None, None, 0, 0, 0,
					    0, FinalX - x, FinalY - y);
			}
			XMapWindow(dpy, w);
		}
...
void
move_window(XEvent *eventp, Window w, FvwmWindow *tmp_win,
    unsigned long context, char *action, int *Module)
{
	move_window_doit(
	    eventp, w, tmp_win, context, action, Module, FALSE, FALSE);
}

void
animated_move_window(XEvent *eventp, Window w, FvwmWindow *tmp_win,
    unsigned long context, char *action, int *Module)
{
	move_window_doit(
	    eventp, w, tmp_win, context, action, Module, TRUE, FALSE);
}

void
move_window_to_page(XEvent *eventp, Window w, FvwmWindow *tmp_win,
    unsigned long context, char *action, int *Module)
{
	move_window_doit(
	    eventp, w, tmp_win, context, action, Module, FALSE, TRUE);
}
...
static void
DoSnapAttract(FvwmWindow *tmp_win, int Width, int Height, int *px, int *py)
{
	int nyt, nxl, dist, closestLeft, closestRight, closestBottom,
	    closestTop;
 ...
 ****************************************************************************/
void
moveLoop(FvwmWindow *tmp_win, int XOffset, int YOffset, int Width, int Height,
    int *FinalX, int *FinalY, Bool opaque_move, Bool AddWindow)
{
	Bool	     finished = False;
	Bool	     done;
	...
	XQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild, &xl, &yt, &JunkX,
	    &JunkY, &button_mask);
	button_mask &=
	...
	if (((!opaque_move) && (!Scr.gs.EmulateMWM)) || (AddWindow))
		MoveOutline(
		    Scr.Root, xl, yt, Width - 1 + 2 * bw, Height - 1 + 2 * bw);
	...
	while (!finished) {
		...
		case ButtonPress:
			...
			if (((Event.xbutton.button == 2) &&
				 ...
				 * another button was pressed during the
				 * operation
				 *  - no button was started at the beginning and
				 * any button except button 1 was pressed. */
				if (button_mask ||
				    (Event.xbutton.button != 1)) {
					if (!opaque_move)
						MoveOutline(
		...
		case MotionNotify:
			...
			/* check Paging request once and only once after outline
			 * redrawn */
			/* redraw after paging if needed - mab */
			paged = 0;
			while (paged <= 1) {
				if (!opaque_move)
					MoveOutline(Scr.Root, xl, yt,
					    Width - 1 + 2 * bw,
					    Height - 1 + 2 * bw);
				else {
					if (tmp_win->flags & ICONIFIED) {
				...
				/* prevent window from lagging behind mouse when
				 * paging - mab */
				if (paged == 0) {
					int dx;
					int dy;
...
void
DisplayPosition(FvwmWindow *tmp_win, int x, int y, int Init)
{
	char str[100];
	int  offset;
 ...
 ****************************************************************************/
void
Keyboard_shortcuts(XEvent *Event, FvwmWindow *w, int ReturnEvent)
{
	...
	XQueryPointer(dpy, Scr.Root, &JunkRoot, &Event->xany.window, &x_root,
	    &y_root, &x, &y, &JunkMask);
    ...
    Window *win, FvwmWindow *tmp_win, int *FinalX, int *FinalY, XEvent *eventp)
{
	...
	if (menuFromFrameOrWindowOrTitlebar) {
		/* warp the pointer to the cursor position from before menu
		 * appeared*/
		XFlush(dpy);
	}

	/* Although a move is usually done with a button depressed we have to
	 * check for ButtonRelease too since the event may be faked. */
	if (eventp->type == ButtonPress || eventp->type == ButtonRelease) {
		DragX = eventp->xbutton.x_root;
		DragY = eventp->xbutton.y_root;
	} else
		XQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild, &DragX,
		    &DragY, &JunkX, &JunkY, &JunkMask);
	...
	moveLoop(tmp_win, XOffset, YOffset, DragWidth, DragHeight, FinalX,
	    FinalY, opaque_move, False);
