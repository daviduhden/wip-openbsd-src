    ...
    FvwmWindow *t, unsigned long tflags, int border_width, int resize_width)
{
	...
	if (!(decor & MWM_DECOR_MINIMIZE)) {
		...
		int i;
		for (i = 0; i < 5; ++i) {
			if (GetDecor(t, left_buttons[i].flags) &
			    MWMDecorMinimize)
				t->left_w[i] = None;
			if (GetDecor(t, right_buttons[i].flags) &
			    MWMDecorMinimize)
				t->right_w[i] = None;
		}
	}
...
check_if_function_allowed(int function, FvwmWindow *t, MenuItem *mi)
{
	if (t) /* should always be ok */
	{
		...
		case F_MAXIMIZE:
			...
			break;
		case F_MOVE:
			/* Move is a funny hint. Keeps it out of the menu, but
		...
		case F_FUNCTION:
			...
			/* Complex functions are a little tricky, ignore them if
			 * no menu item*/
			if (mi && mi->item) {
