...
*/
  ...
  if ((Fw) && Fw->tmpflags.NeverFocus)
    {
      /* Explicitly never focus this window per style. Fall through to set NoFocus. */
      XSetInputFocus (dpy, Scr.NoFocusWin, RevertToParent, lastTimestamp);
      Scr.Focus = NULL;
  ...
  /* ICCCM: WM_TAKE_FOCUS must be sent to the client toplevel window. */
  if (Fw && !Fw->tmpflags.NeverFocus && (Fw->flags & DoesWmTakeFocus))
    send_clientmessage(dpy, Fw->w, _XA_WM_TAKE_FOCUS, lastTimestamp);
