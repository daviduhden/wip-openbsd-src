...
void CreateWindow(int x, int y, int w, int h)
{
  ...
  mysizehints.flags = PWinGravity | PResizeInc | PBaseSize | PMaxSize |
                      PMinSize | USSize | USPosition;
  ...
  holder_win = XCreateSimpleWindow(
    dpy, main_win, PAD_WIDTH3, PAD_WIDTH3,
    mysizehints.width - BAR_WIDTH - PAD_WIDTH3,
    mysizehints.height - BAR_WIDTH - PAD_WIDTH3, 0, fore_pix, back_pix);
  ...
  while (1)
  {
    ...
    case ButtonPress:
               ...
               (Event.xbutton.x < Width - BAR_WIDTH))
      {
        motion = HORIZONTAL;
        target_x_offset =
          (Event.xbutton.x - PAD_WIDTH3 - SCROLL_BAR_WIDTH) *
          target_width /
        ...
        if (target_x_offset + Width - BAR_WIDTH - PAD_WIDTH3 >
            target_width)
          target_x_offset =
            target_width - Width + BAR_WIDTH + PAD_WIDTH3;
        XMoveWindow(dpy, target, -target_x_offset, -target_y_offset);
        RedrawWindow(target);
      }
      else if ((Event.xbutton.y > Height - BAR_WIDTH) &&
    ...
    case ButtonRelease:
      if ((Event.xbutton.y > Height - BAR_WIDTH) &&
          (Event.xbutton.x > Width - BAR_WIDTH) && (motion == QUIT))
      {
        XUnmapWindow(dpy, main_win);
        {
          int root_x, root_y;
          Window dummy;
          if (XTranslateCoordinates(dpy, main_win, Root, 0, 0, &root_x,
                                    &root_y, &dummy))
          {
    ...
    case MotionNotify:
      ...
      {
        target_y_offset =
          (Event.xmotion.y - PAD_WIDTH3 - SCROLL_BAR_WIDTH) *
          target_height /
          (Height - BAR_WIDTH - PAD_WIDTH3 - 2 * SCROLL_BAR_WIDTH);
        if (target_y_offset + Height - BAR_WIDTH - PAD_WIDTH3 >
            target_height)
      ...
      {
        target_x_offset =
          (Event.xmotion.x - PAD_WIDTH3 - SCROLL_BAR_WIDTH) *
          target_width /
          (Width - BAR_WIDTH - PAD_WIDTH3 - 2 * SCROLL_BAR_WIDTH);
        if (target_x_offset < 0)
          target_x_offset = 0;
      ...
      if ((motion == QUIT) && ((Event.xbutton.y < Height - BAR_WIDTH) ||
                               (Event.xbutton.x < Width - BAR_WIDTH)))
      {
        motion = NONE;
        exposed = 2;
    ...
    case PropertyNotify:
      ...
      else if (Event.xproperty.atom == XA_WM_ICON_NAME)
      {
        if (XGetWindowProperty(
              dpy, target, Event.xproperty.atom, 0, MAX_ICON_NAME_LEN,
              False, XA_STRING, &actual, &actual_format, &nitems,
...
void RedrawWindow(Window target)
{
  ...
  RelieveWindow(main_win, PAD_WIDTH3 - 2, PAD_WIDTH3 - 2,
                ...
                Height - BAR_WIDTH - PAD_WIDTH3 + 4, ShadowGC,
                ReliefGC);
  ...
  h = (Height - BAR_WIDTH - PAD_WIDTH3 - 2 * SCROLL_BAR_WIDTH) *
      (Height - BAR_WIDTH - PAD_WIDTH3) / target_height;
  if ((y != yv) || (x != xv) || (w != wv) || (h != hv) || (exposed & 1))
  {
    ...
    wv = w;
    hv = h;
    XClearArea(dpy, main_win, x, PAD_WIDTH3 + SCROLL_BAR_WIDTH, w,
               Height - BAR_WIDTH - PAD_WIDTH3 - 2 * SCROLL_BAR_WIDTH,
               False);
  ...
  if (exposed & 1)
    RelieveWindow(main_win, x - 2, PAD_WIDTH2, w + 4,
                  Height - BAR_WIDTH - PAD_WIDTH2 + 2, ShadowGC,
                  ReliefGC);
  if (exposed)
  {
    if (motion == TOP)
  ...
  if ((y != yh) || (x != xh) || (w != wh) || (h != hh) || (exposed & 1))
  {
    ...
    XClearArea(dpy, main_win, PAD_WIDTH3 + SCROLL_BAR_WIDTH, y,
               Width - BAR_WIDTH - PAD_WIDTH3 - 2 * SCROLL_BAR_WIDTH, h,
               False);
    RelieveWindow(main_win, x, y, w, h, ReliefGC, ShadowGC);
  }
  if (exposed & 1)
  {
    RelieveWindow(main_win, PAD_WIDTH2, y - 2,
                  Width - BAR_WIDTH - PAD_WIDTH2 + 2, h + 4, ShadowGC,
                  ReliefGC);
  }
  if (exposed)
  {
  ...
  if (exposed)
  {
    XClearArea(dpy, main_win, Width - BAR_WIDTH + 2,
               Height - BAR_WIDTH + 2, BAR_WIDTH - 3, BAR_WIDTH - 3,
               False);
    if (motion == QUIT)
...
void GrabWindow(Window target)
{
  ...
  XSync(dpy, 0);
  XGetGeometry(dpy, target, &root, &x, &y, (unsigned int *)&tw,
               (unsigned int *)&th, (unsigned int *)&border_width,
               (unsigned int *)&depth);
  XSync(dpy, 0);
