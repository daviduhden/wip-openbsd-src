	...
	while (1) {
		...
		case ConfigureNotify:
			...
			if ((tw != Width) || (th != Height)) {
				XResizeWindow(dpy, holder_win,
				    tw - BAR_WIDTH - PAD_WIDTH3,
				...
				if (target_y_offset + Height - BAR_WIDTH >
				    target_height)
					target_y_offset =
					    target_height - Height + BAR_WIDTH;
		...
		case FocusIn:
			XSetInputFocus(
			    dpy, target, RevertToParent, CurrentTime);
			break;
		case ColormapNotify: {
...
RedrawWindow(Window target)
{
	...
	if (exposed & 1)
		RelieveWindow(main_win, x - 2, PAD_WIDTH2, w + 4,
		    Height - BAR_WIDTH - PAD_WIDTH2 + 2, ShadowGC, ReliefGC);
	if (exposed) {
		if (motion == TOP)
	...
	if (exposed) {
		XClearArea(dpy, main_win, Width - BAR_WIDTH + 2,
		    ...
		    False);
		if (motion == QUIT)
			RelieveWindow(main_win,
			    Width - SCROLL_BAR_WIDTH - PAD_WIDTH2 - 4,
