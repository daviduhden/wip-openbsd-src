...
static int count_nonsticky_in_hashtab(void *arg)
{
  ...
  if (!(win->fvwm_flags & STICKY) && win->complete &&
      win->manager == man)
    return 1;
  return 0;
}
...
int win_in_viewport(WinData *win)
{
  ...
  case SHOW_PAGE:
    ...
    else if (win->desknum == globals.desknum)
    {
      ...
      flag =
        RECTANGLES_INTERSECT(win->x, win->y, win->width, win->height, 0,
                             0, globals.screenx, globals.screeny);
    }
    break;
  }
  return flag;
