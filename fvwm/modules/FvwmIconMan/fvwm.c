...
static void
set_draw_mode(WinManager *man, int flag)
{
	int num;
...
static void
got_configure(WinManager *man)
{
	if (man && !man->we_are_drawing) {
		man->configures_expected--;
		ConsoleDebug(FVWM,
		    "got_configure on 0x%lx, num_expected now = %d\n",
...
win_in_viewport(WinData *win)
{
	...
	case SHOW_PAGE:
		if (win->fvwm_flags & STICKY) {
			...
			/* win and screen intersect if they are not disjoint in
			 * x and y */
			flag = RECTANGLES_INTERSECT(win->x, win->y, win->width,
			    win->height, 0, 0, globals.screenx,
			    globals.screeny);
		}
		break;
	}
...
static void
set_win_configuration(WinData *win, FvwmPacketBody *body)
{
...
focus_change(FvwmPacketBody *body)
{
	...
	if (win->complete && win->button && win->manager &&
	    win->manager->window_up && win->manager->followFocus) {
...
icon_name(FvwmPacketBody *body)
{
	...
	if (win->iconname && !strcmp((char *)win->iconname, (char *)name)) {
		ConsoleDebug(
...
window_name(FvwmPacketBody *body)
{
	...
	if (win->titlename && !strcmp((char *)win->titlename, (char *)name)) {
		ConsoleDebug(
...
static void
new_window(FvwmPacketBody *body)
{
	WinData *win;
...
static void
update_win_in_hashtab(void *arg)
{
...
static void
ProcessMessage(Ulong type, FvwmPacketBody *body)
{
	int i;
