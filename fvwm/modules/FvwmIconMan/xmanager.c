...
typedef struct {
	int button_x, button_y, button_h,
	    button_w;			    /* dim's of the whole button */
...
static void
ClipRectangle(WinManager *man, int context, int x, int y, int w, int h)
{
	XRectangle r;
	...
	XSetClipRectangles(
	    theDisplay, man->hiContext[context], 0, 0, &r, 1, YXBanded);
}

static int
num_visible_rows(int n, int cols)
{
	return (n - 1) / cols + 1;
}

static int
first_row_len(int n, int cols)
{
	int ret = n % cols;
	if (ret == 0)
		ret = cols;
...
static int
index_to_box(WinManager *man, int index)
{
	...
	if (man->geometry.dir & GROW_DOWN) {
		...
		index +=
		    (man->geometry.rows - num_visible_rows(n, cols)) * cols;
		return index;
	}
}

static int
box_to_index(WinManager *man, int box)
{
	int first_len, n, cols;
...
static int
index_to_row(WinManager *man, int index)
{
	int row;
...
static int
index_to_col(WinManager *man, int index)
{
	int col;
...
static ManGeometry *
figure_geometry(WinManager *man)
{
	...
	ConsoleDebug(X11, "figure_geometry: %s: %d, %d %d %d %d\n",
	    man->titlename, n, ret.width, ret.height, ret.cols, ret.rows);
	...
	ConsoleDebug(X11, "figure_geometry: %d %d %d %d %d\n", n, ret.width,
	    ret.height, ret.cols, ret.rows);

	return &ret;
}

static ManGeometry *
query_geometry(WinManager *man)
{
...
static void
fix_manager_size(WinManager *man, int w, int h)
{
	XSizeHints size;
	long	   mask = 0;
...
resize_window(WinManager *man)
{
	...
	if ((dir & GROW_DOWN) && (dir & GROW_RIGHT)) {
		...
		if (dir & GROW_UP) {
			man->geometry.y =
			    g->y + g->height - man->geometry.height;
			y_changed = 1;
		} else {
			man->geometry.y = g->y;
		}
...
Button *
button_above(WinManager *man, Button *b)
{
...
Button *
button_below(WinManager *man, Button *b)
{
	int n = man->buttons.num_windows;
	int i = -1;
	...
	if (b) {
		i = box_to_index(
		    man, index_to_box(man, b->index) + man->geometry.cols);
	}
	if (i < 0 || i >= n)
		return b;
	else
...
Button *
button_right(WinManager *man, Button *b)
{
	int i = 0;
...
Button *
button_next(WinManager *man, Button *b)
{
	int i = b->index + 1;
...
Button *
button_prev(WinManager *man, Button *b)
{
	int i = b->index - 1;
...
Button *
xy_to_button(WinManager *man, int x, int y)
{
	int row = y / man->geometry.boxheight;
	int col = x / man->geometry.boxwidth;
	int box, index;

	if (x >= 0 && x <= man->geometry.width && y >= 0 &&
	    y <= man->geometry.height) {
...
static void
set_button_geometry(WinManager *man, Button *box)
{
...
static void
set_window_button(WinData *win, int index)
{
	Button *b;
...
set_num_buttons(ButtonArray *buttons, int n)
{
	...
	if (n > buttons->num_buttons) {
		...
		buttons->buttons = (Button **)xreallocarray(
		    buttons->buttons, n, sizeof(Button *));

		for (i = buttons->num_buttons; i < n; i++) {
			buttons->buttons[i] = (Button *)xmalloc(sizeof(Button));
...
static void
increase_num_windows(ButtonArray *buttons, int off)
{
	int n;
...
static void
set_man_gravity_origin(WinManager *man)
{
	if (man->gravity == NorthWestGravity ||
...
static void
set_man_geometry(WinManager *man, ManGeometry *new)
{
	int n;
...
void
set_manager_width(WinManager *man, int width)
{
...
void
force_manager_redraw(WinManager *man)
{
...
void
set_win_picture(WinData *win, Pixmap picture, Pixmap mask, unsigned int depth,
...
void
set_win_iconified(WinData *win, int iconified)
{
...
void
set_win_state(WinData *win, int state)
{
...
void
add_win_state(WinData *win, int flag)
{
...
void
del_win_state(WinData *win, int flag)
{
...
void
set_win_displaystring(WinData *win)
{
	...
	if (man->window_up) {
		...
		maxlen =
		    man->geometry.width / man->fontwidth + 2 /* fudge factor */;
	} else {
		maxlen = 0;
	}
...
static void
clear_empty_region(WinManager *man)
{
	XRectangle rects[2];
	int	   num_rects = 0, n = man->buttons.num_windows,
		   cols = man->geometry.cols;
	...
	if (n == 0 || rows * cols == 0 /* just be to safe */) {
		...
		if (n % cols == 0) {
			rects[0].x = 0;
			rects[0].y =
			    num_visible_rows(n, cols) * man->geometry.boxheight;
			rects[0].width = man->geometry.width;
...
void
set_manager_window_mapping(WinManager *man, int flag)
{
...
void
init_button_array(ButtonArray *array)
{
	array->num_buttons = 0;
	array->num_windows = 0;
	array->buttons = NULL;
...
static void
resize_manager(WinManager *man, int force)
{
	ManGeometry *new;
...
static int
center_padding(int h1, int h2)
{
	return (h2 - h1) / 2;
}

static void
get_title_geometry(WinManager *man, ButtonGeometry *g)
{
	int text_pad;
	assert(man);
	g->button_x = 0;
...
static void
get_button_geometry(WinManager *man, Button *button, ButtonGeometry *g)
{
...
static void
draw_3d_square(WinManager *man, int x, int y, int w, int h, GC rgc, GC sgc)
{
	int	 i;
	XSegment seg[4];
    ...
    int dir, Contexts contextId)
{
	...
	if (iconified == 0) {
		...
		if (dir == 1) {
			draw_3d_square(man, g->icon_x, g->icon_y, g->icon_w,
			    g->icon_h, man->reliefContext[contextId],
			    man->shadowContext[contextId]);
		} else {
			draw_3d_square(man, g->icon_x, g->icon_y, g->icon_w,
			    g->icon_h, man->shadowContext[contextId],
...
static void
iconify_box(WinManager *man, WinData *win, int box, ButtonGeometry *g,
    int iconified, Contexts contextId, int button_already_cleared)
{
#ifdef MINI_ICONS
	XGCValues     gcv;
	unsigned long gcm;
...
/* [BV 16-Apr-97] Mini Icons work on black-and-white too */
	...
	if (man->draw_icons && win->pic.picture) {
		if (iconified == 0 && man->draw_icons != 2) {
			if (!button_already_cleared) {
				XFillRectangle(theDisplay, man->theWindow,
				    man->backContext[contextId], g->icon_x,
				    g->icon_y, g->icon_w, g->icon_h);
			}
		} else {
			...
			XChangeGC(
			    theDisplay, man->hiContext[contextId], gcm, &gcv);

			XCopyArea(theDisplay, win->pic.picture, man->theWindow,
			    man->hiContext[contextId], 0, 0, g->icon_w,
			    g->icon_h, g->icon_x, g->icon_y);
			gcm = GCClipMask;
			gcv.clip_mask = None;
			XChangeGC(
			    theDisplay, man->hiContext[contextId], gcm, &gcv);
		...
		if (theDepth > 2) {
			...
			if (iconified == 0) {
				XFillArc(theDisplay, man->theWindow,
				    man->backContext[contextId], g->icon_x,
				    g->icon_y, g->icon_w, g->icon_h, 0,
				    360 * 64);
			} else {
				XFillArc(theDisplay, man->theWindow,
				    man->hiContext[contextId], g->icon_x,
				    g->icon_y, g->icon_w, g->icon_h, 0,
				    360 * 64);
			}
		}
...
int
change_windows_manager(WinData *win)
{
	WinManager *oldman;
	WinManager *newman;
	...
	check_in_window(win);
	ConsoleDebug(
	    X11, "change_windows_manager: returning %d\n", newman != oldman);
	return (newman != oldman);
}

void
check_in_window(WinData *win)
{
	int in_viewport;
...
static void
get_gcs(WinManager *man, int state, GC *context1, GC *context2)
{
...
static void
draw_relief(WinManager *man, int button_state, ButtonGeometry *g, GC context1,
    GC context2)
{
	int state;
	...
	if (state == BUTTON_EDGEUP || state == BUTTON_EDGEDOWN) {
		draw_3d_square(man, g->button_x, g->button_y, g->button_w,
		    g->button_h, context1, context2);
		draw_3d_square(man, g->button_x + 2, g->button_y + 2,
		    g->button_w - 4, g->button_h - 4, context2, context1);
	} else {
		draw_3d_square(man, g->button_x, g->button_y, g->button_w,
		    g->button_h, context1, context2);
	}
...
static void
draw_button(WinManager *man, int button, int force)
{
	Button	      *b;
	WinData	      *win;
	...
	if (dirty) {
		...
		if (win) {
			if (dirty & GEOMETRY_CHANGED) {
				...
				/* Determine if geometry has changed relative to
				   the window gravity */
	...
	if (win && (draw_background || draw_icon || draw_string)) {
		...
		if (draw_background) {
			...
			if (theDepth > 2) {
				get_gcs(
				    man, button_state, &context1, &context2);
				draw_relief(
				    man, button_state, &g, context1, context2);
		...
		if (draw_icon) {
			...
			iconify_box(man, win, button, &g, win->iconified,
			    button_state, cleared_button);
		}
		if (draw_string) {
			ConsoleDebug(X11, "\tDrawing text: %s\n",
			    b->drawn_state.display_string);
			ClipRectangle(man, button_state, g.text_x, g.text_y,
			    g.text_w, g.text_h);
			if (!cleared_button) {
				XFillRectangle(theDisplay, man->theWindow,
				    man->backContext[button_state], g.text_x,
				    g.text_y, g.text_w, g.text_h);
			}
			XDrawString(theDisplay, man->theWindow,
			    man->hiContext[button_state], g.text_x, g.text_base,
			    b->drawn_state.display_string,
...
static void
draw_empty_manager(WinManager *man)
{
	GC	       context1, context2;
	...
	XDrawString(theDisplay, man->theWindow, man->hiContext[state], g.text_x,
	    g.text_base, man->titlename, strlen(man->titlename));
	XSetClipMask(theDisplay, man->hiContext[state], None);
}

void
draw_manager(WinManager *man)
{
	...
	if (redraw_all || (man->dirty_flags & MAPPING_CHANGED)) {
		force_draw = 1;
		ConsoleDebug(
		    X11, "manager %s: mapping changed\n", man->titlename);
	}
#ifdef SHAPE

	if (man->shaped && (redraw_all || (man->dirty_flags & SHAPE_CHANGED))) {
		/* This little piggie waits until past resize requests get
		 * processed */
		XSync(theDisplay, False);
		XShapeCombineRectangles(theDisplay, man->theWindow,
		    ShapeBounding, 0, 0, man->shape.rects, man->shape.num_rects,
		    ShapeSet, Unsorted);
		XShapeCombineRectangles(theDisplay, man->theWindow, ShapeClip,
		    0, 0, man->shape.rects, man->shape.num_rects, ShapeSet,
		    Unsorted);
		shape_changed = 1;
		update_geometry = 1;
		/* And this little piggie waits for shape to get processed
		   before drawing buttons */
		XSync(theDisplay, False);
	}
#endif
	...
	if (man->buttons.num_windows == 0) {
		...
		/* I was having the problem where when the shape changed the
		   manager wouldn't get redrawn. It appears we weren't getting
		   the expose. How can I tell when I am going to reliably get an
		   expose event? */
...
static int
compare_windows(SortType type, WinData *a, WinData *b)
{
	if (type == SortId) {
...
find_windows_spot(WinData *win)
{
	...
	if (man->sort != SortNone) {
		...
		for (i = start; i != finish && bp[i]->drawn_state.win &&
		    cmp_dir *
			    compare_windows(
				man->sort, win, bp[i]->drawn_state.win) >
			0;
		    i = i + cmp_dir)
			;
		i += correction;
		ConsoleDebug(
		    X11, "find_windows_spot: %s %d\n", win->display_string, i);
		return i;
	} else {
		if (win->button) {
...
static void
move_window_buttons(WinManager *man, int start, int finish, int offset)
{
	...
	if (finish >= n || finish + offset >= n || start < 0 ||
	    start + offset < 0) {
		ConsoleMessage("Internal error in move_window_buttons\n");
		ConsoleMessage(
		    "\tn = %d, start = %d, finish = %d, offset = %d\n", n,
		    start, finish, offset);
		return;
	}
...
static void
insert_windows_button(WinData *win)
{
	int	     spot;
...
void
delete_windows_button(WinData *win)
{
	int	     spot;
	int	     selected_index = -1;
	WinManager  *man = (win->manager);
...
void
resort_windows_button(WinData *win)
{
	...
	if (new_spot != cur_spot) {
		...
		if (selected_index >= 0) {
			move_highlight(
			    man, man->buttons.buttons[selected_index]);
		}
	}
}

void
move_highlight(WinManager *man, Button *b)
{
	WinData *old;
...
void
man_exposed(WinManager *man, XEvent *theEvent)
{
	int	 x1, y1, w1, h1;
	int	 x2, y2, w2, h2;
	int	 i;
	...
	/* There's some weird problem where if we change window shapes, we can't
	   ...
	   get Expose events. This has the disadvantage of drawing buttons
	   twice, but avoids having to match which expose event results from
	   which shape change */
...
void
check_managers_consistency(void)
{
#ifdef PRINT_DEBUG
	int	 i, j;
	Button **b;
	...
	for (i = 0; i < globals.num_managers; i++) {
		for (j = 0, b = globals.managers[i].buttons.buttons;
			...
			if ((*b)->drawn_state.win &&
			    (*b)->drawn_state.win->button != *b) {
				ConsoleMessage(
				    "manager %d, button %d is confused\n", i,
				    j);
				abort();
...
print_button_info(Button *b)
{
	...
	if (b->drawn_state.win) {
		...
		ConsoleMessage("iconified: %d state %d\n",
		    b->drawn_state.iconified, b->drawn_state.state);
		ConsoleMessage(
