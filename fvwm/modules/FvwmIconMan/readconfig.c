...
static int num_builtins =
  sizeof(builtin_functions) / sizeof(FunctionType);
...
static void find_context(char *string, int *output,
                         struct charstring *table, char *tline)
{
  int i = 0, j = 0;
  Bool matched;
  char tmp1;
...
static Function *parse_function(char **line, char *pstop_char)
{
  ...
  builtin_functions_i =
    bsearch((void *)name, (void *)builtin_functions, num_builtins,
            sizeof(FunctionType), funccasecmp);
  if (builtin_functions_i)
  {
    ...
    for (j = 0; j < builtin_functions_i->numargs && *pstop_char != ',';
         j++)
    {
      ...
      case StringArg:
        ptr = DoGetNextToken(ptr, &ftype->args[j].value.string_value,
                             NULL, ",", pstop_char);
...
Binding *ParseMouseEntry(char *tline)
{
  ...
  {
    ConsoleMessage("Binding specified AnyModifier and other modifers "
  ...
  while ((p = read_next_cmd(READ_LINE)))
  {
    ...
    else
    {
      ...
      if (option1[0] >= '0' && option1[0] <= '9')
      {
        ...
        if (extract_int(option1, &manager) == 0 || manager <= 0 ||
      ...
      if (!strcasecmp(option1, "action"))
      {
        ...
        {
          add_to_binding(&globals.managers[manager].bindings[i],
                         binding);
        }
        else
        {
        ...
        for (i = 0; i < NUM_CONTEXTS; i++)
          SET_MANAGER(manager, backColorName[i],
                      conditional_copy_string(
                        &globals.managers[id].backColorName[i], p));
      }
      ...
      else if (!strcasecmp(option1, "dontshow"))
      {
        ...
        do
        {
          ...
          {
            add_to_stringlist(&globals.managers[manager].dontshow,
                              token);
          }
          Free(token);
          p = DoGetNextToken(p, &token, NULL, ",", NULL);
      ...
      else if (!strcasecmp(option1, "foreground"))
      {
        ...
        for (i = 0; i < NUM_CONTEXTS; i++)
          SET_MANAGER(manager, foreColorName[i],
                      conditional_copy_string(
                        &globals.managers[id].foreColorName[i], p));
      }
      ...
      else if (!strcasecmp(option1, "showtitle"))
      {
        ConsoleMessage("Bad line: %s\n", current_line);
        ConsoleMessage(
          "showtitle is no longer an option. Use format\n");
        continue;
      }
