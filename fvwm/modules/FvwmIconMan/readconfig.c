...
print_args(int numargs, BuiltinArg *args)
{
	...
	for (i = 0; i < numargs; i++) {
		...
		case StringArg:
			ConsoleDebug(
			    CONFIG, "String: %s ", args[i].value.string_value);
			break;
...
print_binding(Binding *binding)
{
	...
	while (func) {
		...
		if (i > num_builtins) {
			ConsoleDebug(
			    CONFIG, "\tFunction: not found %p ", func->func);
		}
...
static void
add_to_binding(Binding **list, Binding *binding)
{
 ...
 */
static int
builtin_label(int numargs, BuiltinArg *args)
{
	int j;
...
parse_function(char **line, char *pstop_char)
{
	...
	builtin_functions_i = bsearch((void *)name, (void *)builtin_functions,
	    num_builtins, sizeof(FunctionType), funccasecmp);
	if (builtin_functions_i) {
		...
		for (j = 0;
			...
			case ManagerArg:
				ptr = parse_button(
				    ptr, &ftype->args[j], &flag, pstop_char);
				if (!flag) {
					ConsoleMessage(
					    "%s: too few arguments\n",
					    builtin_functions_i->name);
				...
				/* JmpArg can be a string or an int. However, if
				 * 'JmpArg' is recorded as the argument type in
				 * the argument array for a command, it means it
				 * is a string; the code for 'IntArg' is used
				 * instead for numbers. Note also that if the C
				 * function recieves a 'JmpArg' argument it
				 * means something went wrong, since they should
				 * all be translated to integer jump offsets at
				 * compile time.
				 */
			case JmpArg:
				ptr = DoGetNextToken(
		...
		for (f = tail; f; f = f->prev) {
			for (j = 0; j < (f->numargs); ++j) {
				if (f->args[j].type == JmpArg) {
					ConsoleMessage(
					    "Attempt to jump to non-existant "
					    "label %s; "
...
ParseMouseEntry(char *tline)
{
	...
	if ((mods & AnyModifier) && (mods & (~AnyModifier))) {
		ConsoleMessage(
		    "Binding specified AnyModifier and other modifers "
...
void
run_binding(WinManager *man, Action action)
{
...
static int
GetConfigLineWrapper(int *fd, char **tline)
{
#if FVWM_VERSION == 1
...
static char *
conditional_copy_string(char **s1, char *s2)
{
	if (*s1)
		return *s1;
	else
...
read_in_resources(char *file)
{
	...
	while ((p = read_next_cmd(READ_LINE))) {
		...
		if (!strcasecmp(option1, "nummanagers")) {
			/* If in transient mode, just use the default of 1
			 * manager */
			if (!globals.transient) {
				...
				if (n > 0) {
					allocate_managers(n);
					ConsoleDebug(
					    CONFIG, "num managers: %d\n", n);
				} else {
					ConsoleMessage(
					    "Bad line: %s\n", current_line);
					ConsoleMessage(
					    "You can't have zero managers. "
					    "I'll give you one.\n");
					allocate_managers(1);
				}
			}
			...
			if (!strcasecmp(option1, "action")) {
				...
				if (manager == -1) {
					int j;
					for (j = 0; j < globals.num_managers;
					    j++) {
						add_to_binding(
						    &globals.managers[j]
						        .bindings[i],
				...
				if (!p) {
					ConsoleMessage(
					    "Bad line: %s\n", current_line);
					ConsoleMessage(
					    "Need argument to drawicons\n");
					continue;
				}
				if (!strcasecmp(p, "true")) {
					i = 1;
				}
				/* [NFM 3 Dec 97] added support for
				...
				ConsoleDebug(
				    CONFIG, "Setting drawicons to: %d\n", i);
				SET_MANAGER(manager, draw_icons, i);
#else
				ConsoleMessage(
				    "DrawIcons support not compiled in\n");
#endif
				...
				ConsoleDebug(
				    CONFIG, "Setting followfocus to: %d\n", i);
				...
				handle_button_config(
				    manager, PLAIN_CONTEXT, option1);
			} else if (!strcasecmp(option1, "selectButton")) {
				handle_button_config(
				    manager, SELECT_CONTEXT, option1);
				...
				ConsoleDebug(
				    CONFIG, "Setting usewinlist to: %d\n", i);
