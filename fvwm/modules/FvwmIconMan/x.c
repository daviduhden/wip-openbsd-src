...
find_frame_window(Window win, int *off_x, int *off_y)
{
	...
	while (1) {
		...
		ConsoleDebug(X11, "Adding (%d, %d) += (%d, %d)\n", *off_x,
		    *off_y, attr.x + attr.border_width,
...
xevent_loop(void)
{
	...
	while (XPending(theDisplay)) {
		...
		if (!man) {
			ConsoleDebug(
			    X11, "Event doesn't belong to a manager\n");
			continue;
		}
		...
		case KeyPress:
			...
			/* Here's a real hack - some systems have two keys with
			 * the same keysym and different keycodes. This converts
		...
		case EnterNotify:
			...
			b = xy_to_button(
			    man, theEvent.xcrossing.x, theEvent.xcrossing.y);
		...
		case ConfigureNotify:
			ConsoleDebug(X11,
			    "XEVENT: Configure Notify: %d %d %d %d\n",
			    theEvent.xconfigure.x, theEvent.xconfigure.y,
			    theEvent.xconfigure.width,
		...
		case MotionNotify:
			...
			b = xy_to_button(
			    man, theEvent.xmotion.x, theEvent.xmotion.y);
...
static void
set_window_properties(Window win, char *name, char *icon, XSizeHints *sizehints)
{
...
static int
load_default_context_fore(WinManager *man, int i)
{
	int j = 0;
	...
	return lookup_color(
	    contextDefaults[i].forecolor[j], &man->forecolor[i]);
}

static int
load_default_context_back(WinManager *man, int i)
{
	int j = 0;
	...
	return lookup_color(
	    contextDefaults[i].backcolor[j], &man->backcolor[i]);
}

void
map_manager(WinManager *man)
{
	...
	if (man->sizehints_flags & USPosition) {
		if (man->geometry.dir & GROW_DOWN) {
			if (man->geometry.dir & GROW_RIGHT)
				...
				ConsoleMessage(
				    "Internal error in X_init_manager\n");
...
create_manager_window(int man_id)
{
	...
	ConsoleDebug(X11, "hints: x, y, w, h = %d %d %d %d)\n", sizehints.x,
	    sizehints.y, sizehints.base_width, sizehints.base_height);
	ConsoleDebug(
	    X11, "gravity: %d %d\n", sizehints.win_gravity, man->gravity);
	...
	XChangeWindowAttributes(
	    theDisplay, man->theWindow, CWBitGravity, &winattr);
	set_shape(man);
	...
	for (i = 0; i < NUM_CONTEXTS; i++) {
		...
		XSetForeground(
		    theDisplay, man->hiContext[i], man->forecolor[i]);
		...
		if (theDepth > 2) {
			...
			man->reliefContext[i] = XCreateGC(
			    theDisplay, man->theWindow, gcmask, &gcval);

			gcmask = GCForeground | GCBackground;
			gcval.foreground = man->shadowcolor[i];
			gcval.background = man->backcolor[i];
			man->shadowContext[i] = XCreateGC(
			    theDisplay, man->theWindow, gcmask, &gcval);
		}
	}

	set_window_properties(
	    man->theWindow, man->titlename, man->iconname, &sizehints);
	man->window_up = 1;
	map_manager(man);
}
...
#ifdef SHAPE
	globals.shapes_supported =
	    XShapeQueryExtension(theDisplay, &shapeEventBase, &shapeErrorBase);
#endif
