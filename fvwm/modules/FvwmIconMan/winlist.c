...
add_to_stringlist(StringList *list, char *s)
{
	...
	if (pat) {
		*pat++ = '\0';
		if (!strcasecmp(s, "icon"))
			...
			ConsoleMessage(
			    "Bad element in show/dontshow list: %s\n", s);
			return;
		}
	} else {
		pat = s;
	...
	new = (StringEl *)xmalloc(sizeof(StringEl));
	size_t pat_len = strlen(pat);
	new->string = (char *)xmalloc((pat_len + 1) * sizeof(char));
	new->type = type;
...
static int
stringlist_requires_name(const StringList *list, NameType type)
{
	StringEl *p;
...
static int
matches_string(NameType type, char *pattern, char *tname, char *iname,
    char *rname, char *cname)
{
	int ans = 0;

	ConsoleDebug(
	    WINLIST, "matches_string: type: 0x%x pattern: %s\n", type, pattern);
	ConsoleDebug(
	    WINLIST, "\tstrings: %s:%s %s:%s\n", tname, iname, rname, cname);
...
new_windata(void)
{
	WinData *new = (WinData *)xmalloc(sizeof(WinData));
...
check_win_complete(WinData *p)
{
	...
	ConsoleDebug(WINLIST, "\tdisplaystring: %s\n",
	    (p->display_string ? p->display_string : "No p->display_string"));
...
insert_win_hashtab(WinData *win)
{
	...
	for (p = list->head; p && win->app_id > p->app_id; p = p->win_next)
		;
