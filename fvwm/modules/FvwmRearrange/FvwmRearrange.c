...
static void prepend_client(ModuleState *state, ClientNode *node)
{
  node->prev = NULL;
...
static void release_clients(ModuleState *state)
{
...
static ClientNode *find_client(ModuleState *state, Window frame)
{
...
static void detach_client(ModuleState *state, ClientNode *node)
{
  if (!node)
  {
    return;
...
static int window_matches(ModuleState *state, unsigned long *body)
{
...
static int collect_client(ModuleState *state)
{
...
static int await_configure(ModuleState *state, ClientNode *node)
{
  for (;;)
  {
    unsigned long header[HEADER_SIZE];
...
static int parse_metric(const char *token, unsigned long reference)
{
  char *endptr;
  long value;
...
static void send_resize(ModuleState *state, const ClientNode *node,
...
static void send_move(ModuleState *state, const ClientNode *node, int x,
                      int y)
{
  char command[128];
...
static void tile_clients(ModuleState *state)
{
  ...
  if (state->tile_horizontal)
  {
    ...
    else
    {
      ...
      hdiv =
        (state->bound_y - state->offset_y + 1) / state->client_count;
    }
    ...
    for (int s = 0; cursor && (s < stripes); ++s)
    {
      for (int slot = 0; cursor && (slot < slots_per_stripe); ++slot)
      {
        ...
        int new_height =
          hdiv - cursor->border_width * 2 - cursor->title_height;
        ...
        if (state->resize_clients)
        {
          ...
          send_resize(state, cursor,
                      (new_width > 0) ? (unsigned long)new_width
                                      : cursor->width,
                      (new_height > 0) ? (unsigned long)new_height
                                       : cursor->height);
        }
  ...
  else
  {
    ...
    else
    {
      ...
      state->tile_limit = limit;
      wdiv =
        (state->bound_x - state->offset_x + 1) / state->client_count;
    }
    slots_per_stripe = limit;
...
static void cascade_clients(ModuleState *state)
{
  ClientNode *cursor = state->reverse_order ? state->tail : state->head;
  int current_x = state->offset_x;
  int current_y = state->offset_y;
    ...
    if (state->resize_clients)
    {
      if (state->avoid_stretch)
      {
        if (state->limit_width &&
            cursor->width > (unsigned long)state->limit_width)
        {
          target_width = (unsigned long)state->limit_width;
        }
        if (state->limit_height &&
            cursor->height > (unsigned long)state->limit_height)
        {
...
static void parse_arguments(ModuleState *state, const char *source,
                            int argc, char *argv[], int start_index)
{
  ...
  for (int i = start_index; i < argc; ++i)
  {
    ...
    else
    {
      ...
      if (positional > 4)
      {
        fprintf(state->log, "%s: %s: ignoring unknown arg %s\n",
...
#ifdef USERC
static int tokenise_config(char *line, char ***argv_out)
{
  char *tokens[48];
  int count = 0;
...
#ifdef FVWM1
static char *LoadConfigLine(const char *filename, const char *match)
{
...
#else
  SetMessageMask(state->pipe_fd, M_CONFIGURE_WINDOW | M_DESTROY_WINDOW |
                                   M_END_WINDOWLIST);
#endif
