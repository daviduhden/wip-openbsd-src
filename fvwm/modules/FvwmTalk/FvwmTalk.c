...
main(int argc, char **argv)
{
	...
	if (Root == None) {
		fprintf(
		    stderr, "%s: Screen %d is not valid ", MyName, (int)screen);
		exit(1);
	}
	...
	window = XCreateWindow(dpy, Root, 0, 0, sizehints.width,
	    sizehints.height, (unsigned int)1, CopyFromParent, InputOutput,
...
Loop(int *fd)
{
	...
	while (1) {
		if (My_XNextEvent(dpy, &event)) {
			...
			case KeyPress:
				count = XLookupString(
				    &(event.xkey), kbuf, 99, &keysym, &compose);
...
request_selection(int time)
{
	...
	XConvertSelection(
	    dpy, XA_PRIMARY, XA_STRING, sel_property, window, time);
}
...
DrawWindow(int mode)
{
	...
	if (mode == ALL) {
		...
		XDrawImageString(dpy, window, myGC, 2,
		    (font->ascent + font->descent + 2) + font->ascent + 2,
	...
	XDrawImageString(dpy, window, myGC, 2,
	    2 * (font->ascent + font->descent + 2) + font->ascent + 2, Text,
	    pos);
