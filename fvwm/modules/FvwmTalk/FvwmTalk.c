...
int main(int argc, char **argv)
{
  ...
  window =
    XCreateWindow(dpy, Root, 0, 0, sizehints.width, sizehints.height,
  ...
  gcv.font = font->fid;
  myGC =
    XCreateGC(dpy, window, GCForeground | GCBackground | GCFont, &gcv);
...
void Loop(int *fd)
{
  ...
  while (1)
  {
    if (My_XNextEvent(dpy, &event))
    {
      ...
      case KeyPress:
        count =
          XLookupString(&(event.xkey), kbuf, 99, &keysym, &compose);
...
void paste_primary(int window, int property, int Delete)
{
  ...
  do
  {
    if (XGetWindowProperty(dpy, window, property, nread / 4, PROP_SIZE,
...
void DrawWindow(int mode)
{
  ...
  if (mode == ALL)
  {
    ...
    XDrawImageString(dpy, window, myGC, 2,
                     (font->ascent + font->descent + 2) + font->ascent +
                       2,
                     last_error, strlen(last_error));
    XDrawImageString(dpy, window, myGC, 2, font->ascent + 2,
                     previous_line, strlen(previous_line));
  }
  else
  {
    XClearArea(dpy, window, 0, 2 * (font->ascent + font->descent + 2),
               10000, 10000, 0);
  }
  XDrawImageString(dpy, window, myGC, 2,
                   2 * (font->ascent + font->descent + 2) +
                     font->ascent + 2,
                   Text, pos);
