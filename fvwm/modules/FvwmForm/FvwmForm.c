	...
	while (GetConfigLine(fd, &line_buf), line_buf) {
		...
		if (strncmp(cp, "GrabServer", 10) == 0) {
			...
			fprintf(
			    fp_err, "ButtonFont: %s\n", font_names[f_button]);
			xfs[f_button] =
			    GetFontOrFixed(dpy, font_names[f_button]);
			...
			fprintf(fp_err, "Text \"%s\" [%d, %d]\n",
			    item->text.value, item->header.size_x,
			    item->header.size_y);
	...
	for (l = 0; l < n_lines; l++) {
		...
		case L_LEFT:
			...
			for (i = 0; i < line->n; i++) {
				...
				width +=
				    ITEM_HSPC + line->items[i]->header.size_x;
			}
			break;
		case L_RIGHT:
		...
		case L_CENTER:
			...
			break;
		case L_LEFTRIGHT:
			/* count the number of inputs on the line - the extra
			 * space will be shared amongst these if there are any,
			 * otherwise it will be added as space in between the
			 * elements
			 */
			extra = 0;
			for (i = 0; i < line->n; i++) {
...
GetColors()
{
	...
	if (scr_depth < 8) {
		...
		XStoreNamedColor(dpy, d_cmap, color_names[c_fore],
		    colors[c_fore], DoRed | DoGreen | DoBlue);
		XStoreNamedColor(dpy, d_cmap, color_names[c_back],
		    colors[c_back], DoRed | DoGreen | DoBlue);
...
RedrawFrame()
{
	...
	for (i = 0; i < n_items; i++) {
		...
		case I_TEXT:
			...
			XDrawImageString(dpy, frame, gc_text, x, y,
			    item->text.value, item->text.n);
			break;
		case I_CHOICE:
...
RedrawItem(Item *item, int click)
{
	...
	case I_INPUT:
		...
		if (click) {
			...
			XDrawLine(dpy, item->header.win, gc_button, x, BOX_SPC,
			    x, dy - BOX_SPC);
		}
		...
		if (item == cur_text && !click) {
			x = BOX_SPC + TEXT_SPC +
			    FontWidth(xfs[f_input]) * abs_cursor - 1;
			XDrawLine(dpy, item->header.win, gc_input, x, BOX_SPC,
			    x, dy - BOX_SPC);
		}
		break;
	case I_CHOICE:
		...
		if (item->choice.on) {
			...
			if (item->choice.sel->select.key == IS_MULTIPLE) {
				...
				XDrawSegments(
				    dpy, item->header.win, gc_button, xsegs, 2);
			} else {
				XDrawArc(dpy, item->header.win, gc_button, 5, 5,
				    dx - 10, dy - 10, 0, 360 * 64);
			}
		} else
...
OpenWindows()
{
	...
	frame = XCreateSimpleWindow(dpy, root, x, y, max_width, total_height, 0,
	...
	for (i = 0; i < n_items; i++) {
		...
		switch (item->type) {
		case I_INPUT:
			item->header.win = XCreateSimpleWindow(dpy, frame,
			    item->header.pos_x, item->header.pos_y,
			    item->header.size_x, item->header.size_y, 0,
			    colors[c_back], colors[c_itemback]);
			XSelectInput(dpy, item->header.win,
			    ButtonPressMask | ExposureMask);
			xswa.cursor = xc_ibeam;
			XChangeWindowAttributes(
			    dpy, item->header.win, CWCursor, &xswa);
			break;
		case I_CHOICE:
			item->header.win = XCreateSimpleWindow(dpy, frame,
			    item->header.pos_x, item->header.pos_y,
			    item->header.size_y, item->header.size_y, 0,
			    colors[c_back], colors[c_itemback]);
			XSelectInput(dpy, item->header.win,
			    ButtonPressMask | ExposureMask);
			xswa.cursor = xc_hand;
		...
		if (event.xany.window == frame) {
			...
			case KeyPress: /* we do text input here */
				...
				case '\004': /* ^D */
					...
					break;
				case '\013': /* ^K */
					cur_text->input.value[rel_cursor] =
					    '\0';
				...
				case '\016': /* LINEFEED, TAB, RETURN, ^N, jump
					...
					/* end of all text input fields, check
					 * for buttons */
					for (i = 0; i < n_items; i++) {
						item = items + i;
						fprintf(fp_err,
				...
				default:
					...
					/* unrecognized key press, check for
					 * buttons */
					for (i = 0; i < n_items; i++) {
						item = items + i;
						fprintf(fp_err,
