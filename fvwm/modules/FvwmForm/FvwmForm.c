...
static void ensure_line_capacity(int count)
{
...
static void ensure_line_item_capacity(Line *line, int count)
{
  ...
  int new_cap =
    line->items_cap ? line->items_cap : INITIAL_LINE_ITEMS_CAPACITY;
  while (new_cap < count)
  {
    new_cap *= 2;
  }
...
static Line *add_line(int justify)
{
...
static void append_item_to_line(Line *line, Item *item)
{
...
int FontWidth(XFontStruct *xfs) { return (xfs->per_char[0].width); }
...
void ReadConfig()
{
  ...
  while (GetConfigLine(fd, &line_buf), line_buf)
  {
    ...
    if (*cp != '*')
      continue;
    if (strncmp(++cp, prog_name, prog_name_len) != 0)
      continue;
    cp += prog_name_len;
    ...
    else if (strncmp(cp, "Text", 4) == 0)
    {
      ...
      item->header.size_x =
        XTextWidth(xfs[f_text], item->text.value, item->text.n) +
        2 * TEXT_SPC;
    ...
    else if (strncmp(cp, "Input", 5) == 0)
    {
      ...
      item->header.size_y =
        FontHeight(xfs[f_input]) + 3 * TEXT_SPC + 2 * BOX_SPC;
    ...
    else if (strncmp(cp, "Selection", 9) == 0)
    {
      ...
      cur_sel->select.choices =
        (Item **)malloc(sizeof(Item *) * cur_sel->select.choices_cap);
    ...
    else if (strncmp(cp, "Choice", 6) == 0)
    {
      ...
      {
        fprintf(fp_err,
                "Choice specified before Selection, skipping\n");
        continue;
      }
      if (cur_sel->select.n + 1 > cur_sel->select.choices_cap)
      {
        cur_sel->select.choices_cap *= 2;
        cur_sel->select.choices = (Item **)realloc(
          cur_sel->select.choices,
      ...
      fprintf(fp_err, "Choice %s, \"%s\", [%d, %d]\n",
              item->header.name, item->choice.text, item->header.size_x,
              item->header.size_y);
      append_item_to_line(cur_line, item);
      continue;
    }
    else if (strncmp(cp, "Button", 6) == 0)
    {
      ...
      item->header.size_y =
        FontHeight(xfs[f_button]) + 2 * TEXT_SPC + 2 * BOX_SPC;
      item->header.size_x =
        2 * TEXT_SPC + 2 * BOX_SPC +
...
void Restart()
{
  ...
  for (i = 0; i < n_items; i++)
  {
    ...
    case I_INPUT:
      ...
      strlcpy(item->input.value, item->input.init_value,
              item->input.buf);
      item->input.left = 0;
...
void RedrawItem(Item *item, int click)
{
  ...
  case I_INPUT:
    ...
    if (click)
    {
      ...
      XDrawLine(dpy, item->header.win, gc_button, x, BOX_SPC, x,
                dy - BOX_SPC);
    }
    ...
    {
      x = BOX_SPC + TEXT_SPC + FontWidth(xfs[f_input]) * abs_cursor - 1;
      XDrawLine(dpy, item->header.win, gc_input, x, BOX_SPC, x,
                dy - BOX_SPC);
    }
    break;
  case I_CHOICE:
    ...
    if (item->choice.on)
    {
      ...
      {
        XDrawArc(dpy, item->header.win, gc_button, 5, 5, dx - 10,
                 dy - 10, 0, 360 * 64);
      }
    }
    else
...
void OpenWindows()
{
  ...
  frame =
    XCreateSimpleWindow(dpy, root, x, y, max_width, total_height, 0,
                        BlackPixel(dpy, screen), colors[c_back]);
  ...
  for (i = 0; i < n_items; i++)
  {
    ...
    {
    case I_INPUT:
      item->header.win = XCreateSimpleWindow(
        dpy, frame, item->header.pos_x, item->header.pos_y,
        item->header.size_x, item->header.size_y, 0, colors[c_back],
        colors[c_itemback]);
    ...
    case I_CHOICE:
      item->header.win = XCreateSimpleWindow(
        dpy, frame, item->header.pos_x, item->header.pos_y,
        item->header.size_y, item->header.size_y, 0, colors[c_back],
        colors[c_itemback]);
      XSelectInput(dpy, item->header.win,
                   ButtonPressMask | ExposureMask);
      xswa.cursor = xc_hand;
  ...
  if (warp_pointer)
  {
    XWarpPointer(dpy, None, frame, 0, 0, 0, 0, max_width / 2,
                 total_height - 1);
  }
  ...
  while (XEventsQueued(dpy, QueuedAfterReading))
  {
    ...
    if (event.xany.window == frame)
    {
      ...
      case Expose:
        ...
        {
          if (GrabSuccess == XGrabPointer(dpy, frame, True, 0,
      ...
      case KeyPress: /* we do text input here */
        ...
        case '\005': /* ^E */
          ...
          if ((cur_text->input.left =
                 rel_cursor - cur_text->input.size) < 0)
            cur_text->input.left = 0;
        ...
        default:
          old_cursor = abs_cursor;
          if ((buf[0] >= ' ' && buf[0] < '\177') || (buf[0] >= 160))
          { /* regular or intl char */
            if (++(cur_text->input.n) >= cur_text->input.buf)
            {
              cur_text->input.buf += cur_text->input.size;
              cur_text->input.value = (char *)realloc(
                cur_text->input.value, cur_text->input.buf);
            }
      ...
      redraw_newcursor:
      {
        int x, dy;
        x =
          BOX_SPC + TEXT_SPC + FontWidth(xfs[f_input]) * old_cursor - 1;
        dy = cur_text->header.size_y - 1;
        XSetForeground(dpy, gc_button, colors[c_itemback]);
        XDrawLine(dpy, cur_text->header.win, gc_button, x, BOX_SPC, x,
                  dy - BOX_SPC);
      }
      redraw:
      {
        ...
        if (len > cur_text->input.size)
          ...
          XDrawImageString(
            dpy, cur_text->header.win, gc_input,
            BOX_SPC + TEXT_SPC + FontWidth(xfs[f_input]) * len,
            BOX_SPC + TEXT_SPC + xfs[f_input]->ascent,
            cur_text->input.blanks, cur_text->input.size - len);
        XDrawImageString(
          dpy, cur_text->header.win, gc_input, BOX_SPC + TEXT_SPC,
          BOX_SPC + TEXT_SPC + xfs[f_input]->ascent,
          cur_text->input.value + cur_text->input.left, len);
        x =
          BOX_SPC + TEXT_SPC + FontWidth(xfs[f_input]) * abs_cursor - 1;
        dy = cur_text->header.size_y - 1;
        XDrawLine(dpy, cur_text->header.win, gc_input, x, BOX_SPC, x,
                  dy - BOX_SPC);
      }
      no_redraw:
      ...
      if (event.xany.window == item->header.win)
      {
        ...
        case ButtonPress:
          if (item->type == I_INPUT)
          {
            ...
            cur_text = item;
            RedrawItem(old_item, 1);
            abs_cursor = (event.xbutton.x - BOX_SPC - TEXT_SPC +
                          FontWidth(xfs[f_input]) / 2) /
          ...
          {
            RedrawItem(item, 1);
            XGrabPointer(dpy, item->header.win, False,
        ...
        case ButtonRelease:
          ...
          if (grab_server && server_grabbed)
          {
            XGrabPointer(dpy, frame, True, 0, GrabModeAsync,
                         GrabModeAsync, None, None, CurrentTime);
            XFlush(dpy);
          }
          else
