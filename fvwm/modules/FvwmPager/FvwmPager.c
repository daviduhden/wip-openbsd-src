...
main(int argc, char **argv)
{
	...
	if (argc < 8) {
		fprintf(stderr,
		    "%s Version %s requires two arguments: %s n m\n", MyName,
		    VERSION, MyName);
		fprintf(
...
ParseOptions(void)
{
	...
	for (GetConfigLine(fd, &tline); tline != NULL;
		...
		if (StrEquals(resource, "Geometry")) {
			flags =
			    XParseGeometry(arg1, &g_x, &g_y, &width, &height);
			if (flags & WidthValue) {
				window_w = width;
			}
		...
		/* ... and get Balloon config options ...
			...
			/* turn this on initially so balloon window is created;
			   later this variable is changed to match
...
PagerStringList *
NewPagerStringItem(PagerStringList *last, int desk)
{
