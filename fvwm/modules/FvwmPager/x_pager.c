...
extern Pixel win_back_pix, win_fore_pix, win_hi_back_pix,
  win_hi_fore_pix;
...
XSizeHints sizehints = {
  ...
  0,
  0, /* Max width and height */
  1,
  1, /* Width and height increments */
  {0, 0},
...
void initialize_pager(void)
{
  ...
  if (Scr.d_depth < 2)
  {
    Scr.gray_pixmap = XCreatePixmapFromBitmapData(
      dpy, Scr.Root, g_bits, g_width, g_height, fore_pix, back_pix,
      Scr.d_depth);
    Scr.light_gray_pixmap = XCreatePixmapFromBitmapData(
      dpy, Scr.Root, l_g_bits, l_g_width, l_g_height, fore_pix,
      back_pix, Scr.d_depth);
    Scr.sticky_gray_pixmap = XCreatePixmapFromBitmapData(
      dpy, Scr.Root, s_g_bits, s_g_width, s_g_height, fore_pix,
      back_pix, Scr.d_depth);
  }
  ...
  if (window_w <= 0)
    window_w =
      Columns * ((Scr.VxMax + Scr.MyDisplayWidth) / Scr.VScale + n) +
      Columns - 1;
  if (window_h <= 0)
  {
  ...
  Scr.Pager_w =
    XCreateWindow(dpy, Scr.Root, window_x, window_y, window_w, window_h,
                  (unsigned int)1, CopyFromParent, InputOutput,
  ...
  icon_win =
    XCreateWindow(dpy, Scr.Root, window_x, window_y, icon_w, icon_h,
                  (unsigned int)1, CopyFromParent, InputOutput,
                  (Visual *)CopyFromParent, valuemask, &attributes);
  XGrabButton(dpy, 1, AnyModifier, icon_win, True,
              ButtonPressMask | ButtonReleaseMask | ButtonMotionMask,
              GrabModeAsync, GrabModeAsync, None, None);
  XGrabButton(dpy, 2, AnyModifier, icon_win, True,
              ButtonPressMask | ButtonReleaseMask | ButtonMotionMask,
              GrabModeAsync, GrabModeAsync, None, None);
  XGrabButton(dpy, 3, AnyModifier, icon_win, True,
              ButtonPressMask | ButtonReleaseMask | ButtonMotionMask,
              GrabModeAsync, GrabModeAsync, None, None);
  if (!StartIconic)
  ...
  for (i = 0; i < ndesks; i++)
  {
    ...
    Desks[i].title_w = XCreateWindow(
      dpy, Scr.Pager_w, x, y, w, h, 1, CopyFromParent, InputOutput,
      CopyFromParent, valuemask, &attributes);
    attributes.event_mask = (ExposureMask | ButtonReleaseMask |
                             ButtonPressMask | ButtonMotionMask);
    desk_h = window_h - label_h;
    Desks[i].w = XCreateWindow(dpy, Desks[i].title_w, x, y, w, desk_h,
    ...
    Desks[i].CPagerWin = XCreateWindow(
      dpy, Desks[i].w, -1000, -1000, w, h, 0, CopyFromParent,
  ...
  if (ShowBalloons)
  {
    ...
    {
      fprintf(stderr,
              "%s: Warning: you're not allowed BalloonYOffset 0; "
              "defaulting to +2\n",
              MyName);
      balloon.yoffset = 2;
    }
    ...
    balloon.w = XCreateWindow(
      dpy, Scr.Root, 0, 0, /* coords set later */
      1,                   /* width set later */
      balloon.height, balloon.border, CopyFromParent, InputOutput,
      CopyFromParent, valuemask, &attributes);
...
void DispatchEvent(XEvent *Event)
{
  ...
  case ButtonRelease:
    ...
    else if (Event->xbutton.button == 3)
    {
      for (i = 0; i < ndesks; i++)
      {
        if (Event->xany.window == Desks[i].w)
        {
          XQueryPointer(dpy, Desks[i].w, &JunkRoot, &JunkChild, &JunkX,
                        &JunkY, &x, &y, &JunkMask);
      ...
      if (Event->xany.window == icon_win)
      {
        XQueryPointer(dpy, icon_win, &JunkRoot, &JunkChild, &JunkX,
                      &JunkY, &x, &y, &JunkMask);
  ...
  case MotionNotify:
    while (
      XCheckMaskEvent(dpy, PointerMotionMask | ButtonMotionMask, Event))
      ;
...
void ReConfigure(void)
{
  ...
  for (k = 0; k < Rows; k++)
  {
    for (j = 0; j < Columns; j++)
    {
      ...
      if (i < ndesks)
      {
        XMoveResizeWindow(dpy, Desks[i].title_w, (desk_w + 1) * j - 1,
                          (desk_h + label_h + 1) * k - 1, desk_w,
                          desk_h + label_h);
        XMoveResizeWindow(dpy, Desks[i].w, -1, label_h - 1, desk_w,
                          desk_h);
        ...
        if (uselabel)
          XClearArea(dpy, Desks[i].title_w, 0, 0, desk_w, label_h,
                     True);
      }
    }
  }
...
void DrawGrid(int i, int erase)
{
  ...
  {
    if (uselabel)
      XFillRectangle(dpy, Desks[i].title_w, HiliteGC, 0, 0, desk_w,
                     label_h - 1);
  }
  else
  {
    if (uselabel && erase)
      XClearArea(dpy, Desks[i].title_w, 0, 0, desk_w, label_h - 1,
                 False);
  }
  ...
  if ((w <= desk_w) && (uselabel))
  {
    hor_off = (desk_w - w) / 2;
    if (i == (Scr.CurrentDesk - desk1))
      XDrawString(dpy, Desks[i].title_w, rvGC, hor_off,
                  font->ascent + 1, ptr, strlen(ptr));
    else
...
void AddNewWindow(PagerWindow *t)
{
  ...
  m1 = (Scr.Vy + t->y) / Scr.MyDisplayHeight;
  x =
    (Scr.Vx + t->x) * (desk_w - n) / (Scr.VxMax + Scr.MyDisplayWidth) +
    n1;
  y =
  ...
  if ((i >= 0) && (i < ndesks))
  {
    t->PagerView = XCreateWindow(
      dpy, Desks[i].w, x, y, w, h, 1, CopyFromParent, InputOutput,
      CopyFromParent, valuemask, &attributes);
  ...
  x =
    (Scr.Vx + t->x) * (icon_w - n) / (Scr.VxMax + Scr.MyDisplayWidth) +
    n1;
  y =
...
void Scroll(int window_w, int window_h, int x, int y, int Desk)
{
  ...
  if (Wait == 0)
  {
    ...
    sx = (100 *
          (x * (Scr.VxMax + Scr.MyDisplayWidth) / window_w - Scr.Vx)) /
         Scr.MyDisplayWidth;
    sy = (100 *
          (y * (Scr.VyMax + Scr.MyDisplayHeight) / window_h - Scr.Vy)) /
         Scr.MyDisplayHeight;
...
void MoveWindow(XEvent *Event)
{
  ...
  while (!finished)
  {
    XMaskEvent(dpy, ButtonReleaseMask | ButtonMotionMask | ExposureMask,
               Event);
  ...
  if (KeepMoving)
  {
    ...
    if ((NewDesk >= desk1) && (NewDesk <= desk2))
      XReparentWindow(dpy, t->PagerView, Desks[NewDesk - desk1].w, 0,
                      0);
    else
    {
      XDestroyWindow(dpy, t->PagerView);
    ...
    XTranslateCoordinates(dpy, Scr.Pager_w, Scr.Root, x, y, &x1, &y1,
                          &dumwin);
  ...
  else
  {
    ...
    XTranslateCoordinates(dpy, Scr.Pager_w, Desks[NewDesk].w, x - x1,
                          y - y1, &x2, &y2, &dumwin);
    ...
    m1 = y2 * (Scr.VyMax + Scr.MyDisplayHeight) /
         (desk_h * Scr.MyDisplayHeight);
    x = (x2 - n1) * (Scr.VxMax + Scr.MyDisplayWidth) / (desk_w - n) -
        Scr.Vx;
    y = (y2 - m1) * (Scr.VyMax + Scr.MyDisplayHeight) / (desk_h - m) -
        Scr.Vy;
    ...
    if (NewDesk + desk1 != t->desk)
    {
      ...
      {
        snprintf(command, sizeof(command), "MoveToDesk 0 %d",
                 NewDesk + desk1);
        SendInfo(fd, command, t->w);
...
void PictureWindow(PagerWindow *t)
{
  ...
  if (MiniIcons)
  {
    if (t->mini_icon.picture && (t->PagerView != None))
    {
      ...
      XClearWindow(dpy, t->PagerView);
      XCopyArea(dpy, t->mini_icon.picture, t->PagerView, MiniIconGC, 0,
                0, t->mini_icon.width, t->mini_icon.height, iconX,
                iconY);
    }
  }
}
void PictureIconWindow(PagerWindow *t)
{
  ...
  if (MiniIcons)
  {
    if (t->mini_icon.picture && (t->IconView != None))
    {
      ...
      XChangeGC(dpy, MiniIconGC, Globalgcm, &Globalgcv);
      XClearWindow(dpy, t->IconView);
      XCopyArea(dpy, t->mini_icon.picture, t->IconView, MiniIconGC, 0,
                0, t->mini_icon.width, t->mini_icon.height, iconX,
                iconY);
    }
  }
...
void IconMoveWindow(XEvent *Event, PagerWindow *t)
{
  ...
  if (KeepMoving)
  {
    XTranslateCoordinates(dpy, t->IconView, Scr.Root, x, y, &x1, &y1,
                          &dumwin);
    XUngrabPointer(dpy, CurrentTime);
    XSync(dpy, 0);
    snprintf(command, sizeof(command), "Move %dp %dp", x, y);
  ...
  else
  {
    ...
    x = (x - n1) * (Scr.VxMax + Scr.MyDisplayWidth) / (icon_w - n) -
        Scr.Vx;
    y = (y - m1) * (Scr.VyMax + Scr.MyDisplayHeight) / (icon_h - m) -
        Scr.Vy;
...
void MapBalloonWindow(XEvent *event)
{
  ...
  window_changes.width =
    4 + XTextWidth(balloon.font, t->icon_name, strlen(t->icon_name));
  ...
  XTranslateCoordinates(dpy, view, Scr.Root, x, y, &window_changes.x,
                        &window_changes.y, &dummy);
  ...
  XConfigureWindow(dpy, balloon.w, CWX | CWY | CWWidth,
                   &window_changes);
...
/* -- ric@giccs.georgetown.edu */
void UnmapBalloonWindow(void) { XUnmapWindow(dpy, balloon.w); }
