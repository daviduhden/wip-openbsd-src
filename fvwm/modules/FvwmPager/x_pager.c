...
initialize_pager(void)
{
	...
	if (Scr.d_depth < 2) {
		Scr.gray_pixmap = XCreatePixmapFromBitmapData(dpy, Scr.Root,
	...
	Scr.Pager_w = XCreateWindow(dpy, Scr.Root, window_x, window_y, window_w,
	    window_h, (unsigned int)1, CopyFromParent, InputOutput,
	...
	icon_win = XCreateWindow(dpy, Scr.Root, window_x, window_y, icon_w,
	    icon_h, (unsigned int)1, CopyFromParent, InputOutput,
	    (Visual *)CopyFromParent, valuemask, &attributes);
	...
	for (i = 0; i < ndesks; i++) {
		...
		Desks[i].title_w = XCreateWindow(dpy, Scr.Pager_w, x, y, w, h,
		...
		Desks[i].CPagerWin = XCreateWindow(dpy, Desks[i].w, -1000,
		    -1000, w, h, 0, CopyFromParent, InputOutput, CopyFromParent,
		    valuemask, &attributes);
	...
	if (ShowBalloons) {
		...
		if ((balloon.font = XLoadQueryFont(dpy, BalloonFont)) == NULL) {
			...
			fprintf(stderr,
			    "%s: Can't find font '%s', using fixed.\n", MyName,
			    BalloonFont);
		}
		...
		/* Make sure we don't get balloons initially with the Icon
		 * option. */
...
DispatchEvent(XEvent *Event)
{
	...
	case ButtonRelease:
		if ((Event->xbutton.button == 1) ||
			...
			for (i = 0; i < ndesks; i++) {
				if (Event->xany.window == Desks[i].w) {
					XQueryPointer(dpy, Desks[i].w,
					    &JunkRoot, &JunkChild, &JunkX,
					    &JunkY, &x, &y, &JunkMask);
					Scroll(desk_w, desk_h, x, y, i);
				}
			}
			if (Event->xany.window == icon_win) {
				XQueryPointer(dpy, icon_win, &JunkRoot,
				    &JunkChild, &JunkX, &JunkY, &x, &y,
				    &JunkMask);
	...
	case MotionNotify:
		while (XCheckMaskEvent(
		    dpy, PointerMotionMask | ButtonMotionMask, Event))
			;
		...
		for (j = 0; j < Columns; j++) {
			...
			if (i < ndesks) {
				...
				XMoveResizeWindow(dpy, Desks[i].w, -1,
				    label_h - 1, desk_w, desk_h);
				if (i == Scr.CurrentDesk - desk1)
					XMoveResizeWindow(dpy,
					    Desks[i].CPagerWin, x, y, w, h);
				else
					XMoveResizeWindow(dpy,
					    Desks[i].CPagerWin, -1000, -100, w,
					    h);

				XClearArea(dpy, Desks[i].w, 0, 0, desk_w,
				    desk_h, True);
				if (uselabel)
					XClearArea(dpy, Desks[i].title_w, 0, 0,
					    desk_w, label_h, True);
			}
		}
	}
...
MovePage(void)
{
	...
	if (Scr.CurrentDesk != icon_desk_shown) {
		...
		if (XStringListToTextProperty(&sptr, 1, &name) == 0) {
			fprintf(
			    stderr, "%s: cannot allocate window name", MyName);
			return;
		}
...
DrawGrid(int i, int erase)
{
	...
	if ((Scr.CurrentDesk - desk1) == i) {
		if (uselabel)
			XFillRectangle(dpy, Desks[i].title_w, HiliteGC, 0, 0,
			    desk_w, label_h - 1);
	} else {
		if (uselabel && erase)
...
AddNewWindow(PagerWindow *t)
{
	...
	n1 = (Scr.Vx + t->x) / Scr.MyDisplayWidth;
	m1 = (Scr.Vy + t->y) / Scr.MyDisplayHeight;
	x = (Scr.Vx + t->x) * (desk_w - n) / (Scr.VxMax + Scr.MyDisplayWidth) +
	    n1;
	...
	x = (Scr.Vx + t->x) * (icon_w - n) / (Scr.VxMax + Scr.MyDisplayWidth) +
	    n1;
	...
	if (Scr.CurrentDesk == t->desk) {
		...
		t->IconView = XCreateWindow(dpy, icon_win, -1000, -1000, w, h,
		    1, CopyFromParent, InputOutput, CopyFromParent, valuemask,
		    &attributes);
		XMapRaised(dpy, t->IconView);
	}
			...
			if (t->flags & STICKY) {
				if (t->PagerView != None)
					XSetWindowBackgroundPixmap(dpy,
					    ...
					    Scr.sticky_gray_pixmap);
				XSetWindowBackgroundPixmap(
				    dpy, t->IconView, Scr.sticky_gray_pixmap);
			} else {
				if (t->PagerView != None)
					XSetWindowBackgroundPixmap(dpy,
					    t->PagerView,
					    Scr.light_gray_pixmap);
				XSetWindowBackgroundPixmap(
				    dpy, t->IconView, Scr.light_gray_pixmap);
			}
		}
	} else {
		if (on) {
			if (t->PagerView != None)
				XSetWindowBackground(
				    dpy, t->PagerView, focus_pix);
			XSetWindowBackground(dpy, t->IconView, focus_pix);
		} else {
			if (t->PagerView != None)
				XSetWindowBackground(
				    dpy, t->PagerView, t->back);
...
void
Scroll(int window_w, int window_h, int x, int y, int Desk)
{
#ifndef NON_VIRTUAL
	char command[256];
	int sx, sy;
...
MoveWindow(XEvent *Event)
{
	...
	while (!finished) {
		...
		if (Event->type == MotionNotify) {
			XTranslateCoordinates(dpy, Event->xany.window,
			    Scr.Pager_w, Event->xmotion.x, Event->xmotion.y, &x,
			    &y, &dumwin);
			if (moved == 0) {
				xi = x;
				yi = y;
			...
			XTranslateCoordinates(dpy, Event->xany.window,
			    Scr.Pager_w, Event->xbutton.x, Event->xbutton.y, &x,
			    &y, &dumwin);
			XMoveWindow(dpy, t->PagerView, x - (x1), y - (y1));
			finished = 1;
	...
	if (KeepMoving) {
		...
		if ((NewDesk >= desk1) && (NewDesk <= desk2))
			XReparentWindow(
			    dpy, t->PagerView, Desks[NewDesk - desk1].w, 0, 0);
		else {
			XDestroyWindow(dpy, t->PagerView);
			t->PagerView = None;
		}
		XTranslateCoordinates(
		    dpy, Scr.Pager_w, Scr.Root, x, y, &x1, &y1, &dumwin);
		...
		if (x + Scr.Vx > Scr.MyDisplayWidth + Scr.VxMax)
			x = Scr.MyDisplayWidth + Scr.VxMax - t->frame_width -
			    Scr.Vx;
		if (y + Scr.Vy > Scr.MyDisplayHeight + Scr.VyMax)
			y = Scr.MyDisplayHeight + Scr.VyMax - t->frame_height -
			    Scr.Vy;
		if (((t->flags & ICONIFIED) && (t->flags & StickyIcon)) ||
		    (t->flags & STICKY)) {
			NewDesk = Scr.CurrentDesk - desk1;
		...
		if ((NewDesk >= 0) && (NewDesk < ndesks)) {
			XReparentWindow(
			    dpy, t->PagerView, Desks[NewDesk].w, x, y);
			if (moved) {
				if (t->flags & ICONIFIED)
...
PictureWindow(PagerWindow *t)
{
	...
	if (MiniIcons) {
		if (t->mini_icon.picture && (t->PagerView != None)) {
			...
			XClearWindow(dpy, t->PagerView);
			XCopyArea(dpy, t->mini_icon.picture, t->PagerView,
			    MiniIconGC, 0, 0, t->mini_icon.width,
			    t->mini_icon.height, iconX, iconY);
		}
	}
}
void
...
IconMoveWindow(XEvent *Event, PagerWindow *t)
{
	...
	if (KeepMoving) {
		XTranslateCoordinates(
		    dpy, t->IconView, Scr.Root, x, y, &x1, &y1, &dumwin);
		XUngrabPointer(dpy, CurrentTime);
		XSync(dpy, 0);
		snprintf(command, sizeof(command), "Move %dp %dp", x, y);
		SendInfo(fd, command, t->w);
		...
		if (t->flags & ICONIFIED) {
			...
			*/
			...
			XSetInputFocus(
			    dpy, t->w, RevertToParent, Event->xbutton.time);
		}
	}
}
