...
static int cpp_process(Display *display, const char *host,
                       char *options, const char *config_file,
                       int keep_output);
...
int main(int argc, char **argv)
{
  ...
  if (cpp_process(dpy, display_name, cpp_options, filename,
                  cpp_debug) != 0)
    exit(1);
...
static int cpp_process(Display *display, const char *host,
                       char *cpp_opts, const char *config_file,
                       int keep_output)
{
  Screen *screen;
  Visual *visual;
...
#undef WRITE_NUM
  ...
  if (created_temp && kept_path[0] != '\0')
  {
    ...
    snprintf(msg, sizeof(msg),
             "Echo %s: preprocessor output kept in %s\n", MyName,
             kept_path);
    SendInfo(fd, msg, 0);
  }
