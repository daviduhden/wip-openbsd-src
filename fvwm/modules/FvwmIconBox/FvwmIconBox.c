  ...
  while (1)
  {
    if (My_XNextEvent(dpy, &Event))
    {
      ...
      case ConfigureNotify:
        XGetGeometry(dpy, main_win, &root, &x, &y, (unsigned int *)&tw,
                     (unsigned int *)&th, (unsigned int *)&border_width,
                     (unsigned int *)&depth);
        if (ready && (tw != main_width || th != main_height))
        {
          ...
          XMoveResizeWindow(dpy, holder_win, margin1 + 2, margin1 + 2,
                            tw - h_margin, th - v_margin);
          if (!(local_flags & HIDE_H))
            XResizeWindow(dpy, h_scroll_bar, Width - bar_width * 2,
                          bar_width);
          if (!(local_flags & HIDE_V))
            XResizeWindow(dpy, v_scroll_bar, bar_width,
                          Height - bar_width * 2);
      ...
      case LeaveNotify:
        ...
        if (!(local_flags & HIDE_H) &&
            Event.xbutton.window == l_button && Pressed == l_button)
        {
          Pressed = None;
          RedrawLeftButton(ReliefGC, ShadowGC);
        }
        ...
        case XA_WM_HINTS:
          ...
          if (tmp->wmhints && (tmp->wmhints->flags & IconPixmapHint))
          {
            ...
            if (tmp->icon_maskPixmap != None)
              XShapeCombineMask(dpy, tmp->icon_pixmap_w, ShapeBounding,
                                0, 0, None, ShapeSet);
#endif
            ...
            if (tmp->icon_maskPixmap != None)
              XShapeCombineMask(dpy, tmp->icon_pixmap_w, ShapeBounding,
                                hr, hr, tmp->icon_maskPixmap, ShapeSet);
#endif
...
void RedrawWindow(void)
{
  ...
  RelieveWindow(main_win, margin1, margin1, Width + 4, Height + 4,
                ShadowGC, ReliefGC);
  if (!(local_flags & HIDE_H))
...
void RedrawIcon(struct icon_info *item, int f)
{
  ...
  if ((f & 1) && (item->flags & ICON_OURS))
  {
    if (item->iconPixmap != None && item->icon_pixmap_w != None)
    {
      if (item->icon_depth != d_depth)
        XCopyPlane(dpy, item->iconPixmap, item->icon_pixmap_w, NormalGC,
                   0, 0, item->icon_w, item->icon_h, hr, hr, plane);
      else
    ...
    if (!(item->flags & SHAPED_ICON))
    {
      if (item->icon_w > 0 && item->icon_h > 0)
        RelieveWindow(
          item->icon_pixmap_w, 0, 0, item->icon_w + icon_relief,
          item->icon_h + icon_relief, IconReliefGC, IconShadowGC);
      else
        RelieveWindow(
          item->icon_pixmap_w, 0, 0, max_icon_width + icon_relief,
          max_icon_height + icon_relief, IconReliefGC, IconShadowGC);
    }
  }
  ...
  if (f & 2)
  {
    ...
    if (Hilite == item)
    {
      ...
      XMoveResizeWindow(
        dpy, item->IconWin, item->x + min(0, (diff - 8)) / 2,
        item->y + h, max(tw + 8, w), 6 + font->ascent + font->descent);
      XClearWindow(dpy, item->IconWin);
      XDrawString(dpy, item->IconWin, NormalGC, lm, 3 + font->ascent,
                  label, len);
      RelieveWindow(item->IconWin, 0, 0, max(tw + 8, w),
                    6 + font->ascent + font->descent, IconReliefGC,
                    IconShadowGC);
    }
    else
    {
      XMoveResizeWindow(dpy, item->IconWin, item->x, item->y + h, w,
                        6 + font->ascent + font->descent);
      XClearWindow(dpy, item->IconWin);
      XDrawString(dpy, item->IconWin, NormalGC, lm, 3 + font->ascent,
                  label, len);
      RelieveWindow(item->IconWin, 0, 0, w,
                    6 + font->ascent + font->descent, IconReliefGC,
                    IconShadowGC);
    }
  }
...
void RedrawHScrollbar(void)
{
  ...
  RelieveWindow(h_scroll_bar, x, 0, width, bar_width, ReliefGC,
                ShadowGC);
}
...
void RedrawVScrollbar(void)
{
  ...
  RelieveWindow(v_scroll_bar, 0, y, bar_width, height, ReliefGC,
                ShadowGC);
}
...
void RedrawLeftButton(GC rgc, GC sgc)
{
  ...
  seg[i].x2 = bar_width - 2;
  seg[i++].y2 = 1;

  seg[i].x1 = 0;
  seg[i].y1 = bar_width / 2;
  seg[i].x2 = bar_width - 1;
  seg[i++].y2 = 0;
  ...
  seg[i].y1 = bar_width / 2;
  seg[i].x2 = bar_width - 2;
  seg[i++].y2 = bar_width - 2;

  seg[i].x1 = 0;
  seg[i].y1 = bar_width / 2;
  seg[i].x2 = bar_width - 1;
  seg[i++].y2 = bar_width - 1;
...
/************************************************************************
  RelieveWindow
    Original work from GoodStuff:
      Copyright 1993, Robert Nation.
************************************************************************/
void RelieveWindow(Window win, int x, int y, int w, int h, GC rgc,
                   GC sgc)
{
  XSegment seg[4];
  int i;
...
void CreateWindow(void)
{
  ...
  if (x > -100000)
  {
    ...
    {
      mysizehints.y =
        DisplayHeight(dpy, screen) + y - mysizehints.height;
  ...
  holder_win = XCreateSimpleWindow(
    dpy, main_win, margin1 + 2, margin1 + 2, mysizehints.width,
    mysizehints.height, 0, fore_pix, back_pix);

  icon_win = XCreateSimpleWindow(
    dpy, holder_win, -icon_win_x, -icon_win_y, icon_win_width,
    icon_win_height, 0, fore_pix, back_pix);
  ...
  if (!(local_flags & HIDE_H))
  {
    ...
    h_scroll_bar =
      XCreateWindow(dpy, main_win, margin1 + 2 + bar_width,
                    margin1 + 6 + Height + margin2,
                    Width - bar_width * 2, bar_width, 0, CopyFromParent,
  ...
  if (!(local_flags & HIDE_V))
  {
    ...
    v_scroll_bar = XCreateWindow(
      dpy, main_win, margin1 + 6 + Width + margin2,
      margin1 + 2 + bar_width, bar_width, Height - bar_width * 2, 0,
      CopyFromParent, InputOutput, CopyFromParent, mask, &attributes);
  ...
  if (!(local_flags & HIDE_H))
  {
    ...
    l_button = XCreateWindow(dpy, main_win, margin1 + 2,
                             margin1 + 6 + Height + margin2, bar_width,
                             bar_width, 0, CopyFromParent, InputOutput,
                             CopyFromParent, mask, &attributes);
    attributes.win_gravity = SouthEastGravity;
    r_button = XCreateWindow(
      dpy, main_win, margin1 + 2 + Width - bar_width,
      margin1 + 6 + Height + margin2, bar_width, bar_width, 0,
      CopyFromParent, InputOutput, CopyFromParent, mask, &attributes);
  ...
  {
    attributes.win_gravity = NorthEastGravity;
    t_button = XCreateWindow(
      dpy, main_win, margin1 + 6 + Width + margin2, margin1 + 2,
      bar_width, bar_width, 0, CopyFromParent, InputOutput,
      CopyFromParent, mask, &attributes);
    attributes.win_gravity = SouthEastGravity;
    b_button = XCreateWindow(
      dpy, main_win, margin1 + 6 + Width + margin2,
      margin1 + 2 + Height - bar_width, bar_width, bar_width, 0,
      CopyFromParent, InputOutput, CopyFromParent, mask, &attributes);
...
void GetIconwinSize(int *dx, int *dy)
{
  ...
  *dy = icon_win_height;
  ...
  if (primary == LEFT || primary == RIGHT)
  {
    ...
    icon_win_height =
      max(Height, UHeight * (max(0, (num_icons - 1)) / Lines + 1) - 1 +
                    interval);
  }
  else
  {
    icon_win_width =
      max(Width,
          UWidth * (max(0, num_icons - 1) / Lines + 1) + interval - 1);
...
void ParseOptions(void)
{
  ...
  while (tline != NULL)
  {
    ...
    if (strlen(&tline[0]) > 1)
    {
      if (strncasecmp(tline, CatString3("*", MyName, "Geometry"),
                      Clength + 9) == 0)
      {
        tmp = &tline[Clength + 9];
      ...
      else if (strncasecmp(tline, CatString3("*", MyName, "Font"),
                           Clength + 5) == 0)
        CopyString(&font_string, &tline[Clength + 5]);
      else if (strncasecmp(tline, CatString3("*", MyName, "IconFore"),
                           Clength + 9) == 0)
        CopyString(&IconFore, &tline[Clength + 9]);
      else if (strncasecmp(tline, CatString3("*", MyName, "IconBack"),
                           Clength + 9) == 0)
        CopyString(&IconBack, &tline[Clength + 9]);
      else if (strncasecmp(tline, CatString3("*", MyName, "IconHiFore"),
                           Clength + 11) == 0)
        CopyString(&ActIconFore, &tline[Clength + 11]);
      else if (strncasecmp(tline, CatString3("*", MyName, "IconHiBack"),
                           Clength + 11) == 0)
      ...
      else if (strncasecmp(tline, CatString3("*", MyName, "Padding"),
                           Clength + 8) == 0)
        interval = max(0, atoi(&tline[Clength + 8]));
      else if (strncasecmp(tline, CatString3("*", MyName, "FrameWidth"),
                           Clength + 11) == 0)
      {
      ...
      else if (strncasecmp(tline, CatString3("*", MyName, "Lines"),
                           Clength + 6) == 0)
        Lines = max(1, atoi(&tline[Clength + 6]));
      else if (strncasecmp(tline, CatString3("*", MyName, "SBWidth"),
                           Clength + 8) == 0)
        bar_width = max(5, atoi(&tline[Clength + 8]));
      else if (strncasecmp(tline, CatString3("*", MyName, "Placement"),
                           Clength + 10) == 0)
      ...
      else if (strncasecmp(tline, CatString3("*", MyName, "Resolution"),
                           Clength + 11) == 0)
      {
      ...
      else if (strncasecmp(tline, CatString3("*", MyName, "Mouse"),
                           Clength + 6) == 0)
        parsemouse(&tline[Clength + 6]);
      else if (strncasecmp(tline, CatString3("*", MyName, "Key"),
                           Clength + 4) == 0)
        parsekey(&tline[Clength + 4]);
      else if (strncasecmp(tline, CatString3("*", MyName, "SortIcons"),
                           Clength + 10) == 0)
      {
      ...
      else if (strncasecmp(tline, CatString3("*", MyName, "HideSC"),
                           Clength + 7) == 0)
      {
        tmp = &tline[Clength + 7];
        while (((isspace(*tmp)) && (*tmp != '\n')) && (*tmp != 0))
...
void process_message(unsigned long type, unsigned long *body)
{
      ...
      while (tmp != NULL)
      {
        if (tmp->id == body[0])
        {
          if ((tmp->desk != body[7]) && !(tmp->flags & STICKY))
          {
            ...
            tmp->flags |= STICKY;
          else if (!(body[8] & STICKY) && (tmp->flags & STICKY))
          { /* unstick */
            tmp->flags &= ~STICKY;
            tmp->desk = body[7];
          }
  ...
  case M_ICON_NAME:
    ...
    if (!ready || tmp == NULL)
      break;
    if (sortby != UNSORT && tmp->IconWin != None && desk_cond(tmp) &&
        SortItem(tmp) == True)
      AdjustIconWindows();
...
{
  if (!(local_flags & CURRENT_ONLY) || (item->flags & STICKY) ||
      (item->desk == CurrentDesk))
    return 1;
 ...
 *		Copyright 1994, Mike Finger.
 ***********************************************************************/
struct icon_info *UpdateItem(unsigned long type, unsigned long id,
                             char *item)
{
  struct icon_info *tmp;
  char *str;
...
void ExecuteKey(XEvent event)
{
  ...
  tmp = KeyActions;
  {
    ...
    if (mapping != NULL)
    {
      ...
      KeyCode canonical =
        (primary != NoSymbol) ? XKeysymToKeycode(dpy, primary) : 0;
      if (canonical != 0)
