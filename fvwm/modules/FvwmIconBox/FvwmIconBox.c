...
int main(int argc, char **argv)
{
  ...
  strlcpy(MyName, temp, sizeof(MyName));
...
void SendFvwmPipe(int *fd, char *message,unsigned long window)
{
  ...
  while(1) {
    ...
    if (temp!=NULL) {
      ...
      memcpy(temp_msg, hold, (size_t)(temp-hold));
...
void process_message(unsigned long type, unsigned long *body)
{
  ...
  strlcpy(str, (char *)&body[3], sizeof(str));
    FvwmDefaultIcon = str;
    break;
  case M_ICONIFY:
  case M_DEICONIFY:
...
char *stripcpy2(char *source)
{
  ...
  memcpy(ptr, source, (size_t)count);
  ptr[count]=0;
  return ptr;
}
