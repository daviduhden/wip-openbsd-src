		...
		if (My_XNextEvent(dpy, &Event)) {
			...
			case ConfigureNotify:
				XGetGeometry(dpy, main_win, &root, &x, &y,
				    ...
				    (unsigned int *)&depth);
				if (ready &&
				    (tw != main_width || th != main_height)) {
					main_width = tw;
					main_height = th;
					oldw = Width;
					oldh = Height;
					num_columns =
					    (tw - h_margin - interval + 1) /
					    UWidth;
					num_rows =
					    (th - v_margin - interval + 1) /
					    UHeight;
					Width =
					    UWidth * num_columns + interval - 1;
					Height =
					    UHeight * num_rows + interval - 1;
					XMoveResizeWindow(dpy, holder_win,
					    margin1 + 2, margin1 + 2,
					...
					if (icon_win_y + Height >
					    icon_win_height)
						icon_win_y =
						    icon_win_height - Height;
					XMoveResizeWindow(dpy, icon_win,
...
RedrawIcon(struct icon_info *item, int f)
{
	...
	if (Hilite == item) {
		...
		if (max_icon_height != 0 && (item->flags & ICON_OURS))
			XSetWindowBackground(
			    dpy, item->icon_pixmap_w, act_icon_back_pix);
	...
	if ((f & 1) && (item->flags & ICON_OURS)) {
		if (item->iconPixmap != None && item->icon_pixmap_w != None) {
			if (item->icon_depth != d_depth)
				XCopyPlane(dpy, item->iconPixmap,
				    item->icon_pixmap_w, NormalGC, 0, 0,
				    item->icon_w, item->icon_h, hr, hr, plane);
			else
		...
		if (!(item->flags & SHAPED_ICON)) {
			if (item->icon_w > 0 && item->icon_h > 0)
				RelieveWindow(item->icon_pixmap_w, 0, 0,
				    item->icon_w + icon_relief,
	...
	if (f & 2) {
		...
		if (Hilite == item) {
			...
			XMoveResizeWindow(dpy, item->IconWin,
			    item->x + min(0, (diff - 8)) / 2, item->y + h,
			    max(tw + 8, w), 6 + font->ascent + font->descent);
			XClearWindow(dpy, item->IconWin);
			XDrawString(dpy, item->IconWin, NormalGC, lm,
			    3 + font->ascent, label, len);
			RelieveWindow(item->IconWin, 0, 0, max(tw + 8, w),
			    6 + font->ascent + font->descent, IconReliefGC,
			    IconShadowGC);
		} else {
			XMoveResizeWindow(dpy, item->IconWin, item->x,
			    item->y + h, w, 6 + font->ascent + font->descent);
			XClearWindow(dpy, item->IconWin);
			XDrawString(dpy, item->IconWin, NormalGC, lm,
			    3 + font->ascent, label, len);
	...
	if (Hilite == item) {
		...
		if (max_icon_height != 0 && (item->flags & ICON_OURS))
			XSetWindowBackground(
			    dpy, item->icon_pixmap_w, icon_back_pix);
...
RedrawVScrollbar(void)
{
	...
	RelieveWindow(
	    v_scroll_bar, 0, y, bar_width, height, ReliefGC, ShadowGC);
}
...
CreateWindow(void)
{
	...
	if (x > -100000) {
		if (xneg) {
			mysizehints.x =
			    DisplayWidth(dpy, screen) + x - mysizehints.width;
	...
	if (d_depth < 2) {
		back_pix = icon_back_pix = act_icon_fore_pix =
		    GetColor("white");
		fore_pix = icon_fore_pix = act_icon_back_pix =
		    GetColor("black");
		hilite_pix = icon_hilite_pix = act_icon_shadow_pix =
		    icon_back_pix;
		shadow_pix = icon_shadow_pix = act_icon_hilite_pix =
		    icon_fore_pix;
	} else {
	...
	if (!(local_flags & HIDE_H)) {
		...
		h_scroll_bar = XCreateWindow(dpy, main_win,
		    margin1 + 2 + bar_width, margin1 + 6 + Height + margin2,
	...
	if (!(local_flags & HIDE_V)) {
		...
		v_scroll_bar = XCreateWindow(dpy, main_win,
		    margin1 + 6 + Width + margin2, margin1 + 2 + bar_width,
		    bar_width, Height - bar_width * 2, 0, CopyFromParent,
		    InputOutput, CopyFromParent, mask, &attributes);
	...
	if (!(local_flags & HIDE_H)) {
		...
		r_button = XCreateWindow(dpy, main_win,
		    margin1 + 2 + Width - bar_width,
		    margin1 + 6 + Height + margin2, bar_width, bar_width, 0,
		    CopyFromParent, InputOutput, CopyFromParent, mask,
		    &attributes);
	...
	case M_ADD_WINDOW:
		if (AddItem(body[0], body[7], body[8]) == True && ready) {
			...
			XMoveResizeWindow(dpy, icon_win, -icon_win_x,
			    -icon_win_y, icon_win_width, icon_win_height);
		}
		break;
...
struct icon_info *
SetFlag(unsigned long id, int t)
{
	struct icon_info *tmp;
	tmp = Head;
...
int
desk_cond(struct icon_info *item)
{
 ...
 ***********************************************************************/
Bool
AddItem(unsigned long id, long desk, unsigned long flags)
{
...
Bool
SortItem(struct icon_info *item)
{
...
int
itemcmp(struct icon_info *item1, struct icon_info *item2)
{
	int ret1, ret2;
