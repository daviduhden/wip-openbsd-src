...
**/
void RelieveWindow(Window wn, int width, int x, int y, int w, int h,
                   Pixel relief, Pixel shadow, int rev)
{
  XSegment seg[4];
  unsigned long gcm = 0;
  XGCValues gcv;
...
void RedrawButton(button_info *b, int clean)
{
  ...
  RelieveWindow(MyWindow, f, x, y, BW, BH, buttonHilite(b),
                buttonShadow(b), rev);
    ...
    if (b->flags & b_Container)
    {
      ...
      if (h2)
        XFillRectangle(Dpy, MyWindow, NormalGC, x1, y1 + h - h2, w, h2);
             ...
             !(b->flags & b_Swallow))
      XFillRectangle(Dpy, MyWindow, NormalGC, x + f, y + f, BW - 2 * f,
                     BH - 2 * f);
  }
