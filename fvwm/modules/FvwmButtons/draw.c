...
MakeButton(button_info *b)
{
	...
	*/
	...
	if (!b) {
		fprintf(stderr,
		    "%s: BUG: DrawButton called with NULL pointer\n", MyName);
		exit(2);
	}
	...
	/* At this point iw,ih,ix and iy should be correct. Now all we have to
	   do is place title and iconwin in their proper positions */

	/* For now, use the old routine in icons.h for buttons with icons */
	if (b->flags & b_Icon)
		ConfigureIconWindow(b);

	/* For now, hardcoded window centered, title bottom centered, below
	 * window */
		...
		if (iw > 0 && ih > 0) {
			...
			if (b->flags & b_Right)
				...
				ix += (iw - b->icon_w) / 2;
			XMoveResizeWindow(Dpy, b->IconWin, ix,
...
RedrawButton(button_info *b, int clean)
{
	...
	/* This probably isn't the place for this, but it seems to work here and
	   not elsewhere, so... */
	...
	RelieveWindow(
	    MyWindow, f, x, y, BW, BH, buttonHilite(b), buttonShadow(b), rev);
	...
	if (clean && BW > 2 * f && BH > 2 * f) {
		...
		if (b->flags & b_Container) {
			...
			if (w1)
				XFillRectangle(
				    Dpy, MyWindow, NormalGC, x1, y1, w1, h);
			if (w2)
				XFillRectangle(Dpy, MyWindow, NormalGC,
				    x1 + w - w2, y1, w2, h);
			if (h1)
				XFillRectangle(
				    Dpy, MyWindow, NormalGC, x1, y1, w, h1);
			if (h2)
...
DrawTitle(button_info *b, Window win, GC gc)
{
	    ...
	    BH >= font->descent + font->ascent) /* Clip it somehow? */
	{
		...
		if ((b->flags & b_Icon || (buttonSwallowCount(b) == 3)) &&
			...
			XDrawString(
			    Dpy, win, gc, xpos, iy + ih - font->descent, s, l);
