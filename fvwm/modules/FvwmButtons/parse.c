...
ParseBoxSize(char **ss, unsigned long *flags)
{
	...
	*ss = GetNextTokenIndex(*ss, opts, 0, &m);
	...
	default:
		*flags &= ~(b_SizeFixed | b_SizeSmart);
		fprintf(
		    stderr, "%s: Illegal boxsize option \"%s\"\n", MyName, s);
		break;
	}
	return;
}
		...
		if (*s == ',')
			...
			case 8: /* Swallow - flags */
				...
				if (*s == '(' && s++) {
					...
					char *temp;
					fprintf(stderr,
					    "%s: Illegal swallow in container "
					    "options\n",
					    MyName);
					temp = seekright(&s);
					if (temp)
...
ParseConfigLine(button_info **ubb, char *s)
{
	...
	case 0: /* Geometry */
	{
		...
		if (i == 1) {
			flags =
			    XParseGeometry(geom, &g_x, &g_y, &width, &height);
			w = -1;
			h = -1;
