...
# endif
  ...
  if(argc>6) /* There is a naming argument here! */
    {
      free(MyName);
  MyName=safestrdup(argv[6]);
    }

  if(argc>7) /* There is a config file here! */
    {
  config_file=safestrdup(argv[7]);
    }
		...
		if(strncasecmp(act,"Exec",4)==0)
		  {
			...
			if(i2-i>1)
			  {
                ...
                memcpy(b->hangon, &act[i+1], (size_t)(i2-i-1));
                b->hangon[i2-i-1] = 0;
			  }
			i2++;
		      }
            ...
            tmp=mymalloc(strlen(act)+6);
            strlcpy(tmp, "Exec ", strlen(act)+6);
		    while(act[i2]!=0 && isspace(act[i2]))
		      i2++;
            strlcat(tmp, &act[i2], strlen(act)+6);
		    MySendText(fd,tmp,0);
		    free(tmp);
		  }
...
# endif
...
#if 0
  myclasshints.res_name=safestrdup((CurrentPanel == MainPanel)
  ...
  {
  myclasshints.res_name=(char *)safemalloc(strlen(MyName)+6);
  strlcpy(myclasshints.res_name, MyName, strlen(MyName)+6);
  strlcat(myclasshints.res_name, "Panel", strlen(MyName)+6);
  }
#endif
