...
Window GetRealGeometry(Display *, Window, int *, int *, ushort *,
...
**/
void DeadPipe(int whatever) { exit(0); }

/**
*** TerminateHandler()
*** Signal handler that will make the event-loop terminate
**/
static RETSIGTYPE TerminateHandler(int sig) { isTerminated = True; }
...
void SetTransparentBackground(button_info *ub, int w, int h)
{
  ...
  while (NextButton(&ub, &b, &i, 0))
  {
    ...
    else
    {
      ...
      XFillRectangle(Dpy, pmap_mask, gc, buttonXPos(b, number),
                     buttonYPos(b, number), buttonWidth(b),
                     buttonHeight(b));
    }
  ...
  XFreeGC(Dpy, gc);
  XShapeCombineMask(Dpy, MyWindow, ShapeBounding, 0, 0, pmap_mask,
                    ShapeSet);
}
#endif
...
int main(int argc, char **argv)
{
  ...
  CurrentPanel = MainPanel = (panel_info *)mymalloc(sizeof(panel_info));
  ...
  while (!isTerminated)
  {
    if (My_XNextEvent(Dpy, &Event))
    {
        ...
        do
          if (PanelIndex->uber->swallow) /* is the panel shown? */
          {
            ...
            if (Event.xany.window == MyWindow)
              CurrentButton = b = select_button(
                UberButton, Event.xbutton.x, Event.xbutton.y);
          }
      ...
      case ButtonRelease:
        ...
        do
        {
          if (PanelIndex->uber->swallow)
          {
            ...
            MyWindow = UberButton->IconWinParent;
            if (Event.xany.window == MyWindow)
              b = select_button(UberButton, Event.xbutton.x,
                                Event.xbutton.y);
          }
        ...
        if (b && b == CurrentButton && act)
        {
          if (strncasecmp(act, "Exec", 4) == 0)
          {
            ...
            i = 4;
            while (act[i] != 0 && act[i] != '"' && isspace(act[i]))
              i++;
            if (act[i] == '"')
            {
              i2 = i + 1;
        ...
        UberButton =
          CurrentPanel->uber; /* the panel, the button pressed */
        MyWindow = UberButton->IconWinParent;
      ...
      case ClientMessage:
        ...
        {
          for (ppi = MainPanel->next; ppi != NULL; ppi = ppi->next)
          {
...
{
  *p =
    CachePicture(Dpy, Root, iconPath, pixmapPath, s, save_color_limit);
  if (*p)
    return 1;
  return 0;
}
...
void RecursiveLoadData(button_info *b, int *maxx, int *maxy)
{
  ...
  fprintf(stderr, "%s: Loading: Button 0x%06x: colors", MyName,
          (ushort)b);
#endif
...
*/
  ...
  if (mysizehints.flags & USPosition)
    ...
    fprintf(stderr, "create(-,-,%u,%u,1,%u,%u)...", mysizehints.width,
            mysizehints.height, (ushort)fore_pix, (ushort)back_pix);
#endif
...

  {
    ...
    char *list[] = {NULL, NULL};
    list[0] =
      (CurrentPanel == MainPanel) ? MyName : CurrentPanel->uber->title;
    if (!XStringListToTextProperty(list, 1, &mynametext))
    {
      fprintf(stderr, "%s: Failed to convert name to XText\n", MyName);
      exit(1);
    }
    XSetWMProperties(Dpy, MyWindow, &mynametext, &mynametext, NULL, 0,
...
#ifdef DEBUG_HANGON
        fprintf(stderr,
                "%s: Button 0x%06x did not find a \"%s\" window, %s",
  ...
  while (NextButton(&ub, &b, &button, 0))
    if (b->flags & b_Hangon && strcmp(cbody, b->hangon) == 0)
    {
      ...
      if (buttonSwallowCount(b) == 1)
      {
        ...
        b->IconWinParent = GetRealGeometry(
          Dpy, b->IconWin, &b->x, &b->y, &b->w, &b->h, &b->bw, &d);

#ifdef DEBUG_HANGON
        fprintf(stderr,
                "%s: Button 0x%06x %s 0x%lx \"%s\", parent 0x%lx\n",
                MyName, (ushort)b, "will swallow window", body[0],
                cbody, b->IconWinParent);
#endif
...
**/
Window GetRealGeometry(Display *dpy, Window win, int *x, int *y,
                       ushort *w, ushort *h, ushort *bw, ushort *d)
{
  Window root;
  Window rp = None;
  Window *children;
...
void swallow(unsigned long *body)
{
  ...
  while (NextButton(&ub, &b, &button, 0))
    if ((b->IconWin == (Window)body[0]) && (buttonSwallowCount(b) == 2))
    {
      ...
      p = GetRealGeometry(Dpy, b->IconWin, &b->x, &b->y, &b->w, &b->h,
                          &b->bw, &d);
#ifdef DEBUG_HANGON
      fprintf(stderr, "%s: Button 0x%06x %s 0x%lx, with parent 0x%lx\n",
              MyName, (ushort)b, "trying to swallow window", body[0],
              p);
#endif

      if (p ==
          None) /* This means the window is no more */ /* NO! wrong */
      {
      ...
      {
        fprintf(
          stderr, "%s: Window 0x%lx (\"%s\") was %s (window 0x%lx)\n",
...
void Slide(panel_info *p, button_info *b)
{
  ...
  else
  {
    ...
    root = GetRealGeometry(Dpy, CurrentPanel->uber->IconWinParent, &x,
                           &y, (ushort *)&iw, (ushort *)&ih,
