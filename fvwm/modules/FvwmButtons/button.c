...
**/
void
buttonInfo(button_info *b, int *x, int *y, int *px, int *py, int *f)
{
...
**/
void
GetInternalSize(button_info *b, int *x, int *y, int *w, int *h)
{
	int f;
	int px, py;
...
**/
int
buttonFrameSigned(button_info *b)
{
	if (b->flags & b_Frame)
		return b->framew;
...
**/
button_info *
alloc_button(button_info *ub, int num)
{
	button_info *b;
	if (num >= ub->c->allocated_buttons)
...
PlaceAndExpandButton(int x, int y, button_info *b, button_info *ub)
{
	...
	if (y >= c->num_rows || y < 0) {
		if (b->flags & b_PosFixed || !(ub->c->flags & b_SizeSmart) ||
		    y < 0) {
			fprintf(stderr,
	...
	if (y + b->BHeight > c->num_rows) {
		if (c->flags & b_SizeSmart) {
			...
			fprintf(
			    stderr, "%s: Button too tall. Giving up\n", MyName);
			fprintf(stderr,
...
ShrinkButton(button_info *b, container_info *c)
{
	...
	if (!b) {
		fprintf(
		    stderr, "error: shrink1: button is empty but shouldn't\n");
		exit(1);
	}
...
ShuffleButtons(button_info *ub)
{
	...
	if (!(c->flags & b_SizeFixed) || !(c->num_rows) || !(c->num_columns)) {
		...
		if (c->num_columns == 0)
			c->num_columns =
			    1 + (actual_buttons_used - 1) / c->num_rows;
		if (c->num_rows == 0)
			c->num_rows =
			    1 + (actual_buttons_used - 1) / c->num_columns;
	...
	if (c->flags & b_SizeSmart) {
		/* Set rows/columns to at least the height/width of largest
		 * button */
		for (i = 0; i < num_items; i++) {
			b = local_buttons[i];
...
**/
button_info *
NextButton(button_info **ub, button_info **b, int *i, int all)
{
	/* Get next button */
	(*i)++;
	/* Skip fake buttons */
...
select_button(button_info *ub, int x, int y)
{
	...
	if (i == -1)
		return ub;
	b = ub->c->buttons[i];
	return select_button(b,
	    x - (i % ub->c->num_columns) * ub->c->ButtonWidth,
