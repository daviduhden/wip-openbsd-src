...
DumpButtons(button_info *b)
{
	...
	if (b != UberButton) {
		...
		fprintf(stderr,
		    "0x%lx(%ix%i@(%i,%i),0x%04lx): ", (unsigned long)b,
		    b->BWidth, b->BHeight, buttonXPos(b, button),
		    buttonYPos(b, button), b->flags);
	} else
		fprintf(stderr, "0x%lx(%ix%i@,0x%04lx): ", (unsigned long)b,
		    b->BWidth, b->BHeight, b->flags);

	if (b->flags & b_Font)
		fprintf(stderr, "Font(%s,0x%lx) ", b->font_string,
		    (unsigned long)b->font);
	...
	if (b->flags & b_Container) {
		int i = 0;
		fprintf(stderr,
		    "  Container(%ix%i=%i buttons 0x%04lx (alloc %i), size "
		    "%ix%i, pos %i,%i)\n{ ",
		    b->c->num_columns, b->c->num_rows, b->c->num_buttons,
		    b->c->flags, b->c->allocated_buttons, b->c->ButtonWidth,
...
SaveButtons(button_info *b)
{
	...
	if (b->flags & b_Action) {
		...
		for (i = 1; i < 4; i++)
			if (b->action[i])
				fprintf(stderr, "Action (Mouse %i) `%s` ", i,
				    b->action[i]);
	}

	if (b->flags & b_Container) {
		fprintf(stderr, "Container (Columns %i Rows %i ",
		...
		if (b->c->flags) {
			...
			if (b->c->flags & b_Padding)
				fprintf(stderr, "Padding %i %i ", b->c->xpad,
				    b->c->ypad);
