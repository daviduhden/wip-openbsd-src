...
int main(int argc, char **argv)
{
  ...
  {
    size_t cap = strlen(temp) + 2;
    MyName = safemalloc(cap);
    strlcpy(MyName, "*", cap);
    strlcat(MyName, temp, cap);
  }
...
void list_window_name(unsigned long *body)
{
  if((app_win == (Window)body[1])||(app_win == (Window)body[0]))
    {
  strlcpy(target.name, (char *)&body[3], 256);
    }
}
...
void list_icon_name(unsigned long *body)
{
  if((app_win == (Window)body[1])||(app_win == (Window)body[0]))
    {
  strlcat(target.icon_name, (char *)&body[3], 256);
    }
}
...
void list_class(unsigned long *body)
{
  if((app_win == (Window)body[1])||(app_win == (Window)body[0]))
    {
  strlcat(target.class, (char *)&body[3], 256);
    }
}
...
void list_res_name(unsigned long *body)
{
  if((app_win == (Window)body[1])||(app_win == (Window)body[0]))
    {
  strlcat(target.res, (char *)&body[3], 256);
    }
}
...
/**********************************************************************
 *
 * If no application window was indicated on the command line, question
