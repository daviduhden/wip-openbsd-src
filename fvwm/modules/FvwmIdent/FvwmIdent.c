...
list_end(void)
{
	...
	XQueryPointer(dpy, Root, &JunkRoot, &JunkChild, &x, &y, &JunkX, &JunkY,
	    &JunkMask);
...
GetTargetWindow(Window *app_win)
{
	...
	while ((trials < 100) && (val != GrabSuccess)) {
		val = XGrabPointer(dpy, Root, True, ButtonReleaseMask,
		...
		AddToList("  - WM_TAKE_FOCUS:", tfstr);
		{
			...
			if ((getrc = XGetWMSizeHints(dpy,
				...
				if (supplied_return &
					...
					AddToList("Minimum aspect ratio:",
					    mymin_aspect);
					snprintf(max_aspect, sizeof(max_aspect),
					    "%d/%d", size_hints->max_aspect.x,
					    size_hints->max_aspect.y);
					AddToList("Maximum aspect ratio:",
					    max_aspect);
				} /* end aspect ratio */
				XFree(size_hints);
