...
#define SomeButtonDown(a)                                              \
  ((a) & Button1Mask || (a) & Button2Mask || (a) & Button3Mask)
...
GC graph[MAX_COLOUR_SETS], shadow[MAX_COLOUR_SETS],
  hilite[MAX_COLOUR_SETS];
...
char *Module;
int WindowIsUp = 0, win_width = 5, win_height = 5, win_grav, win_x,
    win_y, win_title, win_border;
int Clength, Transient = 0, Pressed = 0, ButPressed, Checked = 0;
int MinWidth = DEFMINWIDTH, MaxWidth = DEFMAXWIDTH;
ButtonArray buttons;
List windows;
char *ClickAction[3] = {"Iconify -1,Raise", "Iconify", "Lower"},
     *EnterAction, *BackColor[MAX_COLOUR_SETS] = {"white"},
...
int main(int argc, char **argv)
{
  ...
  SetMessageMask(
    Fvwm_fd, M_CONFIGURE_WINDOW | M_RES_CLASS | M_RES_NAME |
               ...
               M_DEICONIFY | M_ICONIFY | M_END_WINDOWLIST | M_NEW_DESK |
               M_NEW_PAGE | M_FOCUS_CHANGE | M_WINDOW_NAME |
#ifdef MINI_ICONS
               M_MINI_ICON |
#endif
               M_STRING);
...
void ProcessMessage(unsigned long type, unsigned long *body)
{
  ...
  case M_ICONIFY:
    ...
    if (type == M_DEICONIFY && !(flags & ICONIFIED))
      break;
    if (type == M_ICONIFY && flags & ICONIFIED)
      break;
    flags ^= ICONIFIED;
    ...
    if (i != current_focus || (flags & ICONIFIED))
      if (UpdateButtonSet(&buttons, i, (flags & ICONIFIED) ? 1 : 0) !=
          -1)
        redraw = 1;
    free(name);
    break;
...
void ParseConfig(void)
{
  ...
  while (tline != (char *)0)
  {
    if (strlen(tline) > 1)
    {
      ...
      else if (strncasecmp(tline, CatString3(Module, "Fore", ""),
                           Clength + 4) == 0)
      ...
      else if (strncasecmp(tline, CatString3(Module, "Back", ""),
                           Clength + 4) == 0)
                           ...
                           Clength + 9) == 0)
      {
        ...
        CopyString(&BackColor[3], &tline[Clength + 9]);
                           ...
                           Clength + 8) == 0)
        Anchor = 0;
      else if (strncasecmp(tline, CatString3(Module, "Action", ""),
                           Clength + 6) == 0)
...
void LoopOnEvents(void)
{
  ...
  {
    XQueryPointer(dpy, win, &dummyroot, &dummychild, &x1, &y1, &x, &y,
                  &dummy1);
  ...
  while (XPending(dpy))
  {
    ...
    case LeaveNotify:
      if (!SomeButtonDown(Event.xcrossing.state))
        break;
      if (ButPressed != -1)
        SwitchButton(&buttons, ButPressed);
      Pressed = 0;
      break;
    case MotionNotify:
      if (!Pressed)
...
void MakeMeWindow(void)
{
  ...
  for (i = 0; i != MAX_COLOUR_SETS; i++)
    ...
    {
      back[i] =
        GetColor(BackColor[i] == NULL ? BackColor[0] : BackColor[i]);
      fore[i] =
        GetColor(ForeColor[i] == NULL ? ForeColor[0] : ForeColor[i]);
    }

  win = XCreateSimpleWindow(dpy, Root, hints.x, hints.y, hints.width,
                            hints.height, 0, fore[0], back[0]);
  ...
  if (!Transient)
  {
    XGrabButton(dpy, 1, AnyModifier, win, True, GRAB_EVENTS,
                GrabModeAsync, GrabModeAsync, None, None);
    XGrabButton(dpy, 2, AnyModifier, win, True, GRAB_EVENTS,
                GrabModeAsync, GrabModeAsync, None, None);
