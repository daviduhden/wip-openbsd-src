...
int main(int argc, char **argv)
{
  ...
  /* Setup my name */
  {
    ...
    Module = safemalloc(cap);
    strlcpy(Module, "*", cap);
    strlcat(Module, temp, cap);
  }
...
char *ptr;
  ...
  *ptr = '\0';
  if (flags & ICONIFIED) strlcpy(ptr, "(", strlen(string) + 3);
  else ptr[0] = '\0';
  strlcat(ptr, string, strlen(string) + 3);
  if (flags & ICONIFIED) strlcat(ptr, ")", strlen(string) + 3);
  return ptr;
}
