...
ProcessMessage(unsigned long type, unsigned long *body)
{
	...
	case M_ICON_NAME:
		if ((type == M_ICON_NAME && !UseIconNames) ||
			...
			break;
		if ((i = UpdateItemName(&windows, body[0], (char *)&body[3])) ==
		    -1)
			break;
		string = (char *)&body[3];
		...
		if (UpdateButton(&buttons, i, name, -1) == -1) {
			...
			UpdateButtonDesk(
			    &buttons, i, ItemDesk(&windows, body[0]));
		}
		free(name);
		if (WindowIsUp)
			AdjustWindow();
...
LoopOnEvents(void)
{
	...
	if (Transient && !Checked) {
		XQueryPointer(dpy, win, &dummyroot, &dummychild, &x1, &y1, &x,
		    &y, &dummy1);
	...
	while (XPending(dpy)) {
		...
		case EnterNotify:
			if (!SomeButtonDown(Event.xcrossing.state))
				break;
			num = WhichButton(
			    &buttons, Event.xcrossing.x, Event.xcrossing.y);
			if (num != -1) {
				SwitchButton(&buttons, num);
				ButPressed = num;
			} else
		...
		case MotionNotify:
			if (!Pressed)
				break;
			num = WhichButton(
			    &buttons, Event.xmotion.x, Event.xmotion.y);
			if (num == ButPressed)
				break;
			if (ButPressed != -1)
				SwitchButton(&buttons, ButPressed);
...
AdjustWindow(void)
{
	...
	if (WindowIsUp &&
		...
		if (Anchor) {
			...
			XMoveResizeWindow(
			    dpy, win, win_x, win_y, new_width, new_height);
			MyXUngrabServer(dpy);
		} else
...
MakeMeWindow(void)
{
	...
	if (geometry != NULL) {
		...
		if (ret & XValue && ret & YValue) {
			...
			if (ret & XNegative)
				hints.x +=
				    XDisplayWidth(dpy, screen) - win_width;

			hints.y = y;
			if (ret & YNegative)
				hints.y +=
				    XDisplayHeight(dpy, screen) - win_height;
