  ...
  while ((pathlist)&&(*pathlist))
    {
      ...
      if (dir_end != NULL)
  {
    size_t dlen = (size_t)(dir_end - pathlist);
    if (dlen >= pathlen) dlen = pathlen - 1;
    memcpy(path, pathlist, dlen);
    path[dlen] = '\0';
  }
      else
...
Bool xpmcolor_require_c_color(XpmColor *p)
{
  ...
  p->c_color = safestrdup(p->g_color);
  else if (p->g4_color != NULL)
  p->c_color = safestrdup(p->g4_color);
  else if (p->m_color != NULL)
  p->c_color = safestrdup(p->m_color);
  else
  p->c_color = safestrdup("none");
