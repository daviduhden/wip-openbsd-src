...
FvwmPicture *LoadPicture(Display *dpy, Window Root, char *path,
                         int color_limit)
{
  int l;
  FvwmPicture *p;
  ...
  if (XReadBitmapFile(dpy, Root, path, &p->width, &p->height,
                      &p->picture, &l, &l) == BitmapSuccess)
  {
    p->depth = 0;
    p->mask = None;
    return p;
...
FvwmPicture *CachePicture(Display *dpy, Window Root, char *IconPath,
                          char *PixmapPath, char *name, int color_limit)
{
  char *path;
...
static Color_Info base_array[] = {
  ...
  {"magenta"},        {"DodgerBlue"},  {"SteelBlue"}, {"chartreuse"},
  {"wheat"},          {"turquoise"},   {"CadetBlue"}, {"gray87"},
  {"CornflowerBlue"}, {"YellowGreen"}, {"NavyBlue"},  {"MediumBlue"},
  {"plum"},           {"aquamarine"},  {"orchid"},    {"ForestGreen"},
  {"lightyellow"},    {"brown"},       {"orange"},    {"red3"},
  {"HotPink"},        {"LightBlue"},   {"gray47"},    {"pink"},
  {"red4"},           {"violet"},      {"purple"},    {"gray63"},
  {"gray94"},         {"plum1"},       {"PeachPuff"}, {"maroon"},
  {"lavender"},       {"salmon"}, /* for peachpuff, orange gap */
...
void c200_substitute_color(char **my_color, int color_limit)
{
  ...
  *my_color = safemalloc(len); /* area for new color */
  strlcpy(*my_color, base_array[minind].c_color,
          len); /* put it there */
  return;       /* all done */
}
