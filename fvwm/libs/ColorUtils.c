...
enum ColorChannel { CHANNEL_RED = 0, CHANNEL_GREEN = 1, CHANNEL_BLUE = 2 };

static void
color_mult(unsigned short *red, unsigned short *green, unsigned short *blue,
    double factor)
{
	double components[3];
	...
	if (lightness <= HALF_SCALE) {
		...
		new_max =
		    saturation * SCALE + lightness - saturation * lightness;
	}
