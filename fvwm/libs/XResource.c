			     ...
			     char *bindstr)
{
  ...
  {
    size_t add = (size_t)(end - line);
    size_t cur = strlen(resource);
    size_t rem = (reslen > cur) ? reslen - cur : 0;
    if (rem > 0)
    {
      /* strlcat appends up to siz - strlen(dst) - 1 */
      if (add >= rem) add = rem - 1;
      char saved = line[add];
      ((char *)line)[add] = '\0';
      strlcat(resource, line, reslen);
      ((char *)line)[add] = saved;
    }
  }
